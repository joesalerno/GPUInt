...
export function readDataFromTexture(gl, framebuffer, width, height, extractSingleComponent = true) {
    ...
    console.log(`[webgl-utils readDataFromTexture] received width: ${width}, height: ${height}, extractSingleComponent: ${extractSingleComponent}`); // DEBUG
    const pixelDataRGBA = new Float32Array(width * height * 4); // RGBA
    console.log(`[webgl-utils readDataFromTexture] allocated pixelDataRGBA.length: ${pixelDataRGBA.length}`); // DEBUG
    ...
    if (extractSingleComponent) {
        ...
        // console.log(`Data read from texture (${width}x${height}) and R component extracted.`); // Removed
        return singleComponentData;
    } else {
        // console.log(`Data read from texture (${width}x${height}) as RGBA.`); // Removed
