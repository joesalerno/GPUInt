...
describe('BigIntPrimitive WebGL Operations', () => {
  ...
  describe('multiply() - WebGL Path (_webgl_multiply_full integration)', () => {
    ...
    it('should multiply "7" by "8" (WebGL path, specific debug case)', async () => {
      ...
      expect(logs).toContain("[Multiply Debug 7*8] Pass 1 (PartialProducts) output (RGBA, 1x1): [56,0,0,1]"); // P00 = 7*8=56. (56%BASE, 0)
      expect(logs).toContain("[Multiply Debug 7*8] Pass 2 (SumAntiDiagonals) output (RGBA, 1x1): [56,0,0,1]"); // S0 = P00. (56%BASE, 0) - Blue channel is 0.0
      ...
      // Iteration 1: Input Tex[0]=(56,0), Tex[1]=(0,0)
      // k=0: val=56, left_g=0. sum=56. out.r=56, out.g=0
      // k=1: val=0, left_g (from k=0 output.g) =0. sum=0. out.r=0, out.g=0
      expect(logs).toContain("[Multiply Debug 7*8] Pass 3 (Carry Propagation) iter 1 output (R,G pairs): [56,0,0,0]");
