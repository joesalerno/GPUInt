...
describe('BigIntPrimitive WebGL Operations', () => {
  ...
  describe('multiply() - WebGL Path (Simple Cases)', () => {
    it('[_webgl_multiply_one_limb_by_bigint] should multiply limb 12 by BigInt "3" (Actual GPU)', async () => {
        ...
        // Regex updated to include "Step 3" and ensure it correctly captures u_carryTexture
        expect(logs).toMatch(/\[WebGL MultLimb Debug Step 3\] Locations:.*u_carryTexture=\w+/);
        expect(logs).toContain("[WebGL MultLimb Debug Step 3] Textures created successfully."); // Log message includes "Step 3"
        expect(logs).toContain("[WebGL MultLimb Debug] Drawing arrays...");
        expect(logs).toContain("[WebGL MultLimb Debug] Raw outputDataRGBA:");
        expect(logs).toContain("[WebGL MultLimb Debug] Processing original shader output:"); // Switched from debug-specific log
        expect(logs).toContain("[WebGL MultLimb Debug] webGLResult.toString(): 36"); // Expecting the correct result to be logged
