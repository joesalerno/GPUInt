...
describe('BigIntPrimitive WebGL Operations', () => {
  ...
  describe('multiply() - WebGL Path (Simple Cases)', () => {
    it('[_webgl_multiply_one_limb_by_bigint] should multiply limb 12 by BigInt "3" (Actual GPU)', async () => {
        ...
        expect(logs).toContain("[WebGL MultLimb Debug] WebGL Result: 36");
  ...
  describe('multiply() - WebGL Path (_webgl_multiply_full integration)', () => {
    ...
    it('should multiply two large multi-limb numbers (WebGL path)', async () => {
      ...
      const expected = "12193263113702171333485751812224";
      ...
      // For large numbers, WebGL may fall back to CPU due to precision issues, which is acceptable
      ...
      // For large numbers, fallback to CPU is acceptable due to precision limits
      if (fellBackToCpu) {
          console.log(`[Test Info] WebGL fell back to CPU for large numbers (${numStr1}*${numStr2}), which is acceptable for precision reasons.`);
      }
  ...
  describe('divide() - WebGL Path', () => {
    ...
    it('should divide "10" by "2" using WebGL path', async () => {
      ...
      expect(logs).toContain("[_webgl_reciprocal_using_bigint_ops Iter 2]");
