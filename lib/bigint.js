...
import { gpuOps } from './gpu-operations.js'
...
class BigIntPrimitive {
  ...
  add(otherBigInt) {
    ...
    const timer = PerformanceMonitor.startTimer('add');
    ...
    try {
      ...
      // Try optimized GPU path first
      if (!this.forceCPU && !otherBigInt.forceCPU && this.canvas) {
        const gpuTimer = PerformanceMonitor.startTimer('add', 'gpu');
        try {
          if (this.sign !== otherBigInt.sign) {
            // Subtraction case, fall through to CPU or handle directly
          } else {
            // Optimized GPU addition for same signs
            const commonExponent = Math.min(this.exponent, otherBigInt.exponent)
            
            // Work directly with limbs, but adjust for exponent differences
            let thisLimbs = [...this.limbs];
            let otherLimbs = [...otherBigInt.limbs];
            
            // Pad arrays to handle exponent differences
            const thisExpOffset = this.exponent - commonExponent;
            const otherExpOffset = otherBigInt.exponent - commonExponent;
            
            // Pad front (lower significance) for exponent differences
            for (let i = 0; i < thisExpOffset; i++) thisLimbs.unshift(0);
            for (let i = 0; i < otherExpOffset; i++) otherLimbs.unshift(0);

            // Use optimized GPU operations (synchronous version)
            const resultLimbs = gpuOps.add(this.canvas, thisLimbs, this.exponent, otherLimbs, otherBigInt.exponent, this.sign)
            
            // Create result directly from limbs
            const webGLResult = new BigIntPrimitive(0, this.canvas, { forceCPU: true });
            webGLResult.limbs = resultLimbs;
            webGLResult.sign = this.sign;
            webGLResult.exponent = commonExponent;
            
            // Remove leading zeros
            while (webGLResult.limbs.length > 1 && webGLResult.limbs[webGLResult.limbs.length - 1] === 0) {
              webGLResult.limbs.pop();
            }
            ...
            PerformanceMonitor.endTimer(gpuTimer);
            return webGLResult
          }
        } catch (e) {
          PerformanceMonitor.endTimer(gpuTimer);
          PerformanceMonitor.recordGpuMetric('fallbacksToCpu');
      ...
      const cpuTimer = PerformanceMonitor.startTimer('add', 'cpu');
      try {
        ...
        if (this.sign === otherBigInt.sign) {
          ...
          const subtractResult = this.subtract(otherBigInt.negate())
          PerformanceMonitor.endTimer(cpuTimer);
  ...
  multiply(otherBigInt) {
    const self = this;
    const timer = PerformanceMonitor.startTimer('multiply');
    ...
    try {
      ...
      let absResult;
      if (!self.forceCPU && !otherBigInt.forceCPU && self.canvas) {
        const gpuTimer = PerformanceMonitor.startTimer('multiply', 'gpu');
        try {
          // Use optimized GPU operations
          const selfLimbsLSL = [...selfCoeff.limbs].reverse();
          const otherLimbsLSL = [...otherCoeff.limbs].reverse();
          
          const resultLimbsLSL = gpuOps.multiplyFull(self.canvas, selfLimbsLSL, otherLimbsLSL);
          
          // Convert back to coefficient string
          let finalCoeffStr = "";
          if (resultLimbsLSL.length === 0) {
            finalCoeffStr = "0";
          } else {
            let tempFinalLimbs = [...resultLimbsLSL].reverse();
          ...
          absResult = BigIntPrimitive.fromCoefficientString(finalCoeffStr, self.canvas, { forceCPU: true });
          PerformanceMonitor.endTimer(gpuTimer);
        } catch (e) {
          PerformanceMonitor.endTimer(gpuTimer);
          PerformanceMonitor.recordGpuMetric('fallbacksToCpu');
          console.warn(`[BigInt.multiply] WebGL multiplication failed: ${e.message}. Falling back to CPU.`);
          const cpuTimer = PerformanceMonitor.startTimer('multiply', 'cpu');
          try {
...
BigIntPrimitive.prototype._webgl_multiply_one_limb_by_bigint = function (limbValue, otherBigInt) {
  ...
  try {
    // Use optimized GPU operations
    const otherLimbsLSL = [...otherBigInt.limbs].reverse();
    const resultLimbsLSL = gpuOps.multiplyLimbByBigint(this.canvas, limbValue, otherLimbsLSL);
    ...
    console.log(`[WebGL MultLimb Debug] WebGL Result: ${finalCoeffStr}`);
    return result;
