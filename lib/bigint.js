...
class BigIntPrimitive {
  ...
  cmp(otherBigInt) {
    ...
    if (thisIsZero && otherIsZero) { return 0; } // Both zero
    // If signs differ, the positive number is greater (or negative is smaller)
    // This also handles one number being zero correctly (e.g., 5 vs 0, -5 vs 0)
    if (this.sign !== otherBigInt.sign) {
        return this.sign > otherBigInt.sign ? 1 : -1;
    }

    // Signs are the same and non-zero (otherwise caught by isZero checks or different signs)
    let magResult = this.compareMagnitude(otherBigInt);
    if (this.sign === 1) { // Both positive
        return magResult;
    } else { // Both negative
        // If magnitudes are equal, result is 0 (positive zero)
        // Otherwise, invert the magnitude comparison result
