...
class BigIntPrimitive {
  ...
  _core_subtract(positiveOtherBigInt) {
      ...
      resultNumCPU.limbs = finalResultLimbs;
      resultNumCPU.sign = 1;
      if (resultNumCPU.isZero()) { resultNumCPU.sign = 1; } // exponent is already 0 from constructor
      return resultNumCPU;
  }
  ...
  _multiply_limb_by_bigint(limbValue, otherNumber) {
   ...
   _core_multiply(num1, num2) {
    ...
    for (let i = 0; i < n1_limbs_reversed.length; i++) {
        ...
        // Before shifting, ensure partialProductMagnitude has an exponent of 0 relative to its limbs
        // This is implicitly handled by _multiply_limb_by_bigint returning a new BigInt
        // Then, adjust exponent for the positional value of digitOfNum1
        partialProductMagnitude.exponent += i; // THIS IS THE KEY CHANGE for _core_multiply based on original logic.
                                             // The original _multiplyByPowerOfBase was effectively doing this.
                                             // But since _multiply_limb_by_bigint returns a new BigInt with exp 0,
                                             // we adjust its exponent here.

        totalResult = totalResult.add(partialProductMagnitude); // public add handles exponents
        if (!totalResult) { throw new Error("Error during accumulation in _core_multiply."); }
    }
    totalResult.sign = 1; // Magnitude is positive
    if (totalResult.isZero()) totalResult.sign = 1; // Ensure normalized zero is positive
    return totalResult;
  }
  ...
  multiply(otherBigInt) {
    ...
    const absThisLen = absThis.limbs.length + absThis.exponent; // Effective length based on MSD
    const absOtherLen = absOther.limbs.length + absOther.exponent; // Effective length
    ...
    if (absThisLen < KARATSUBA_THRESHOLD || absOtherLen < KARATSUBA_THRESHOLD) {
        ...
        const n = Math.max(absThis.limbs.length, absOther.limbs.length); // Karatsuba typically splits based on raw limb length
