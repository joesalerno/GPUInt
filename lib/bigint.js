...
import propagateCarriesSinglePassFragmentShaderSrc from './shaders/propagate_carries_singlepass.frag?raw'
...
class BigIntPrimitive {
    ...
    if (!this.forceCPU && !otherBigInt.forceCPU && this.canvas && typeof webglUtilsModule !== 'undefined') {
      try {
        ...
        if (this.sign !== otherBigInt.sign) {
          ...
          const finalData = webglUtilsModule.readDataFromTexture(gl, carryFbOutput, texWidth, 1, true);
          const actualLimbsArrLSL = [...finalData];
          ...
          const finalLimbsMSL = actualLimbsArrLSL.reverse();
  ...
  _webgl_multiply_full(cache, num1, num2) { // Added cache as first parameter
    const gl = cache.gl;
    ...
    try {
      ...
      gl.uniform1i(gl.getUniformLocation(sumAntidiagonalsProgram, "u_partialProductsTexture"), 0);
            gl.uniform1f(gl.getUniformLocation(sumAntidiagonalsProgram, "u_texWidth"), partialProductTexDim);
      ...
      // Pass 3: Iterative Propagate Carries (Ping-Pong)
      const propagateTexWidth = outputTexWidthCoeffSums + 5; // Extra padding for safety
      const initialData = new Float32Array(propagateTexWidth * 4);
      for (let i = 0; i < outputTexWidthCoeffSums; i++) {
          initialData[i * 4] = coeffSumsRawData[i * 4]; // R = sum
          initialData[i * 4 + 1] = coeffSumsRawData[i * 4 + 1]; // G = carry
      }

      texPropagatePing = webglUtilsModule.createDataTexture(gl, initialData, propagateTexWidth, 1, true);
      ...
      propagateCarriesProgram = BigIntPrimitive._getOrCreateProgram(cache, 'carry_mult', vertexShaderSrc, propagateCarriesSinglePassFragmentShaderSrc);
      ...
      gl.uniform1i(gl.getUniformLocation(propagateCarriesProgram, "u_inputTexture"), 0);
      ...
      const MAX_CARRY_PASSES = propagateTexWidth;

      let readTex = texPropagatePing;
      let writeFb = fbPropagatePong;

      for (let i = 0; i < MAX_CARRY_PASSES; i++) {
          ...
          // Swap for next pass
          let tempTex = readTex;
          readTex = texPropagatePong;
          texPropagatePong = tempTex;

          let tempFb = writeFb;
          writeFb = fbPropagatePing;
          fbPropagatePing = tempFb;
      }

      gl.bindFramebuffer(gl.FRAMEBUFFER, writeFb);
      const finalResultRGBA = webglUtilsModule.readDataFromTexture(gl, writeFb, propagateTexWidth, 1, false);
