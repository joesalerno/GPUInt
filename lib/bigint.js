...
import propagateCarriesFragmentShaderSrc from './shaders/propagate_carries_singlepass.frag?raw'
...
class BigIntPrimitive {
  ...
  _webgl_subtract(gl, minuendLimbsLSLPadded, subtrahendLimbsLSLPadded, texWidth) {
    ...
    let program, texMinuend, texSubtrahend, texInput, fbInput, positionBuffer;
    try {
      ...
      // Prepare input RGBA: R = limb1 - limb2, G = 0 (borrow), B = 0, A = 1
      const inputData = new Float32Array(texWidth * 4);
      for (let i = 0; i < texWidth; i++) {
        inputData[i * 4 + 0] = minuendLimbsLSLPadded[i] - subtrahendLimbsLSLPadded[i];
        inputData[i * 4 + 1] = 0;
        inputData[i * 4 + 2] = 0;
        inputData[i * 4 + 3] = 1;
      }
      texInput = webglUtilsModule.createDataTexture(gl, inputData, texWidth, 1, true);
      fbInput = gl.createFramebuffer();
      gl.bindFramebuffer(gl.FRAMEBUFFER, fbInput);
      gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, texInput, 0);
      const fbStatus = gl.checkFramebufferStatus(gl.FRAMEBUFFER);
      if (fbStatus !== gl.FRAMEBUFFER_COMPLETE) {
        throw new Error(`WEBGL_FRAMEBUFFER_ERROR: Framebuffer incomplete for subtraction input. Status: ${webglUtilsModule.getFramebufferStatusString(gl, fbStatus)}`);
      }

      // Use the new single-pass propagate_carries shader
      const vsPropagate = webglUtilsModule.createShader(gl, gl.VERTEX_SHADER, vertexShaderSrc);
      const fsPropagate = webglUtilsModule.createShader(gl, gl.FRAGMENT_SHADER, propagateCarriesFragmentShaderSrc);
      program = webglUtilsModule.createProgram(gl, vsPropagate, fsPropagate);
      ...
      gl.uniform1f(gl.getUniformLocation(program, "u_textureWidth"), texWidth);
      ...
      gl.finish();
      // Read result
      const finalData = webglUtilsModule.readDataFromTexture(gl, fbInput, texWidth, 1, false);
      const actualLimbsArrLSL = new Array(texWidth);
      for (let i = 0; i < texWidth; i++) actualLimbsArrLSL[i] = Math.round(finalData[i * 4 + 0]);
      ...
      if (texInput) gl.deleteTexture(texInput);
      if (fbInput) gl.deleteFramebuffer(fbInput);
      if (program) gl.deleteProgram(program);
      if (positionBuffer) gl.deleteBuffer(positionBuffer);
      return actualLimbsArrLSL;
    } finally {
      // No-op: all resources cleaned up above
    }
  }
  ...
  _webgl_multiply_full(gl, num1, num2) { // Added gl as first parameter
    ...
    try {
      ...
      // Pass 3: Single-pass Propagate Carries (parallel prefix sum in-shader)
      ...
      gl.activeTexture(gl.TEXTURE0);
      gl.bindTexture(gl.TEXTURE_2D, texPropagatePing);
      gl.uniform1i(uInputTexLocProp, 0);
      gl.uniform1f(uBaseLocProp, BASE);
      gl.uniform1f(uTexWidthLocProp, propagateTexWidthForTexture);
      gl.drawArrays(gl.TRIANGLES, 0, 6);
      gl.finish();
      const finalResultRGBA = webglUtilsModule.readDataFromTexture(gl, fbPropagatePing, propagateTexWidthForTexture, 1, false);
