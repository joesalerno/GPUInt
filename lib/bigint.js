...
class BigIntPrimitive {
  ...
  constructor(value, canvas, options = {}) {
    ...
    let originalStringForSignCheck = "";
    let stringValue = '';
    ...
    if (typeof value === 'number') {
      ...
      if (value === 0 && 1/value < 0) {
          this.sign = -1; // Capture -0 from number input
      }
    } else if (typeof value === 'string') {
      originalStringForSignCheck = value.trim(); // Keep original for "-0" check
      stringValue = originalStringForSignCheck;
    } else {
    ...
    // Initial sign parsing from string (if not already set by numeric -0)
    ...
    // If no sign char, this.sign remains its current value (1 by default, or -1 if numeric -0)
    ...
    // Check if the coefficient string (after removing decimal, but before removing leading zeros) is all zeros.
    let allZerosInCoeff = true;
    for (let i = 0; i < coefficientStr.length; i++) {
        if (coefficientStr[i] !== '0') {
            allZerosInCoeff = false;
            break;
        }
    }

    if (allZerosInCoeff) { // Handles "0", "0.0", "000" etc.
        this.limbs = [0];
        this.exponent = 0;
        // Sign is already parsed. If original was "-0", sign is -1. Otherwise, it's 1.
        return;
    }
    ...
    coefficientStr = coefficientStr.replace(/^0+/, ''); // Remove leading zeros from non-zero coefficient
    // If coefficientStr became empty, it means it was all zeros, handled above.
    ...
    while (this.limbs.length > 1 && this.limbs[this.limbs.length - 1] === 0) { this.limbs.pop(); this.exponent += BASE_LOG10; }

    if (this.isZero()) { // Should only be true if limbs array is [0]
      this.exponent = 0;
      // Sign is preserved from initial parsing for "-0"
    }
  }
}
...
BigIntPrimitive.prototype.negate = function() { const n = new BigIntPrimitive(this, this.canvas, {forceCPU:this.forceCPU}); if(!n.isZero() || n.sign === -1) n.sign *= -1; return n;};
...
BigIntPrimitive.prototype.toString = function() {
  if (this.isZero() && this.sign === -1) return "0"; // toString() normalizes -0 to "0"
  if (this.isZero()) {
    if (typeof this._roundedDp === 'number' && this._roundedDp > 0) return '0.' + '0'.repeat(this._roundedDp);
    return "0";
  }
  ...
  if (this.sign === -1 && !(s === "0" || (s.startsWith("0.") && parseFloat(s) === 0))) return "-" + s;
  return s;
};
...
BigIntPrimitive.prototype.add = function(other) {
  if(!(other instanceof BigIntPrimitive)) other=new BigIntPrimitive(other.toString(),this.canvas,{forceCPU:true});
  if(this.isZero() && this.sign === 1) return new BigIntPrimitive(other,this.canvas,{forceCPU:this.forceCPU||other.forceCPU});
  if(other.isZero() && other.sign === 1) return new BigIntPrimitive(this,this.canvas,{forceCPU:this.forceCPU||other.forceCPU});
  if (this.isZero() && this.sign === -1) { const res = new BigIntPrimitive(other); res.sign = other.sign; if (other.isZero() && other.sign === -1) res.sign = 1; return res;}
  if (other.isZero() && other.sign === -1) { const res = new BigIntPrimitive(this); res.sign = this.sign; if (this.isZero() && this.sign === -1) res.sign = 1; return res;}
  ...
  if(res.isZero()){ res.sign= (this.sign === -1 && other.sign === -1) ? -1 : 1; res.exponent=0;}
  return res;
};
...
BigIntPrimitive.prototype.subtract = function(other) {
  ...
  if (this.isZero() && other.isZero()) { return new BigIntPrimitive( (this.sign === -1 && other.sign === 1) ? "-0" : "0", this.canvas); }
  const comparison = this.compareMagnitude(other);
  if (comparison === 0 && this.sign === other.sign) return new BigIntPrimitive("0", this.canvas, { forceCPU: true });
...
BigIntPrimitive.prototype.multiply = function(other) {
  ...
  if (self.isZero() || other.isZero()) {
    let resultSign = self.sign * other.sign;
    // Special -0 handling for multiplication as per big.js
    if ( (self.isZero() && self.sign === -1 && other.sign === 1 && !other.isZero()) ||
         (other.isZero() && other.sign === -1 && self.sign === 1 && !self.isZero()) ) {
      resultSign = -1;
    } else {
      resultSign = 1; // All other 0 products are +0
    }
    const zero = new BigIntPrimitive("0", self.canvas, { forceCPU: self.forceCPU || other.forceCPU });
...
BigIntPrimitive.prototype.pow = function(exp){
  ...
  if(this.isZero()) {
    if (this.sign === -1 && exp % 2 !== 0) return new BigIntPrimitive("-0", this.canvas, cO);
    return new BigIntPrimitive("0", this.canvas, cO);
  }
...
BigIntPrimitive.prototype._decimalDivide_cpu = function(absDividend, absDivisor) {
  ...
  if (result.isZero()) { result.exponent = 0; result.sign = 1; } // Standardize +0 for result of division
...
BigIntPrimitive.prototype.divideAndRemainder = function(divisor) {
  ...
  if (this.isZero()) return { quotient: new BigIntPrimitive("0", this.canvas, {forceCPU:true}), remainder: new BigIntPrimitive(this) }; // Preserve sign of -0 if this is -0
  ...
  r_coeff.sign = remainderSign; if (r_coeff.isZero()) r_coeff.sign = 1; // Remainder 0 is always +0
...
BigIntPrimitive.prototype.divide = function(n) {
  ...
  if (this.isZero()) { // Handle 0/x and -0/x
    const zero = new BigIntPrimitive("0", this.canvas, { forceCPU: this.forceCPU || n.forceCPU });
    if (this.sign === -1) zero.sign = -1 * n.sign; // (-0)/X -> sign is -X.sign
    else zero.sign = 1 * n.sign; // 0/X -> sign is X.sign (becomes 1 if X is also 0)
    if (zero.isZero()) zero.sign = 1; // Normalize 0 result unless specific -0 rules apply elsewhere
    return zero;
  }
...
BigIntPrimitive.prototype.remainder = function(n) {
  ...
  // For 0 % x = 0. If this was -0, remainder should be -0.
  if (this.isZero()) {
    return new BigIntPrimitive(this); // Preserves sign of this if it's -0
  }
  const original_x_sign = this.sign;

  // Store and temporarily set BigIntPrimitive.DP for the division part of remainder
  const Ctor = this.constructor;
  const oldDP = Ctor.DP;
  const oldRM = Ctor.RM; // Though RM is not directly used by divide, good practice

  // big.js: The result of x.div(y) is effectively truncated to an integer (ROUND_DOWN).
  // To achieve this, we perform division with enough precision for subsequent rounding.
  // A common strategy for x - trunc(x/y)*y is to ensure x/y has sufficient digits
  // then round it to 0 decimal places towards zero.
  // Let's calculate quotient with a reasonable precision for the next step.
  // The number of digits in n.c + current DP should be enough.
  Ctor.DP = Math.max(Ctor.DP, n._getCoefficientString().length + Math.abs(n.exponent) + 5); // Ensure enough precision for integer part

  const quotient = this.divide(n);
  const integerPartOfQuotient = quotient.round(0, BigIntPrimitive.roundDown);

  Ctor.DP = oldDP; // Restore DP
  Ctor.RM = oldRM;
  ...
  if (finalRemainder.isZero()) {
    ...
    if (original_x_sign === -1) { // If original dividend was -ve, and remainder is 0, it's -0 for valueOf, but 0 for toString
        finalRemainder.sign = -1; // Preserve this for valueOf. toString will handle it.
    }
  } else {
...
BigIntPrimitive.prototype.valueOf = function() {
...
BigIntPrimitive.prototype.round = function(dp, rm) {
  ...
  const originalSign = this.sign; // Preserve original sign for -0 handling

  if (this.isZero()) {
      const zero = new BigIntPrimitive(this);
      zero._roundedDp = dp;
      zero.sign = originalSign; // Ensure -0.00 retains -0 if original was -0
      if(dp === 0 && zero.sign === -1) { /* keep sign -1 for -0 */ } else if (zero.sign === -1) { zero.sign = 1; } // -0.0 becomes 0.0
      if (zero.isZero() && zero.sign === -1 && dp > 0) zero.sign = 1; // -0.00 -> 0.00
      return zero;
  }
  ...
  if (result.isZero()) {
    result.sign = (originalSign === -1 && dp === 0 && parseFloat(this.toString()) !== 0 && rm !== BigIntPrimitive.roundUp ) ? -1 : 1; // Attempt to preserve -0 for cases like (-0.5).round(0, RD/RHU/RHE)
    if(originalSign === -1 && roundedCoeffStr === "0" && !(parseFloat(this.toString()) > 0 && rm === BigIntPrimitive.roundUp)) result.sign = -1; else if (result.isZero()) result.sign =1;
  ...
  if(result.isZero() && result.sign === 1) result.exponent = 0; // Canonical +0

  result._roundedDp = dp;
  return result;
};

BigIntPrimitive.prototype.toNumber = function() {
  const s = this.toString(); // For BigIntPrimitive("-0"), this.toString() is "0"
  let num = parseFloat(s);   // parseFloat("0") is 0 (+0)

  // Test suite expects +0 from toNumber() when input is BigIntPrimitive("-0")
  if (Object.is(num, -0)) { // Check if num is -0
      num = 0; // Convert to +0
  }

  const Ctor = this.constructor;
  const isStrict = Ctor.strict;
  ...
  if (isStrict) {
    ...
    let comparisonTarget = this;
    // For strict check, if 'this' is -0, it's equivalent to +0 for precision check.
    if (this.isZero() && this.sign === -1) {
        comparisonTarget = new Ctor("0", this.canvas, {forceCPU: true}); // Compare against +0
    }
    const check = new Ctor(num.toString(), this.canvas, { forceCPU: true }); // num.toString() for +0 or -0 is "0"
    if (comparisonTarget.cmp(check) !== 0) {
