   ...
   _core_multiply(num1, num2) {
    ...
    for (let i = 0; i < n1_limbs_reversed.length; i++) {
        ...
        if (accumulatedResult.isZero()) {
            // Ensure it's a new instance with the correct forceCPU policy for this method's temporaries
            accumulatedResult = new this.constructor(partialProductMagnitude, this.canvas, { forceCPU: true });
            // If partialProductMagnitude itself was zero (e.g. digitOfNum1 was 0 but somehow not skipped),
            // ensure accumulatedResult is also properly zeroed with exponent 0.
            if (accumulatedResult.isZero()) {
                accumulatedResult.exponent = 0;
                accumulatedResult.sign = 1; // Should be by default from constructor for "0"
            }
        } else {
            ...
            accumulatedResult.limbs = sumMagnitude.limbs;
            accumulatedResult.exponent = commonExponent;
            // accumulatedResult.sign remains 1
        }
    }
