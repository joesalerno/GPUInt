...
class BigIntPrimitive {
  ...
  _webgl_subtract(gl, minuendLimbsLSLPadded, subtrahendLimbsLSLPadded, texWidth) {
    ...
    try {
      ...
      // Use the subtraction shader
      const vsSubtract = webglUtilsModule.createShader(gl, gl.VERTEX_SHADER, subtractVertexShaderSrc);
      const fsSubtract = webglUtilsModule.createShader(gl, gl.FRAGMENT_SHADER, subtractFragmentShaderSrc);
      program = webglUtilsModule.createProgram(gl, vsSubtract, fsSubtract);
      ...
      gl.finish();
      // Read result
      const rawResult = webglUtilsModule.readDataFromTexture(gl, fbInput, texWidth, 1, false);

      // CPU-based borrow propagation
      const actualLimbsArrLSL = new Array(texWidth).fill(0);
      let borrow = 0;
      for (let i = 0; i < texWidth; i++) {
          let diff = rawResult[i * 4] - borrow;
          if (diff < 0) {
              diff += BASE;
              borrow = 1;
          } else {
  ...
  _webgl_multiply_full(cache, num1, num2) { // Added cache as first parameter
    ...
    try {
      ...
      // Read partial products from GPU
      ...
      // CPU-based summation and carry propagation
      const resultLimbs = new Array(outputTexWidthCoeffSums).fill(0);
