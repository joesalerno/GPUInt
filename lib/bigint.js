...
// ... (rest of the BigIntPrimitive class methods: fromCoefficientString, _getCoefficientString, negate, abs, ..., valueOf, _staticRound, _staticRound_cpu)
// Ensure these are copied verbatim from the original file or the last known good state.
// For brevity, I'm assuming they are present and correct up to the 'round' method.
...
BigIntPrimitive.prototype.toString = function() {
  if (this.isZero() && this.sign === -1) return "0";
  if (this.isZero()) {
...
BigIntPrimitive.prototype._core_add = function(other) {
    let rL = [], cs = 0, tL = [...this.limbs].reverse(), oL = [...other.limbs].reverse();
    const maxL = Math.max(tL.length, oL.length);
    for(let i=0; i < maxL; i++) { let s = (tL[i]||0)+(oL[i]||0)+cs; rL.push(s%BASE); cs=Math.floor(s/BASE); }
    if(cs) rL.push(cs);
...
BigIntPrimitive.prototype.divideAndRemainder = function(divisor) {
  ...
  if (this.isZero()) return { quotient: new BigIntPrimitive("0", this.canvas, {forceCPU:true}), remainder: new BigIntPrimitive(this) };
...
BigIntPrimitive.prototype.divide = function(n) {
  ...
  if (this.isZero()) {
    ...
    if (this.sign === -1) zero.sign = -1 * n.sign;
    else zero.sign = 1 * n.sign;
    if (zero.isZero()) zero.sign = 1;
    return zero;
  }
...
BigIntPrimitive.prototype.remainder = function(n) {
  ...
  Ctor.DP = Math.max(Ctor.DP, n._getCoefficientString().length + Math.abs(n.exponent) + 5);
...
BigIntPrimitive.prototype._staticRound_cpu = function(coeffStr, decisionIndex, rm, isNegative) {
    ...
    if (rm === Ctor.roundDown) { /* no change */ }
        ...
        let hasFractionalPartToRoundUp = false;
        for(let k=decisionIndex; k < coeffStr.length; k++) if (coeffStr[k] !== '0') { hasFractionalPartToRoundUp = true; break; }

        if (hasFractionalPartToRoundUp && !isNegative) resultCoeffStr = incrementStringCoeff(resultCoeffStr);
    }
...
BigIntPrimitive.prototype.round = function(dp, rm) {
  ...
  if (this.isZero()) {
      ...
      zero._roundedDp = dp;
      zero.sign = originalSign;
      if(dp === 0 && zero.sign === -1) { /* keep sign -1 for -0 */ } else if (zero.sign === -1) { zero.sign = 1; }
      if (zero.isZero() && zero.sign === -1 && dp > 0) zero.sign = 1;
      return zero;
  }
  ...
  let coeffForRounding = coeffStr;
  if (decisionIndexInCoeff > originalCoeffLength) {
    coeffForRounding += '0'.repeat(decisionIndexInCoeff - originalCoeffLength);
  }

  let roundedCoeffOnlyStr = this._staticRound_cpu(coeffForRounding, decisionIndexInCoeff, rm, this.sign < 0);

  const result = new BigIntPrimitive("0", this.canvas, {forceCPU: this.forceCPU});
  if (roundedCoeffOnlyStr === "" || roundedCoeffOnlyStr === "0") {
      result.limbs = [0];
      result.exponent = 0;
      result.sign = 1;
  } else {
    result.limbs = Ctor.fromCoefficientString(roundedCoeffOnlyStr, this.canvas, {forceCPU:true}).limbs;
    result.sign = originalSign;
    if(result.isZero()) result.sign = 1;
  }
  result.exponent = -dp;
...
BigIntPrimitive.prototype.toNumber = function() {
  ...
  if (isStrict) {
    ...
    if (this.isZero() && this.sign === -1) {
        comparisonTarget = new Ctor("0", this.canvas, {forceCPU: true});
    }
    const check = new Ctor(num.toString(), this.canvas, { forceCPU: true });
