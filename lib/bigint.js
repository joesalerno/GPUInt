...
class BigIntPrimitive {
  ...
  _multiply_limb_by_bigint(limbValue, otherNumber) {
      ...
      if (!this.forceCPU && !otherNumber.forceCPU && this.canvas && typeof webglUtilsModule !== 'undefined' && limbValue !== 0 && !otherNumber.isZero() && false) { // Added '&& false'
  ...
  toFixed(dpUndefined, rmUndefined) {
    ...
    if (dp === undefined) {
      // big.js: if dp is omitted, return value in normal notation.
      // This means not using scientific notation, regardless of current NE/PE.
      // We achieve this by temporarily overriding NE/PE for the toString call
      // on a clean copy of the number that has no _roundedDp.
      const tempInstance = new BigIntPrimitive(this); // Create a copy
      delete tempInstance._roundedDp; // Ensure no prior rounding state interferes

      const originalPE = BigIntPrimitive.PE;
      const originalNE = BigIntPrimitive.NE;
      let resultString;
      try {
        BigIntPrimitive.PE = 10000000; // A very large value to prevent positive sci-notation
        BigIntPrimitive.NE = -10000000; // A very small value to prevent negative sci-notation
        resultString = tempInstance.toString();
      } finally {
        BigIntPrimitive.PE = originalPE; // Restore static PE
        BigIntPrimitive.NE = originalNE; // Restore static NE
      }
      return resultString;
    }
