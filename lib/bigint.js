...
class BigIntPrimitive {
  ...
  toString() {
    ...
    if (useSciNotation) {
        ...
        // AND _roundedDp is not set (which means we are not trying to preserve specific precision from toExponential)
        if (s.includes('.') && typeof this._roundedDp !== 'number') {
           s = s.replace(/\.?0+$/, '');
        }
        ...
        if (this.sign !== otherBigInt.sign) {
          ...
          try {
            ...
            if (gl && this._webglTempData) { // Check gl as well
                ...
                // Other resources created inside try should be locally scoped or cleaned by earlier specific cleanup
            }
  ...
  prec(sd, rm) {
    ...
    if (this.isZero()) {
      // For 0, prec(N) is 0. big.js seems to return positive 0.
      // Let's ensure the result is a new instance with positive sign.
      const zeroResult = new Ctor("0", this.canvas, { forceCPU: this.forceCPU });
      // if sd is specified, big.js seems to format 0 to 0e+0 if sd=1, or 0.0e+0 if sd=2 etc for toExponential style
      // but prec itself returns a Big number.
      // For our purpose, returning a new BigIntPrimitive(0) is fine.
      // The tests will verify behavior against big.js string output.
      return zeroResult;
    }

    // The number of decimal places for toExponential is sd - 1.
    const dpForExponential = sd - 1;

    // Call this.toExponential(sd - 1, actualRm).
    // toExponential itself handles rounding to dpForExponential.
    const exponentialString = this.toExponential(dpForExponential, actualRm);

    // Create and return a new BigIntPrimitive from the string result of toExponential.
    return new Ctor(exponentialString, this.canvas, { forceCPU: this.forceCPU });
  }
