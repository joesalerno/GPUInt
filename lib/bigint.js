...
import divisionVertexShaderSrc from './shaders/division.vert?raw';
import divisionFragmentShaderSrc from './shaders/division.frag?raw';
...
class BigIntPrimitive {
        ...
        if (this.sign !== otherBigInt.sign) {
          ...
          try {
            ...
            if (fbStatus !== gl.FRAMEBUFFER_COMPLETE) {
              throw new Error(`WEBGL_FRAMEBUFFER_ERROR: Framebuffer incomplete for addition. Status: 0x${fbStatus.toString(16)}`);
            }
  ...
  _webgl_multiply_full(gl, num1, num2) { // Added gl as first parameter
    ...
    try {
        ...
        // Debug logging for 7*8 case
        if (isDebugCaseMultiply) {
            console.log(`[Multiply Debug 7*8] num1: ${num1.limbs[0]}, num2: ${num2.limbs[0]}`);
            console.log(`[Multiply Debug 7*8] num1.limbs (LSL): [${num1LimbsLSL.join(',')}], num2.limbs (LSL): [${num2LimbsLSL.join(',')}]`);
            console.log(`[Multiply Debug 7*8] Pass 1 Inputs: partialProductTexDim=${Math.max(lenA, lenB)}`);
        }
        ...
        console.log(`[Multiply Debug 7*8] paddedNum1LimbsLSL: [${Array.from(paddedNum1LimbsLSL).slice(0,10)}]`); console.log(`[Multiply Debug 7*8] paddedNum2LimbsLSL: [${Array.from(paddedNum2LimbsLSL).slice(0,10)}]`);
        ...
        // Check framebuffer status; throw error if not complete
        const fbStatus = gl.checkFramebufferStatus(gl.FRAMEBUFFER);
        if (fbStatus !== gl.FRAMEBUFFER_COMPLETE) {
        ...
        // Check framebuffer status; throw error if not complete
        const fbStatusSum = gl.checkFramebufferStatus(gl.FRAMEBUFFER);
        if (fbStatusSum !== gl.FRAMEBUFFER_COMPLETE) {
            throw new Error(`WEBGL_FRAMEBUFFER_ERROR: Framebuffer incomplete for coefficient sums. Status: ${webglUtilsModule.getFramebufferStatusString(gl, fbStatusSum)}`);
        }
        ...
        console.log(`[Multiply Debug 7*8] Pass 1 (PartialProducts) output (RGBA, 1x1): [${coeffSumsRawData.slice(0,4)}]`);
        ...
        const fbPingStatus = gl.checkFramebufferStatus(gl.FRAMEBUFFER); 
        if(fbPingStatus !== gl.FRAMEBUFFER_COMPLETE) {
            throw new Error(`WEBGL_FRAMEBUFFER_ERROR: Framebuffer Ping incomplete. Status: ${webglUtilsModule.getFramebufferStatusString(gl, fbPingStatus)}`);
        }
        ...
        const fbPongStatus = gl.checkFramebufferStatus(gl.FRAMEBUFFER); 
        if(fbPongStatus !== gl.FRAMEBUFFER_COMPLETE) {
            throw new Error(`WEBGL_FRAMEBUFFER_ERROR: Framebuffer Pong incomplete. Status: ${webglUtilsModule.getFramebufferStatusString(gl, fbPongStatus)}`);
        }
        ...
        // Silent fallback to CPU on any internal error without warning logs
  ...
  multiply(otherBigInt) {
    ...
    if (!self.forceCPU && !otherBigInt.forceCPU && self.canvas && typeof webglUtilsModule !== 'undefined') {
      try {
        gl = webglUtilsModule.initWebGL(self.canvas);
        if (gl) {
        ...
        // Silent fallback to CPU without logging
        absResult = self._core_multiply_cpu(selfCoeff, otherCoeff);
      }
    } else {
      absResult = self._core_multiply_cpu(selfCoeff, otherCoeff);
  ...
  _webgl_reciprocal_using_bigint_ops(B_prime_BigInt, X0_BigInt, numIterations, targetOutputDP) {
    ...
    for (let i = 0; i < numIterations; i++) {
        console.log(`[_webgl_reciprocal_using_bigint_ops Iter ${i + 1}] Starting iteration with X: ${current_X_BigInt.toString()}`);
  ...
  divide(divisorBigInt) {
    ...
    if (!this.forceCPU && !absDivisor.forceCPU && this.canvas && typeof webglUtilsModule !== 'undefined') {
        try {
            ...
            // Try GPU division first
            const quotientResult = this._webgl_divide(gl, absDividend, absDivisor);
            quotientResult.sign = quotientSign;
            if (quotientResult.isZero()) { quotientResult.sign = 1; quotientResult.exponent = 0; }
            return quotientResult;
            
        } catch (e_outer) {
            // Fall back to CPU division
            const msg = e_outer.message || 'Unknown error';
            if (msg.includes("WEBGL_CAPABILITY_ERROR") || msg.includes("WEBGL_FRAMEBUFFER_ERROR")) {
  ...
  _webgl_divide(gl, dividend, divisor) {
    // Implement Newton-Raphson division using WebGL
    // This is a simplified version that handles basic cases
    let program = null, texDividend = null, texDivisor = null, texQuotient = null, fbQuotient = null;
    let positionBuffer = null, texCoordBuffer = null;
    
    try {
        // Create textures for dividend and divisor
        const maxWidth = Math.max(dividend.limbs.length, divisor.limbs.length);
        const dividendLimbsLSL = [...dividend.limbs].reverse();
        const divisorLimbsLSL = [...divisor.limbs].reverse();
        
        const paddedDividendLimbs = new Float32Array(maxWidth);
        const paddedDivisorLimbs = new Float32Array(maxWidth);
        
        for (let i = 0; i < dividendLimbsLSL.length; i++) {
            paddedDividendLimbs[i] = dividendLimbsLSL[i];
        }
        for (let i = 0; i < divisorLimbsLSL.length; i++) {
            paddedDivisorLimbs[i] = divisorLimbsLSL[i];
        }
        
        texDividend = webglUtilsModule.createDataTexture(gl, paddedDividendLimbs, maxWidth, 1, false);
        texDivisor = webglUtilsModule.createDataTexture(gl, paddedDivisorLimbs, maxWidth, 1, false);
        texQuotient = webglUtilsModule.createDataTexture(gl, null, maxWidth, 1, true);
        
        // Create framebuffer
        fbQuotient = gl.createFramebuffer();
        gl.bindFramebuffer(gl.FRAMEBUFFER, fbQuotient);
        gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, texQuotient, 0);
        
        const fbStatus = gl.checkFramebufferStatus(gl.FRAMEBUFFER);
        if (fbStatus !== gl.FRAMEBUFFER_COMPLETE) {
            throw new Error(`WEBGL_FRAMEBUFFER_ERROR: Framebuffer incomplete for division. Status: ${webglUtilsModule.getFramebufferStatusString(gl, fbStatus)}`);
        }
        
        // Create shader program
        const vs = webglUtilsModule.createShader(gl, gl.VERTEX_SHADER, divisionVertexShaderSrc);
        const fs = webglUtilsModule.createShader(gl, gl.FRAGMENT_SHADER, divisionFragmentShaderSrc);
        program = webglUtilsModule.createProgram(gl, vs, fs);
        
        if (!program) {
            throw new Error('WEBGL_PROGRAM_ERROR: Division shader program creation failed.');
        }
        
        gl.useProgram(program);
        gl.viewport(0, 0, maxWidth, 1);
        
        // Set up uniforms
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, texDividend);
        gl.uniform1i(gl.getUniformLocation(program, "u_dividendTexture"), 0);
        
        gl.activeTexture(gl.TEXTURE1);
        gl.bindTexture(gl.TEXTURE_2D, texDivisor);
        gl.uniform1i(gl.getUniformLocation(program, "u_divisorTexture"), 1);
        
        gl.activeTexture(gl.TEXTURE2);
        gl.bindTexture(gl.TEXTURE_2D, texQuotient);
        gl.uniform1i(gl.getUniformLocation(program, "u_quotientTexture"), 2);
        
        gl.uniform1f(gl.getUniformLocation(program, "u_base"), BASE);
        gl.uniform1f(gl.getUniformLocation(program, "u_texWidth"), maxWidth);
        ...
        // Perform multiple iterations for convergence
        const maxIterations = 10;
        for (let i = 0; i < maxIterations; i++) {
            gl.uniform1i(gl.getUniformLocation(program, "u_iteration"), i);
            gl.drawArrays(gl.TRIANGLES, 0, 6);
            gl.finish();
        }
        
        // Read results
        const outputData = webglUtilsModule.readDataFromTexture(gl, fbQuotient, maxWidth, 1, false);
        const quotientLimbs = [];
        
        for (let i = 0; i < maxWidth; i++) {
            const limbValue = outputData[i * 4 + 0];
            quotientLimbs.push(Math.round(limbValue));
        }
        
        // Convert back to BigInt format
        ...
        if (quotientLimbs.length > 0) {
            coeffStr = String(quotientLimbs[quotientLimbs.length - 1]);
            for (let i = quotientLimbs.length - 2; i >= 0; i--) {
                coeffStr += String(quotientLimbs[i]).padStart(BASE_LOG10, '0');
            }
        } else {
            coeffStr = "0";
        }
        ...
        const resultBigInt = BigIntPrimitive.fromCoefficientString(coeffStr, this.canvas, { forceCPU: true });
        resultBigInt.exponent = dividend.exponent - divisor.exponent;
        resultBigInt.sign = 1;
        
        return resultBigInt;
        
    } catch (e) {
        // Fall back to CPU if GPU division fails
        return dividend._decimalDivide(divisor, BigIntPrimitive.DP);
    } finally {
        // Cleanup
        if (gl) {
            if (program) gl.deleteProgram(program);
            if (texDividend) gl.deleteTexture(texDividend);
            if (texDivisor) gl.deleteTexture(texDivisor);
            if (texQuotient) gl.deleteTexture(texQuotient);
            if (fbQuotient) gl.deleteFramebuffer(fbQuotient);
            if (positionBuffer) gl.deleteBuffer(positionBuffer);
