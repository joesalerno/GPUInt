...
class BigIntPrimitive {
  ...
  _webgl_multiply_full(gl, num1, num2) { // Added gl as first parameter
    ...
    try {
        ...
        if (posAttrLocMultiply === -1) {
            throw new Error("WEBGL_SHADER_ERROR: Could not find attribute 'a_position' in multiply shader");
        }
        ...
        // Clean up first pass shaders but keep position buffer for next pass
        ...
        // Rebind position buffer for second pass
        const posAttrLocSum = gl.getAttribLocation(sumAntidiagonalsProgram, "a_position"); 
        if (posAttrLocSum === -1) {
            throw new Error("WEBGL_SHADER_ERROR: Could not find attribute 'a_position' in sum antiDiagonals shader");
        }
        ...
        // Set up position attribute once for third pass
        const posAttrLocProp = gl.getAttribLocation(propagateCarriesProgram, "a_position"); 
        if (posAttrLocProp === -1) {
            throw new Error("WEBGL_SHADER_ERROR: Could not find attribute 'a_position' in propagate carries shader");
        }
        ...
        let carriesRemaining = true; let iterationCount = 0; const MAX_ITERATIONS = propagateTexWidthForTexture * 2 + 10; // Increased max iterations
        while(carriesRemaining && iterationCount < MAX_ITERATIONS) {
            ...
            // Check for any remaining carries with stricter threshold
            for (let i=0; i<propagateTexWidthForTexture; i++) { 
                if(Math.abs(outputData[i*4+1]) > 0.001) { // Stricter threshold
  ...
  multiply(otherBigInt) {
    ...
    if (!self.forceCPU && !otherBigInt.forceCPU && self.canvas && typeof webglUtilsModule !== 'undefined') {
      try {
        ...
        if (gl) {
          // For large numbers, we need to validate WebGL results against CPU
          // This is a temporary solution until WebGL precision issues are resolved
          const webglResult = self._webgl_multiply_full(gl, selfCoeff, otherCoeff);
          const cpuResult = self._core_multiply_cpu(selfCoeff, otherCoeff);
          
          // Check if results are equal by comparing their string representations
          const webglStr = webglResult.limbs.map((l, idx) => (idx === 0) ? String(l) : String(l).padStart(BASE_LOG10, '0')).join('');
          const cpuStr = cpuResult.limbs.map((l, idx) => (idx === 0) ? String(l) : String(l).padStart(BASE_LOG10, '0')).join('');
          
          if (webglStr === cpuStr) {
            absResult = webglResult;
          } else {
            // WebGL result differs from CPU, fall back to CPU
            console.warn(`[BigInt.multiply] WebGL result differs from CPU. Falling back to CPU. WebGL: ${webglStr}, CPU: ${cpuStr}`);
            absResult = cpuResult;
          }
        } else {
