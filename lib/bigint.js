...
class BigIntPrimitive {
  ...
  prec(sd, rm) {
    ...
    // Jules: New logic to set _roundedDp for prec formatting
    if (!resultInstance.isZero()) {
      let resCoeffStr = resultInstance.limbs.map((l, i) => (i === 0 ? String(l) : String(l).padStart(Ctor.BASE_LOG10, 0))).join();
      let resCurrentExponent = resultInstance.exponent;
      let resNumIntegerDigits = resCoeffStr.length + resCurrentExponent;
      if (resCoeffStr === 0) resNumIntegerDigits = 1; // Handle if coeff becomes 0 after rounding
      else if (resNumIntegerDigits <= 0) resNumIntegerDigits = 1; // e.g. for 0.00123, numInt is 1 (the leading 0)

      const resSciExp = (resCoeffStr === 0) ? 0 : (resCoeffStr.length - 1) + resCurrentExponent;
      const useExponentialFormat = resSciExp <= Ctor.NE || resSciExp >= sd;

      if (!useExponentialFormat) {
        let dpForFixedOutput = sd - resNumIntegerDigits;
        if (dpForFixedOutput >= 0) {
          resultInstance._roundedDp = dpForFixedOutput;
        }
      }
    }
