...
class BigIntPrimitive {
  ...
  multiply(otherBigInt) {
    const self = this; // Explicitly capture 'this'
    ...
    const absThis = self.abs(); // Use self
    const absOther = otherBigInt.abs();

    // _core_multiply is called on 'self' (the original instance, which has the canvas),
    // and passed the absolute values.
    let result = self._core_multiply(absThis, absOther); // Use self

    // Ensure result has the correct canvas. abs() and _core_multiply should preserve it.
    // result.canvas = self.canvas; // Explicitly re-assign if there's doubt
    ...
    // Ensure canvas is from the original 'self' instance, in case result is a new obj from mock etc.
    // Though _core_multiply should handle this. This is a safeguard.
    if (result.canvas !== self.canvas) {
