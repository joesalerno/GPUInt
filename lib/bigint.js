...
class BigIntPrimitive {
  ...
  static strict = false; // big.js API: Big.strict
  static NE = -7;
  static PE = 21;
  ...
  static roundDown = 0; // big.js API
  static roundHalfUp = 1; // big.js API
  static roundHalfEven = 2; // big.js API
  static roundUp = 3; // big.js API

  constructor(value, canvas, options = {}) {
    // Check if called as a function to create a new constructor (Big.js behavior)
    if (!(this instanceof BigIntPrimitive) && value === undefined && canvas === undefined && options === undefined) {
      const CurrentBigIntPrimitive = BigIntPrimitive; // Reference to the class being called

      function NewBig(v, c, o) {
        // When NewBig is called with 'new', 'this' is the new instance.
        // We must call the original BigIntPrimitive constructor on this new instance.
        if (!(this instanceof NewBig)) {
            return new NewBig(v,c,o);
        }
        // Reflect.construct allows calling a constructor (CurrentBigIntPrimitive)
        // with a specific `this` target for the new instance (which is `this` from NewBig)
        // and a specific prototype for the new instance (`NewBig.prototype`).
        // However, simpler is to just ensure the instance methods are on NewBig.prototype
        // and call the original constructor which will set up the instance fields.
        CurrentBigIntPrimitive.call(this, v, c, o);
        // Ensure the constructor property is NewBig for instances created by `new NewBig()`
        // This is implicitly handled if NewBig.prototype.constructor = NewBig
      }

      // Set up prototype chain: NewBig instances should inherit from BigIntPrimitive.prototype
      NewBig.prototype = Object.create(CurrentBigIntPrimitive.prototype);
      // Set constructor property on NewBig.prototype to NewBig itself
      NewBig.prototype.constructor = NewBig;

      // Copy static configurable properties from the original BigIntPrimitive class
      NewBig.DP = CurrentBigIntPrimitive.DP;
      NewBig.RM = CurrentBigIntPrimitive.RM;
      NewBig.NE = CurrentBigIntPrimitive.NE;
      NewBig.PE = CurrentBigIntPrimitive.PE;
      NewBig.strict = CurrentBigIntPrimitive.strict;

      // Also copy the static rounding mode constants
      NewBig.roundDown = CurrentBigIntPrimitive.roundDown;
      NewBig.roundHalfUp = CurrentBigIntPrimitive.roundHalfUp;
      NewBig.roundHalfEven = CurrentBigIntPrimitive.roundHalfEven;
      NewBig.roundUp = CurrentBigIntPrimitive.roundUp;

      // Copy any other static methods/properties if necessary (e.g. _shaderProgramsCache if it were meant to be per-constructor)
      // For now, _shaderProgramsCache can remain shared via the BigIntPrimitive class object.
      // If truly independent caches were needed, NewBig would need its own _shaderProgramsCache.
      NewBig._shaderProgramsCache = {}; // Give NewBig its own cache
    ...
    let stringValue = '';
    let initialSign = 1; // To track sign before normalization for zero
    ...
    if (typeof value === 'number') {
      ...
      // Check for "-0" or "-0.0..." before stripping sign
      if (/^-(0+(\.0*)?|\.0+)$/.test(stringValue)) {
        initialSign = -1;
      }
    } else {
    ...
    if (stringValue.startsWith('-')) {
      this.sign = -1; // This might be initialSign or actual sign for non-zero
      stringValue = stringValue.substring(1);
    } else if (stringValue.startsWith('+')) {
      this.sign = 1;
      stringValue = stringValue.substring(1);
    } else {
      // If no explicit sign, and initialSign was -1 (from -0 literal), keep it.
      this.sign = (initialSign === -1 && /^0+(\.0*)?$/.test(stringValue)) ? -1 : 1;
    }
    // Ensure this.sign is correctly set if initialSign was determined from -0 numeric or string
    if (initialSign === -1 && /^0+(\.0*)?$/.test(stringValue)) {
        this.sign = -1;
    }
    ...
    if (this.limbs.length === 1 && this.limbs[0] === 0) { // Is zero
      this.exponent = 0;
      // Preserve sign if it was explicitly -1 for a zero value
      if (this.sign !== -1) {
        this.sign = 1;
      }
    }
  ...
  toString() {
    ...
    const Ctor = this.constructor; // Use this.constructor for static properties
    const useSciNotation = (typeof this._roundedDp !== 'number') &&
                           (decimalPointActualPosition <= Ctor.NE || decimalPointActualPosition > Ctor.PE);
  ...
  toNumber() {
    ...
    if (this.constructor.strict) { // Changed from BigIntPrimitive.strict
      // Check for lossy conversion.
      // A simple check: if the string representation changes after parseFloat and back to string.
      // This covers most cases but might need refinement for edge cases like "1.0" vs "1".
      // A more robust check for large numbers involves checking digit count.
      const parsed = parseFloat(str);

      // Handle zeros first: they are precise. Object.is distinguishes +0 and -0.
      if (parsed === 0 || Object.is(parsed, -0)) {
        const tempForZeroCheck = new (this.constructor)(str); // Use current constructor for parsing
        if (tempForZeroCheck.isZero()) { // Check if original string represents a true zero
          return parsed;
        }
      }

      // Primary check for strict precision: convert original string to Number, then back to string.
      // Then compare with a normalized version of the original string.
      const numberRepresentationToString = Number(str).toString();
      let normalizedOriginalString = str.replace(/^\+/, ''); // Remove leading '+'

      // Case 1: Direct match (e.g., "123", "1.23", "1e5")
      if (numberRepresentationToString === normalizedOriginalString) {
        // This is usually precise.
        // However, for very large integers, Number().toString() might match but precision is lost.
        // e.g. Number("12345678901234567890").toString() is "12345678901234567000" (or similar, platform dependent)
        // The test for MAX_SAFE_INTEGER + 100 already covers this type of failure.
        // If `Number(str).toString()` itself shows a change from `normalizedOriginalString` for large integers,
        // that's caught by the `else` block below.
      }
      // Case 2: Formatting differences for integers (e.g., str="1.0", Number(str).toString()="1")
      else if (normalizedOriginalString.includes('.') && !normalizedOriginalString.includes('e') &&
               !numberRepresentationToString.includes('.') && // Number is integer
               Number(normalizedOriginalString) === parsed && // Numerically equal
               normalizedOriginalString.replace(/\.0+$/, '') === numberRepresentationToString) { // String match after stripping .0
        // This is precise (e.g. "1.0" is same as 1)
      }
      // Case 3: Other mismatches usually mean imprecision or significant formatting change not allowed.
      else {
          // This is the path that "1.0000000000000001" might take if Number("1.0000000000000001").toString() is "1"
          // but parseFloat("1.0000000000000001").toString() is "1.0000000000000001".
          // We need to ensure that if `parsed` (from parseFloat) *does* retain the full precision
          // and its string form matches the original, it's fine.
          if (parsed.toString() !== normalizedOriginalString && parsed.toString() !== str /* check original too */) {
            // If even parseFloat().toString() doesn't match, it's likely an issue.
            // One final check for "1.0" vs "1" type normalization if `parsed.toString()` was different from `Number(str).toString()`
            let normParsed = parsed.toString();
            if (normParsed.includes('.') && !normParsed.includes('e')) normParsed = normParsed.replace(/\.?0+$/, '');
            let normOrig = normalizedOriginalString;
            if (normOrig.includes('.') && !normOrig.includes('e')) normOrig = normOrig.replace(/\.?0+$/, '');
            if (normOrig.startsWith('.')) normOrig = '0' + normOrig;
            if (normParsed.startsWith('.')) normParsed = '0' + normParsed;


            if (normParsed !== normOrig) {
                 throw new Error("[big.js] Imprecise conversion");
            }
          }
      }
       // An additional check specifically for very long numbers without exponents.
       // Number.MAX_SAFE_INTEGER is 9007199254740991 (16 digits).
       // Numbers with more than 15-17 significant digits often lose precision.
       const digitsOnly = str.replace(/[^0-9]/g, '');
       if (!str.includes('e') && digitsOnly.length > 15) {
           if (Number(str) !== parsed) { // If Number() and parseFloat() differ for long numbers
               throw new Error("[big.js] Imprecise conversion");
           }
           // If Number(str) and parseFloat(str) are the same, but different from original string value
           if (Number(str).toString() !== str.replace(/^\+/,'')) {
                // Allow if it's just "1.0" vs "1"
                if (!(str.includes('.') && Number(str) === parseFloat(str.replace(/\.0+$/, '')))) {
                   // This was the path for MAX_SAFE_INTEGER+100
                   throw new Error("[big.js] Imprecise conversion");
                }
           }
       }
  ...
  isZero() {
    // A number is zero if its coefficient is zero. Exponent doesn't matter.
    // The constructor normalizes true zero to have exponent 0.
        ...
        if (gl) {
          ...
          if (!additionProgram) {
            ...
            if (vs && fs) {
              ...
              // Log shader info log if available for debugging
  ...
  subtract(otherBigInt) {
    ...
    let gpuPathSuccess = false;
    let subtractedMagnitudeResultLimbs;
    let gl;
    ...
    if (!this.forceCPU && !otherBigInt.forceCPU && this.canvas && typeof webglUtilsModule !== 'undefined') {
      ...
      if (gl) {
        const Ctor = this.constructor;
        const cacheKey = "subtractionProgram";
        let subtractionProgram = Ctor._shaderProgramsCache[cacheKey];

        if (!subtractionProgram) {
          // Assuming subtractVertexShaderSrc and subtractFragmentShaderSrc are imported correctly at the top
          ...
          if (vs && fs) {
            subtractionProgram = webglUtilsModule.createProgram(gl, vs, fs);
            if (subtractionProgram) {
              Ctor._shaderProgramsCache[cacheKey] = subtractionProgram;
            } else { gl = null; } // Program linking failed
          } else { gl = null; } // Shader compilation failed
        }

        if (subtractionProgram && gl) {
          const texWidth = Math.max(tempMinuend.limbs.length, tempSubtrahend.limbs.length);

          const tex1 = webglUtilsModule.createDataTexture(gl, new Float32Array(tempMinuend.limbs), texWidth, 1, false);
          const tex2 = webglUtilsModule.createDataTexture(gl, new Float32Array(tempSubtrahend.limbs), texWidth, 1, false);
          const outTex = webglUtilsModule.createDataTexture(gl, null, texWidth, 1, true); // RGBA for result limb and borrowOut
          ...
          if (tex1 && tex2 && outTex) {
            ...
            if (gl.checkFramebufferStatus(gl.FRAMEBUFFER) === gl.FRAMEBUFFER_COMPLETE) {
              gl.useProgram(subtractionProgram);

              gl.uniform1f(gl.getUniformLocation(subtractionProgram, "u_texWidth"), texWidth);
              gl.uniform1f(gl.getUniformLocation(subtractionProgram, "u_base"), BASE);

              gl.activeTexture(gl.TEXTURE0); gl.bindTexture(gl.TEXTURE_2D, tex1);
              gl.uniform1i(gl.getUniformLocation(subtractionProgram, "u_num1Texture"), 0);

              gl.activeTexture(gl.TEXTURE1); gl.bindTexture(gl.TEXTURE_2D, tex2);
              gl.uniform1i(gl.getUniformLocation(subtractionProgram, "u_num2Texture"), 1);

              gl.viewport(0, 0, texWidth, 1);
              const quadBuffer = webglUtilsModule.setupGpgpuQuad(gl, subtractionProgram);
              ...
              if (quadBuffer) {
                ...
                if (gpuResultRGBA) {
                  let finalLimbs = [];
                  let js_borrow_propagate = 0.0;

                  for (let i = texWidth - 1; i >= 0; i--) {
                    let digit_val_from_shader = gpuResultRGBA[i * 4 + 0];
                    let borrow_signal_from_shader = gpuResultRGBA[i * 4 + 1];

                    let current_digit = digit_val_from_shader - js_borrow_propagate;

                    js_borrow_propagate = 0.0; // Reset for this iteration, only take from shader

                    if (current_digit < 0) {
                      current_digit += BASE;
                      js_borrow_propagate = 1.0; // We performed a borrow in JS
                    }
                    // Add shader's borrow signal. If shader borrowed AND we borrowed, total borrow is 2 (which is fine, means prev limb was -1)
                    // This logic needs to be correct: the shader output `borrowOut` is local to its calculation.
                    // The JS loop handles the cascading borrow.
                    // `digit_val_from_shader` is already `limb1 - limb2` or `limb1 - limb2 + BASE`.
                    // `borrow_signal_from_shader` is 1 if `limb1 - limb2 < 0`.
                    // Let's rethink the JS borrow propagation from the shader output.
                    // Shader: resultLimb = (limb1 - limb2 < 0) ? (limb1 - limb2 + BASE) : (limb1 - limb2)
                    // Shader: borrowOut = (limb1 - limb2 < 0) ? 1.0 : 0.0
                    // JS: current_digit_final = resultLimb_from_shader_at_i - js_borrow_from_i+1
                    //     if current_digit_final < 0:
                    //         current_digit_final += BASE
                    //         js_borrow_for_i-1 = 1.0
                    //     else:
                    //         js_borrow_for_i-1 = 0.0
                    // This is the standard CPU _core_subtract loop. The shader's borrowOut is not directly summed.
                    // The shader does one stage. JS must do the rest.
                    // The shader output (resultLimb, borrowOut) means:
                    // resultLimb is the local result. borrowOut is if that local result required a borrow from its *next most significant* shader calc.
                    // This is what the CPU _core_subtract does in its loop.
                    // So, the shader is like one iteration of the _core_subtract loop.
                    // The JS part here must be like the outer loop that manages carries/borrows between shader "iterations" (pixels).
                    // This is what the ADD shader's JS loop does.

                    // Re-aligning with ADD shader's JS logic for carry:
                    // finalLimbs.unshift(sum_with_js_carry % BASE);
                    // js_carry_in = Math.floor(sum_with_js_carry / BASE) + carry_from_shader;
                    // For subtraction:
                    // current_digit is (digit_val_from_shader_at_i) - js_borrow_propagate_from_i+1
                    // if current_digit < 0: current_digit += BASE; next_js_borrow = 1
                    // else: next_js_borrow = 0
                    // next_js_borrow += borrow_signal_from_shader_at_i (this seems wrong, shader borrow is already incorporated in its resultLimb)
                    // The shader's `borrowOut` tells us if the *shader itself* had to borrow to produce `resultLimb`.
                    // Correct logic for JS borrow propagation based on shader output:
                    // `resultLimb` from shader is `(minuend[i] - subtrahend[i])` potentially with `+u_base`
                    // `borrowOut` from shader is `1.0` if `minuend[i] - subtrahend[i] < 0`, else `0.0`

                    // Let's use a clearer loop structure, similar to CPU _core_subtract
                    // The shader provides (digit, local_borrow_generated_by_shader_for_this_digit)
                    // We then apply the propagated borrow from the *previous* (less significant) digit.
                  }

                  // Simpler borrow propagation, assuming shader output is (val, borrow_it_generated)
                  // Iterate from LSB (texWidth-1) to MSB (0)
                  let propagated_borrow = 0.0;
                  for (let i = texWidth - 1; i >= 0; i--) {
                      let limb_val = gpuResultRGBA[i * 4 + 0]; // resultLimb from shader for this position
                      let shader_generated_borrow_for_next_pos = gpuResultRGBA[i * 4 + 1]; // borrowOut from shader

                      let current_limb_val = limb_val - propagated_borrow;
                      propagated_borrow = 0.0; // reset for current iteration's own calculation

                      if (current_limb_val < 0) {
                          current_limb_val += BASE;
                          propagated_borrow = 1.0; // JS performs a borrow
                      }
                      // The shader_generated_borrow_for_next_pos is what the *next* (more significant)
                      // limb to the left should account for from the shader's perspective.
                      // Our JS propagated_borrow is what this current limb needs from its left.
                      finalLimbs.unshift(current_limb_val);
                      propagated_borrow += shader_generated_borrow_for_next_pos; // This is what the shader decided the next stage needs
                  }


                  if (propagated_borrow > 0) {
                    console.error("GPU subtraction resulted in a final borrow. GPU path failed.");
                  } else {
                    while (finalLimbs.length > 1 && finalLimbs[0] === 0) { finalLimbs.shift(); }
                    if (finalLimbs.length === 0) finalLimbs = [0];
                    subtractedMagnitudeResultLimbs = finalLimbs;
                    gpuPathSuccess = true;
                  }
                }
                gl.deleteBuffer(quadBuffer);
              }
            } else { console.error("WebGL FBO not complete for subtraction."); }
            gl.bindFramebuffer(gl.FRAMEBUFFER, null);
            gl.deleteFramebuffer(fbo);
            gl.deleteTexture(tex1); gl.deleteTexture(tex2); gl.deleteTexture(outTex);
          } else { console.error("Failed to create WebGL textures for subtraction."); }
        }
      }
    } // End of GPU path

    if (gpuPathSuccess) {
      result.limbs = subtractedMagnitudeResultLimbs;
    } else {
      // CPU Fallback
  ...
  _multiply_limb_by_bigint(limbValue, otherNumber) {
      // GPU Path Attempt
      let gpuPathSuccess = false;
      let finalResultLimbsFromGPU;

      if (!this.forceCPU && !otherNumber.forceCPU && this.canvas && typeof webglUtilsModule !== 'undefined' && limbValue !== 0 && !otherNumber.isZero()) {
        let gl = webglUtilsModule.initWebGL(this.canvas);
        if (gl) {
          const Ctor = this.constructor;
          const cacheKey = "multiplyLimbProgram";
          let multiplyLimbProgram = Ctor._shaderProgramsCache[cacheKey];

          if (!multiplyLimbProgram) {
            ...
            if (vs && fs) {
              multiplyLimbProgram = webglUtilsModule.createProgram(gl, vs, fs);
              if (multiplyLimbProgram) {
                Ctor._shaderProgramsCache[cacheKey] = multiplyLimbProgram;
              } else { gl = null; }
            } else { gl = null; }
          }

          if (multiplyLimbProgram && gl) {
            const texWidth = otherNumber.limbs.length;
            // texWidth check for 0 is implicitly handled by !otherNumber.isZero() for well-formed numbers

            const texOtherNum = webglUtilsModule.createDataTexture(gl, new Float32Array(otherNumber.limbs), texWidth, 1, false);
            const outTex = webglUtilsModule.createDataTexture(gl, null, texWidth, 1, true); // RGBA for prod_limb, prod_carry

            if (texOtherNum && outTex) {
              const fbo = gl.createFramebuffer();
              gl.bindFramebuffer(gl.FRAMEBUFFER, fbo);
              gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, outTex, 0);

              if (gl.checkFramebufferStatus(gl.FRAMEBUFFER) === gl.FRAMEBUFFER_COMPLETE) {
                gl.useProgram(multiplyLimbProgram);

                gl.uniform1f(gl.getUniformLocation(multiplyLimbProgram, "u_texWidth"), texWidth);
                gl.uniform1f(gl.getUniformLocation(multiplyLimbProgram, "u_base"), BASE);
                gl.uniform1f(gl.getUniformLocation(multiplyLimbProgram, "u_limbVal"), limbValue);

                gl.activeTexture(gl.TEXTURE0); gl.bindTexture(gl.TEXTURE_2D, texOtherNum);
                gl.uniform1i(gl.getUniformLocation(multiplyLimbProgram, "u_otherNumTexture"), 0);

                gl.viewport(0, 0, texWidth, 1);
                const quadBuffer = webglUtilsModule.setupGpgpuQuad(gl, multiplyLimbProgram);
                ...
                if (quadBuffer) {
                  ...
                  if (gpuResultRGBA) {
                    let resultLimbsReversed = []; // Build LSB to MSB
                    let js_carry_propagate = 0.0;

                    // otherNumber.limbs is MSB-first. gpuResultRGBA is also MSB-first (tex coords map 0->MSB).
                    // We process from LSB of otherNumber for correct carry propagation.
                    // Shader output index corresponds to otherNumber.limbs index.
                    for (let i = texWidth - 1; i >= 0; i--) { // Iterate from LSB of otherNumber
                      let shader_local_prod_digit = gpuResultRGBA[i * 4 + 0];
                      let shader_local_prod_carry = gpuResultRGBA[i * 4 + 1];

                      let current_sum = shader_local_prod_digit + js_carry_propagate;
                      resultLimbsReversed.push(current_sum % BASE);
                      js_carry_propagate = shader_local_prod_carry + Math.floor(current_sum / BASE);
                    }

                    while (js_carry_propagate > 0) {
                      resultLimbsReversed.push(js_carry_propagate % BASE);
                      js_carry_propagate = Math.floor(js_carry_propagate / BASE);
                    }

                    finalResultLimbsFromGPU = resultLimbsReversed.reverse(); // Convert back to MSB-first
                    gpuPathSuccess = true;
                  }
                  gl.deleteBuffer(quadBuffer);
                }
              } else { console.error("WebGL FBO not complete for multiplyLimb."); }
              gl.bindFramebuffer(gl.FRAMEBUFFER, null);
              gl.deleteFramebuffer(fbo);
              gl.deleteTexture(texOtherNum); gl.deleteTexture(outTex);
            } else { console.error("Failed to create WebGL textures for multiplyLimb."); }
          }
        }
      } // End of GPU Path Attempt

      if (gpuPathSuccess) {
        if (finalResultLimbsFromGPU.length === 0) finalResultLimbsFromGPU = [0];
        while (finalResultLimbsFromGPU.length > 1 && finalResultLimbsFromGPU[0] === 0) { finalResultLimbsFromGPU.shift(); }

        const resultNumGPU = new BigIntPrimitive("0", this.canvas, { forceCPU: this.forceCPU });
        resultNumGPU.limbs = finalResultLimbsFromGPU;
        resultNumGPU.sign = 1;
        resultNumGPU.exponent = otherNumber.exponent;
        if (resultNumGPU.isZero()) { resultNumGPU.sign = 1; resultNumGPU.exponent = 0; }
        return resultNumGPU;
      } else {
  ...
  pow(exp) {
    ...
    // big.js: The exponent must be an integer from -1e+6 to 1e+6.
    if (exp < -1000000 || exp > 1000000) {
      throw new RangeError("[big.js] Exponent out of range: " + exp);
    }

    const currentOptions = { forceCPU: this.forceCPU };

    if (exp < 0) {
      // TODO: Ensure division for pow(-n) correctly uses DP/RM like big.js
      // The current divide method uses _longDivide which relies on BigInt,
      // which might not respect DP/RM for fractional results in the way big.js expects.
      // For now, this will produce a result, but precision might differ from big.js.
      const one = new BigIntPrimitive("1", this.canvas, currentOptions);
      const positiveExpResult = this.pow(-exp);
      if (positiveExpResult.isZero()) { // 1 / 0
          throw new Error("[big.js] Division by zero");
      }
      return one.divide(positiveExpResult);
    }
  ...
  static _staticRound(inputLimbsMsbFirst, inputExponent, inputSign, dpUndefined, rmUndefined, Ctor) {
    // Ctor is passed to access Ctor.RM, Ctor.NE, Ctor.PE if needed by logic here
    // or by constructor calls within.
    const actualCtor = Ctor || BigIntPrimitive; // Fallback if Ctor not passed (e.g. direct static call)

    const dp = dpUndefined === undefined ? 0 : dpUndefined;
    const rm = rmUndefined === undefined ? actualCtor.RM : rmUndefined;

    const tempNumForStr = new actualCtor("0"); // Use actualCtor
    ...
    // Temporarily override NE/PE of the specific constructor for toString behavior
    const originalPETemp = actualCtor.PE;
    const originalNETemp = actualCtor.NE;
    actualCtor.PE = 100000; // Use a large range to force non-scientific notation for intermediate string
    actualCtor.NE = -100000;
    let s = tempNumForStr.toString();
    actualCtor.PE = originalPETemp; // Restore
    actualCtor.NE = originalNETemp; // Restore
    ...
    // Ensure 'rm' is one of the defined rounding modes of 'actualCtor'
    // This check might be redundant if rm is always sourced from actualCtor.RM or validated upstream.
    // However, big.js does this internally in its equivalent of staticRound.
    // if (rm !== 0 && rm !== 1 && rm !== 2 && rm !== 3) {
    //   throw Error('!round!'); // big.js internal error message
    // }
    ...
    // Use actualCtor to create the result number from the rounded string
    const resultNum = new actualCtor(finalRoundedStr);
  ...
  round(dp, rm) {
    const Ctor = this.constructor;
    const roundingMode = rm === undefined ? Ctor.RM : rm;
    const dpToUse = dp === undefined ? 0 : dp;

    // Pass Ctor to _staticRound so it can use the correct RM, NE, PE settings
    const roundedParts = BigIntPrimitive._staticRound(
        this.limbs, this.exponent, this.sign, dpToUse, roundingMode, Ctor
    );

    const result = new Ctor("0", this.canvas, { forceCPU: this.forceCPU });
  ...
  sqrt() {
    ...
    const S = this;
    // originalDP and originalRM are now sourced from Ctor (this.constructor)
    ...
    // Ensure internalDP uses the Ctor's DP
    const internalDP = Ctor.DP + (String(Math.abs(S.exponent)).length || 1) + S.limbs.length + 7;
    ...
    const internalDivide = (dividend, divisor, precision) => {
      ...
      const toCoefficientAndExponent = (numInstance) => {
        ...
        // Constructor ensures limbs are not empty for non-zero numbers
        // and that coeffStr from limbs.join('') is the normalized coefficient.
      ...
      // numEffectiveExp is the exponent if numCoeffStr was an integer, scaled by precision
      let numEffectiveExp = dividendInfo.exp + precision;
      ...
      if (expDiffForBigInt >= 0) {
        // If numCoeffStr is "0", appending zeros doesn't change its value ("0" vs "000")
      ...
      if (biDen === BigInt(0)) {
        // This case should ideally be caught by divisor.isZero() earlier,
        // but if denCoeffStr was "0" and scaled by '0'.repeat, it remains "0".
        throw new Error("[big.js] Division by zero in internalDivide for sqrt (denominator is zero).");
      }

      const quotientValStr = (biNum / biDen).toString();

      // Reconstruct the BigIntPrimitive to ensure full normalization, especially for exponent.
      // The `quotientValStr` is an integer. Its notional exponent needs to be `-precision`.
      let effectiveExponentForQuotientVal = -precision;
      let stringToReconstruct;
      if (quotientValStr === "0") {
        stringToReconstruct = "0"; // Normalize 0e-X to just "0"
      } else {
        stringToReconstruct = `${quotientValStr}e${effectiveExponentForQuotientVal}`;
      }

      let quotient = new Ctor(stringToReconstruct, dividend.canvas, { forceCPU: dividend.forceCPU });

      if (dividend.sign !== divisor.sign && !quotient.isZero()) { // Use updated isZero
        quotient.sign = -1;
      } else if (quotient.isZero()) {
        quotient.sign = 1; // Ensure zero is always positive
      }
      return quotient;
    };
    ...
    const MAX_ITERATIONS = Ctor.DP + S.limbs.length + Math.abs(S.exponent) + 20; // Use Ctor.DP
    let iterations = 0;
    let previous_x_n_rounded_str = "";

    // Temporarily set DP for the Ctor for internal calculations
    const CtorDPOriginal = Ctor.DP; // Save Ctor's original DP
    Ctor.DP = internalDP;

    for (iterations = 0; iterations < MAX_ITERATIONS; iterations++) {
      if (x_n.isZero()) {
        Ctor.DP = CtorDPOriginal; // Restore DP before throwing
        throw new Error("[big.js] Division by zero during sqrt iteration: x_n became zero.");
      }

      const s_div_xn = internalDivide(S, x_n, internalDP); // internalDP is already Ctor.DP here

      const sum_terms = s_div_xn.add(x_n);
      x_n_plus_1 = internalDivide(sum_terms, two, internalDP); // internalDP is Ctor.DP

      // For convergence check, round to a slightly higher precision than originalDP
      const tempConvergenceDP = originalDP + 5; // Use originalDP of the calling instance's Ctor

      // Temporarily set Ctor.DP for this specific rounding call for convergence check
      const CtorDPForConvCheckOriginal = Ctor.DP;
      Ctor.DP = tempConvergenceDP;
      const current_x_n_plus_1_rounded_str = x_n_plus_1.round(tempConvergenceDP, originalRM).toString();
      Ctor.DP = CtorDPForConvCheckOriginal; // Restore Ctor.DP to internalDP for next iteration's division
    ...
    Ctor.DP = CtorDPOriginal; // Restore Ctor's DP to its original value
    // RM was not changed on Ctor, only used from originalRM (which is Ctor.RM of the instance)

    const finalResult = x_n_plus_1.round(originalDP, originalRM); // Use originalDP and originalRM

    let isInteger = finalResult.exponent >= 0;
    if (isInteger) {
        // Use Ctor.roundDown for this check
        const tempRoundedToZero = finalResult.round(0, Ctor.roundDown);
        if (finalResult.eq(tempRoundedToZero)) {
            if (originalDP > 0 || finalResult._roundedDp !== undefined) {
  ...
  toExponential(dpUndefined, rmUndefined) {
    ...
    // Create coeff using Ctor to ensure it uses Ctor's settings if its methods are called later
    const coeff = new Ctor(this, this.canvas, { forceCPU: this.forceCPU });
    coeff.sign = 1;
    coeff.exponent = -(this.limbs.length - 1);

    let roundedCoeff;
    if (dp === undefined) {
        // Temporarily override PE/NE on Ctor for this block
        const tempPE = Ctor.PE; const tempNE = Ctor.NE;
        Ctor.PE = 10000; Ctor.NE = -10000;
        let coeffStrTemp = coeff.toString();
        Ctor.PE = tempPE; Ctor.NE = tempNE; // Restore
        if(coeffStrTemp.includes('.')) coeffStrTemp = coeffStrTemp.replace(/\.?0+$/, '');
        roundedCoeff = new Ctor(coeffStrTemp); // Use Ctor
        ...
        } else if (parseFloat(roundedCoeff.toString()) >= 10.0) { // This check might be problematic due to parseFloat precision
            sciExp++;
            roundedCoeff = new Ctor(roundedCoeff.toString() + "e-1"); // Use Ctor
        }
    } else {
        roundedCoeff = coeff.round(dp, actualRm); // round will use this.constructor.RM etc.
        // Create tempCoeffValCheck using Ctor
        let tempCoeffValCheck = new Ctor(roundedCoeff, this.canvas);
        tempCoeffValCheck.exponent = 0;

        let checkStr = tempCoeffValCheck.limbs.join('');
        // Check if roundedCoeff is >= 10.
        // parseFloat might not be precise enough for all cases.
        // A more robust check: if its integer part has more digits than it should for a single digit before decimal.
        // Or, if roundedCoeff.cmp(new Ctor(10)) >= 0
        if (checkStr.length > 1 && tempCoeffValCheck.limbs[0] !== 0 && roundedCoeff.gte(new Ctor("10")) ) {
             sciExp++;
             let tempDivBy10 = new Ctor(roundedCoeff, this.canvas); // Use Ctor
             tempDivBy10.exponent--; // Effectively divide by 10
             roundedCoeff = tempDivBy10.round(dp, actualRm); // Re-round after division
        }
    }
  ...
  toFixed(dpUndefined, rmUndefined) {
    ...
    if (dp === undefined) {
      const tempInstance = new Ctor(this);
      delete tempInstance._roundedDp;

      const originalPE = Ctor.PE;
      const originalNE = Ctor.NE;
      let resultString;
      try {
        Ctor.PE = 10000000;
        Ctor.NE = -10000000;
        resultString = tempInstance.toString();
      } finally {
        Ctor.PE = originalPE;
        Ctor.NE = originalNE;
      }
    ...
    // If dp is defined, round this number to dp decimal places
    const roundedNum = this.round(dp, actualRm);
    // The round() method returns a new BigIntPrimitive with _roundedDp set to dp.
    // The toString() method of that roundedNum will then use its _roundedDp
    // to format the string, including padding with zeros if necessary.
