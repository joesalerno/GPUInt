...
class BigIntPrimitive {
  ...
  toNumber() {
    ...
    const primitiveNumber = Number(originalString); // Standard JS conversion
    ...
    if (this.constructor.strict) {
        if (isFinite(primitiveNumber)) {
            ...
            // Compare the string representation of the BigInt created from the number
            // with the original BigInt's string representation.
            if (tempBig.toString() !== originalString) {
                 throw new Error("[big.js] Imprecise conversion");
            }
        } else { // primitiveNumber is Infinity or -Infinity or NaN
            const OldStrict = this.constructor.strict;
            try {
                this.constructor.strict = false;
                // Try to create a BigInt from the primitiveNumber.
                // Our constructor will throw if primitiveNumber is NaN, Infinity, or -Infinity.
                new BigIntPrimitive(primitiveNumber, this.canvas, { forceCPU: this.forceCPU });
                // If it *didn't* throw (e.g. if constructor was modified to handle Infinity)
                // then we would proceed to check if originalString also represented that Infinity.
                // But with current constructor, this path is primarily for NaN.
                this.constructor.strict = OldStrict;

                // If primitiveNumber was NaN, and originalString was not "NaN" (or some equivalent
                // that our constructor parses as NaN, which it doesn't directly), then it's imprecise.
                // However, if originalString was "NaN", our main constructor already throws.
                // This path is tricky. Let's assume if Number(originalString) is NaN,
                // and originalString itself isn't something our constructor *successfully* parses to NaN,
                // then it's effectively an imprecise conversion for strict mode.
                // The test "should throw for "NaN" string if intermediate BigInt creation fails" covers this.
                // For Infinity/-Infinity, big.js allows them. The fact that our constructor throws for
                // new BigIntPrimitive(Infinity) is what the "intermediate creation failed" test catches.
                // If primitiveNumber is non-finite, and our constructor *would* fail to create tempBig:
                //   - If originalString was "NaN", main constructor throws.
                //   - If originalString was "1e1000", primitiveNumber is Infinity. `new BigIntPrimitive(Infinity)` throws.
                //     This is the path hit by the failing Infinity tests.
                //     In this specific case (primitiveNumber is non-finite, constructor for tempBig throws),
                //     big.js does *not* consider it an "Imprecise conversion". It returns Infinity.
                //     So, the "intermediate creation failed" error should not be thrown if !isFinite(primitiveNumber).

            } catch (e) { // Error during new BigIntPrimitive(primitiveNumber, ...)
                this.constructor.strict = OldStrict;
                // If primitiveNumber is NaN, this catch block is hit.
                // If primitiveNumber is Infinity/-Infinity, this catch block is hit.
                // For strict mode, if Number(originalString) results in NaN,
                // and originalString was not something our main constructor *already* rejected or parsed as NaN,
                // then it's an imprecise conversion.
                if (Number.isNaN(primitiveNumber)) {
                     throw new Error("[big.js] Imprecise conversion (NaN from string)");
                }
                // If primitiveNumber is Infinity/-Infinity, big.js allows this. So, no error here.
            }
        }
    }
