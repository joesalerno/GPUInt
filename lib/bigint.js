...
class BigIntPrimitive {
  ...
  _webgl_multiply_full(num1, num2) {
    let gl;
    let program = null, texNum1 = null, texNum2 = null, fbPartialProducts = null, positionBuffer = null, texPartialProducts = null; // texCoordBuffer removed
    ...
    try {
        ...
        if (posAttrLocMultiply === -1) throw new Error("[WebGL Multiply Full - Pass 1] Attribute a_position not found.");
        gl.enableVertexAttribArray(posAttrLocMultiply);
        gl.vertexAttribPointer(posAttrLocMultiply, 2, gl.FLOAT, false, 0, 0);

        // texCoordBuffer and a_texCoord attribute setup removed as v_texCoord is derived from a_position in shader

        gl.viewport(0, 0, partialProductTexDim, partialProductTexDim);
        let preDrawErrorCheck = gl.getError(); // Check error BEFORE draw
        if (preDrawErrorCheck !== gl.NO_ERROR) console.error(`[Multiply Debug ${debugCaseName}] GL Error BEFORE Pass 1 draw: ${preDrawErrorCheck}`);
        ...
        if (posAttrLocSum === -1) throw new Error("[WebGL Multiply Full - Pass 2] Attribute a_position not found.");
        ...
        // texCoordBuffer and a_texCoord attribute setup removed
        ...
        for (let k_val = 0; k_val < outputTexWidthCoeffSums; k_val++) {
            ...
            preDrawErrorCheck = gl.getError(); // Check error BEFORE draw
            if (preDrawErrorCheck !== gl.NO_ERROR) console.error(`[Multiply Debug ${debugCaseName}] GL Error BEFORE Pass 2 draw for k=${k_val}: ${preDrawErrorCheck}`);
        ...
        // Read data for Pass 3 *before* deleting Pass 2 program
        const coeffSumsRawData = webglUtilsModule.readDataFromTexture(gl, fbCoeffSums, outputTexWidthCoeffSums, 1, false);
        if (isDebugCaseMultiply) {
            // Log the data that was just read for coeffSumsRawData, satisfying the test's expectation for Pass 2 output log
            console.log(`[Multiply Debug ${debugCaseName}] Pass 2 (SumAntiDiagonals) output (RGBA, ${outputTexWidthCoeffSums}x1): [${Array.from(coeffSumsRawData).slice(0, outputTexWidthCoeffSums * 4).join(',')}]`);
            // This next log is now slightly redundant but harmless, confirms what's used for Pass 3.
            console.log(`[Multiply Debug ${debugCaseName}] coeffSumsRawData (for Pass 3 input) read as: [${Array.from(coeffSumsRawData).slice(0, outputTexWidthCoeffSums * 4).join(',')}]`);
        }
        ...
        // Input: texCoeffSums (1D texture from sum_antidiagonals) - NOW USING coeffSumsRawData
        // Output: final result limbs (1D texture)
        // This pass iteratively resolves carries.
        // The initial input for propagation will be from coeffSumsRawData.
        ...
        let propagateTexWidthForTexture = Math.max(4, propagateTexWidthActual);
        propagateTexWidthForTexture = propagateTexWidthActual;

        // Prepare initial data for propagation: take coeffSumsRawData and put it into a texture of width propagateTexWidthForTexture
        const initialPropagationDataR = new Float32Array(propagateTexWidthForTexture * 4); // RGBA
        for (let k = 0; k < outputTexWidthCoeffSums; k++) {
            initialPropagationDataR[k * 4 + 0] = coeffSumsRawData[k*4+0];
            initialPropagationDataR[k * 4 + 1] = coeffSumsRawData[k*4+1];
        ...
        if (uInputTexLocProp === null || uInputTexLocProp === -1) throw new Error("[WebGL Multiply Full - Pass 3] Uniform u_inputTexture not found.");
        if (uBaseLocProp === null || uBaseLocProp === -1) throw new Error("[WebGL Multiply Full - Pass 3] Uniform u_base not found.");
        if (uTexWidthLocProp === null || uTexWidthLocProp === -1) throw new Error("[WebGL Multiply Full - Pass 3] Uniform u_textureWidth not found.");
        ...
        while (carriesRemaining && iterationCount < MAX_ITERATIONS) {
            ...
            if (posAttrLocProp === -1) throw new Error("[WebGL Multiply Full - Pass 3] Attribute a_position not found.");
            ...
            preDrawErrorCheck = gl.getError(); // Check error BEFORE draw
            if (preDrawErrorCheck !== gl.NO_ERROR) console.error(`[Multiply Debug ${debugCaseName}] GL Error BEFORE Pass 3 iter ${iterationCount} draw: ${preDrawErrorCheck}`);
            gl.drawArrays(gl.TRIANGLES, 0, 6);
        ...
        if (isDebugCaseMultiply) {
            console.log(`[Multiply Debug ${debugCaseName}] PRE-READ from finalResultFramebuffer (associated with texture ${finalResultTexture === texPropagatePing ? 'texPropagatePing' : 'texPropagatePong'}). Width: ${propagateTexWidthForTexture}`);
        }
        ...
        if (isDebugCaseMultiply) {
            // Log the raw RGBA data
            console.log(`[Multiply Debug ${debugCaseName}] RAW finalResultRGBA (length ${finalResultRGBA.length}): [${Array.from(finalResultRGBA).join(',')}]`);
            // Log the R,G pairs interpretation
            const logData = []; for(let i=0; i<propagateTexWidthForTexture; ++i) logData.push(finalResultRGBA[i*4], finalResultRGBA[i*4+1]);
            console.log(`[Multiply Debug ${debugCaseName}] Pass 3 Final Output (R,G pairs from texture_width ${propagateTexWidthForTexture}): [${logData.join(',')}]`);
        }
        ...
        for (let i = 0; i < propagateTexWidthActual; i++) { // Read up to actual needed limbs
            const r_val = finalResultRGBA[i * 4 + 0]; // .r channel
            const g_val = finalResultRGBA[i * 4 + 1]; // .g channel (carry)
            if (isDebugCaseMultiply) {
                console.log(`[Multiply Debug ${debugCaseName}] finalLimbsLSL construction: i=${i}, raw_r_val=${r_val}, raw_g_val=${g_val}, rounded_r_val=${Math.round(r_val)}`);
            }
            // Ensure that carry (g_val) is zero before accepting r_val. If carry is non-zero, something is wrong.
            if (Math.abs(g_val) > 0.0001 && isDebugCaseMultiply) { // Check for non-zero carry
                 console.warn(`[Multiply Debug ${debugCaseName}] Non-zero carry ${g_val} at limb ${i} in final output texture.`);
            }
            finalLimbsLSL.push(r_val === undefined || isNaN(r_val) ? 0 : Math.round(r_val));
        }
         if (isDebugCaseMultiply) {
            console.log(`[Multiply Debug ${debugCaseName}] Final Limbs (LSL, sanitized, width ${propagateTexWidthActual}): [${finalLimbsLSL.join(',')}]`);
        }
