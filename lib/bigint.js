...
class BigIntPrimitive {
  ...
  add(otherBigInt) {
    ...
    if (!this.forceCPU && this.canvas && typeof webglUtilsModule !== 'undefined' && (gl = webglUtilsModule.initWebGL(this.canvas))) {
      // Inside the add method's WebGL block:
   const Ctor = this.constructor; // For creating new BigIntPrimitive instances later
   const webGLAdd = () => { // Wrap WebGL logic in a function for clarity/error handling
       let program;
       let vao; // Vertex Array Object
       let positionBuffer;
       let texCoordBuffer;
       let num1Texture, num2Texture, carryInTexture, outputTexture;
       let fbo;
       let vs, fs; // Make shaders accessible in finally

       try {
           console.log("--- DEBUG WebGL Add: Vertex Shader Source ---");
           console.log(vertexShaderSrc);
           console.log("--- DEBUG WebGL Add: Fragment Shader Source ---");
           console.log(fragmentShaderSrc);
           console.log("---------------------------------------------");
           vs = webglUtilsModule.createShader(gl, gl.VERTEX_SHADER, vertexShaderSrc);
           fs = webglUtilsModule.createShader(gl, gl.FRAGMENT_SHADER, fragmentShaderSrc);
           ...
           const num1TextureLocation = gl.getUniformLocation(program, "u_num1Texture");
           const num2TextureLocation = gl.getUniformLocation(program, "u_num2Texture");
           const carryInTextureLocation = gl.getUniformLocation(program, "u_carryTexture");

           // Prepare data & textures (adjust limb arrays for consistent length)
           const commonExponent = Math.min(this.exponent, otherBigInt.exponent);
           const thisAlignedLimbs = [...this.limbs];
           for (let i = 0; i < (this.exponent - commonExponent); i++) thisAlignedLimbs.push(0);
           const otherAlignedLimbs = [...otherBigInt.limbs];
           for (let i = 0; i < (otherBigInt.exponent - commonExponent); i++) otherAlignedLimbs.push(0);

           const texWidth = Math.max(thisAlignedLimbs.length, otherAlignedLimbs.length);
           if (texWidth === 0) { // Both numbers are effectively zero (e.g. after alignment)
           ...
           const padArray = (arr, len) => {
               ...
               // JS Float32Array is zero-initialized, so remaining elements are 0.0
               return res;
           };

           const num1Padded = padArray(thisAlignedLimbs, texWidth);
           const num2Padded = padArray(otherAlignedLimbs, texWidth);
           const carryInPadded = new Float32Array(texWidth).fill(0.0); // All zeros for initial carry

           num1Texture = webglUtilsModule.createDataTexture(gl, num1Padded, texWidth, 1, false);
           num2Texture = webglUtilsModule.createDataTexture(gl, num2Padded, texWidth, 1, false);
           carryInTexture = webglUtilsModule.createDataTexture(gl, carryInPadded, texWidth, 1, false);
           outputTexture = webglUtilsModule.createDataTexture(gl, null, texWidth, 1, true); // RGBA output

           if (!num1Texture || !num2Texture || !carryInTexture || !outputTexture) {
               throw new Error("Failed to create one or more WebGL textures.");
           }
           ...
           // Setup VAO (optional but good practice)
           if (gl.createVertexArray) { // WebGL2 or OES_vertex_array_object
           ...
           gl.uniform1i(num1TextureLocation, 0);

           gl.activeTexture(gl.TEXTURE1);
           gl.bindTexture(gl.TEXTURE_2D, num2Texture);
           gl.uniform1i(num2TextureLocation, 1);

           gl.activeTexture(gl.TEXTURE2);
           gl.bindTexture(gl.TEXTURE_2D, carryInTexture);
           gl.uniform1i(carryInTextureLocation, 2);

           gl.drawArrays(gl.TRIANGLES, 0, 6);

           const resultDataRGBA = webglUtilsModule.readDataFromTexture(gl, fbo, texWidth, 1, false);
           if (!resultDataRGBA) throw new Error("Failed to read data from output texture.");

           // CPU Carry Propagation
           let gpuResultLimbs = new Float32Array(texWidth);
           let gpuCarryOuts = new Float32Array(texWidth);
           for (let i = 0; i < texWidth; i++) {
               gpuResultLimbs[i] = resultDataRGBA[i * 4 + 0]; // R channel
               gpuCarryOuts[i] = resultDataRGBA[i * 4 + 1];   // G channel
           }

           let finalLimbs = [];
           let currentPropagatedCarry = 0;
           for (let i = 0; i < texWidth; i++) {
               // gpuResultLimbs[i] is already (l1[i]+l2[i]+0)%BASE from shader
               // gpuCarryOuts[i] is floor((l1[i]+l2[i]+0)/BASE) from shader
               let sumForPos = gpuResultLimbs[i] + currentPropagatedCarry;
               finalLimbs.push(sumForPos % BASE);
               currentPropagatedCarry = gpuCarryOuts[i] + Math.floor(sumForPos / BASE);
           }
           while (currentPropagatedCarry > 0) {
               finalLimbs.push(currentPropagatedCarry % BASE);
               currentPropagatedCarry = Math.floor(currentPropagatedCarry / BASE);
           }
            // Remove leading zeros from finalLimbs (which are MSB if generated LSB-first)
           while (finalLimbs.length > 1 && finalLimbs[finalLimbs.length - 1] === 0) {
               finalLimbs.pop();
           }
           if (finalLimbs.length === 0) finalLimbs = [0]; // Should not happen if texWidth > 0

           return { limbs: finalLimbs.reverse(), exponent: commonExponent, sign: this.sign }; // Reverse if processed LSB in texture

       } catch (e) {
           console.error("WebGL Add Error:", e);
           return null; // Fallback to CPU
       } finally {
           // Cleanup WebGL resources
           if (gl) {
               gl.deleteProgram(program);
               if(vs) gl.deleteShader(vs); // Check if vs was defined
               if(fs) gl.deleteShader(fs); // Check if fs was defined
   ...
   }; // end of webGLAdd function

   const webglResultParts = webGLAdd();

   if (webglResultParts) {
       const finalResult = new Ctor("0", this.canvas, { forceCPU: true }); // Create with CPU force to avoid recursion
       finalResult.limbs = webglResultParts.limbs;
       finalResult.exponent = webglResultParts.exponent;
       finalResult.sign = webglResultParts.sign;
       if (finalResult.isZero()) { finalResult.sign = 1; finalResult.exponent = 0; }
       // Normalize exponent from trailing zeros in limbs (if any)
       while (finalResult.limbs.length > 1 && finalResult.limbs[finalResult.limbs.length - 1] === 0) {
           finalResult.limbs.pop();
           finalResult.exponent++;
       }
       if (finalResult.isZero()) { finalResult.sign = 1; finalResult.exponent = 0; } // Re-check zero after normalization
       return finalResult;
   }
    // Fallback to CPU if WebGL path fails or forceCPU is true
    // (The original CPU logic follows here, ensure it's outside the WebGL if block or handled by the fallback)
    }
    // Original CPU Logic (should be here if WebGL failed or was skipped)
    if (this.isZero()) {
