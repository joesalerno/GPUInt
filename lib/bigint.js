...
class BigIntPrimitive {
  ...
  constructor(value, canvas, options = {}) {
    ...
    // console.log('[Debug Constructor options] options:', JSON.stringify(options));
    // if (options) {
    //   console.log('[Debug Constructor options] options.forceCPU:', options.forceCPU);
    //   console.log('[Debug Constructor options] typeof options.forceCPU:', typeof options.forceCPU);
    // }
  ...
  toNumber() {
    ...
    const primitiveNumber = Number(originalString); // This can be Infinity or NaN

    if (this.constructor.strict) {
      if (!isFinite(primitiveNumber)) {
        // In strict mode, if Number(originalString) is not finite (NaN, Infinity, -Infinity),
        // it's considered an imprecise conversion.
        throw new Error("[big.js] Imprecise conversion (non-finite number)");
      }

      // If it's finite, check if it can be converted back to the same string accurately.
      // This ensures that the number representation doesn't lose precision.
      let tempBig;
      const oldStrict = this.constructor.strict; // Save current strict state (should be true)
      try {
        this.constructor.strict = false; // Temporarily disable strict for internal BigIntPrimitive creation
        tempBig = new BigIntPrimitive(primitiveNumber, this.canvas, { forceCPU: this.forceCPU });
        this.constructor.strict = oldStrict; // Restore original strict state
      } catch (e) {
        // This catch is if new BigIntPrimitive(finiteNumber) itself fails,
        // which is unlikely for a standard finite number but good to safeguard.
        this.constructor.strict = oldStrict; // Restore original strict state
        throw new Error("[big.js] Imprecise conversion (internal creation failed for finite number)");
      }

      // Compare the string representation of the re-created BigIntPrimitive with the original.
      // If they differ, precision was lost.
      if (tempBig.toString() !== originalString) {
        throw new Error("[big.js] Imprecise conversion (roundtrip failed)");
      }
    }
    // If not in strict mode, or if all strict checks passed, return the number.
  ...
  add(otherBigInt) {
    ...
    let gl;
    // Ensure WebGL path is taken only if neither operand forces CPU
    // console.log(`[Debug forceCPU in add] this.forceCPU: ${this.forceCPU}, otherBigInt.forceCPU: ${otherBigInt.forceCPU}`);
    // console.log(`[Debug forceCPU in add] typeof this.forceCPU: ${typeof this.forceCPU}, typeof otherBigInt.forceCPU: ${typeof otherBigInt.forceCPU}`);
    ...
    if (this.sign === otherBigInt.sign) {
      // Store original instances for their properties
      const originalThis = this;
      const originalOther = otherBigInt;

      const commonExponent = Math.min(originalThis.exponent, originalOther.exponent);

      let thisCoeffStr = originalThis._getCoefficientString();
      const numZerosThis = originalThis.exponent - commonExponent;
      if (numZerosThis > 0) {
        thisCoeffStr += '0'.repeat(numZerosThis);
      }
      // Ensure tempThisAligned is positive for _core_add
      const tempThisAligned = BigIntPrimitive.fromCoefficientString(thisCoeffStr, this.canvas, { forceCPU: true });
      // tempThisAligned.sign is already 1 from fromCoefficientString if not zero (or should be set by it)
      // The exponent of tempThisAligned will be 0 from fromCoefficientString.

      let otherCoeffStr = originalOther._getCoefficientString();
      const numZerosOther = originalOther.exponent - commonExponent;
      if (numZerosOther > 0) {
        otherCoeffStr += '0'.repeat(numZerosOther);
      }
      // Ensure tempOtherAligned is positive for _core_add
      const tempOtherAligned = BigIntPrimitive.fromCoefficientString(otherCoeffStr, this.canvas, { forceCPU: true });
      // tempOtherAligned.sign is already 1 from fromCoefficientString if not zero
      // The exponent of tempOtherAligned will be 0.

      const sumMagnitudeResult = tempThisAligned._core_add(tempOtherAligned);

      result.limbs = sumMagnitudeResult.limbs;
      result.exponent = commonExponent; // The result's magnitude is at this common exponent
      result.sign = originalThis.sign; // Sign is the common sign of the original numbers

    } else {
      // Signs are different, effectively subtraction.
      // a + (-b) = a - b
      // (-a) + b = b - a
      return this.subtract(otherBigInt.negate());
    }

    // Normalization for zero result or if result became zero after operations
    if (result.isZero()) {
      result.sign = 1;
      result.exponent = 0;
    } else {
      // This normalization of removing trailing zero limbs and adjusting exponent
      // should ideally be part of the BigIntPrimitive constructor or a dedicated normalize method.
      // For now, let's ensure it's applied correctly for add.
      // If _core_add and constructor are robust, this might be redundant or simplified.
      // The exponent is already set to commonExponent. If sumMagnitudeResult has limbs like [X, Y, 0, 0] (BASE-wise),
      // and these are fractional zeros, they should be trimmed and commonExponent adjusted.
      // However, the scaling to commonExponent turns numbers into "integers" relative to that exponent.
      // E.g. 0.1 (1e-1) + 0.0001 (1e-4). commonExponent = -4.
      // 0.1   -> scaled to 1000 (coeff) at exp -4. (1000 * 10^-4)
      // 0.0001 -> scaled to 1    (coeff) at exp -4. (1    * 10^-4)
      // sum = 1001. Result is 1001 * 10^-4 = 0.1001.
      // The sumMagnitudeResult.limbs should be the limbs of "1001". Its exponent should be 0.
      // So result.limbs are correct, result.exponent = commonExponent is correct.
      // The existing while loop for popping zeros might be misinterpreting these.
      // This loop was: `while (result.limbs.length > 1 && result.limbs[result.limbs.length - 1] === 0) { result.limbs.pop(); result.exponent++; }`
      // This implies that if a LSB limb is 0, it's a whole multiple of BASE, so exponent increases by BASE_LOG10.
      // This is more for normalizing integers like "1230000" (limbs [123,0], exp 0) to (limbs [123], exp 1*BASE_LOG10).
      // For results of addition at a commonExponent, this type of normalization might not be what's needed,
      // or rather, the _getCoefficientString and constructor should handle it.

      // Re-evaluating normalization:
      // If the result is not zero, it's already set with sumMagnitudeResult.limbs and commonExponent.
      // The constructor of BigIntPrimitive already normalizes by removing trailing zero limbs and adjusting exponent.
      // The _core_add method also produces a normalized BigIntPrimitive (for its inputs).
      // So, if sumMagnitudeResult is correctly formed and its limbs are assigned to result,
      // and result.exponent is set to commonExponent, this should be sufficient.
      // The main risk is if sumMagnitudeResult itself isn't fully normalized (e.g. [0, 123]).
      // _core_add does: `while (finalResultLimbs.length > 1 && finalResultLimbs[0] === 0) { finalResultLimbs.shift(); }`
      // This handles leading zeros in the coefficient.
      // The critical part is that sumMagnitudeResult is an integer coefficient for the commonExponent.
      // No further exponent adjustment by popping zero limbs should be needed here if inputs to _core_add were correctly scaled integers.
    }
    return result;
  }
  ...
  _core_subtract(positiveOtherBigInt) {
      ...
      let resultLimbsReversed = [];
      let borrow = 0;
      const len1 = minuendLimbs.length; // Should be max length of minuend/subtrahend for proper iteration
      const len2 = subtrahendLimbs.length;
      const maxLength = Math.max(len1, len2);

      for (let i = 0; i < maxLength; i++) { // Iterate up to the maxLength of the two numbers
        let digit1 = minuendLimbs[i] || 0; // Use 0 if index out of bounds
        const digit2 = subtrahendLimbs[i] || 0; // Use 0 if index out of bounds

        let diff = digit1 - borrow - digit2;
        if (diff < 0) { diff += BASE; borrow = 1; } else { borrow = 0; }
        resultLimbsReversed.push(diff);
      }
      // If borrow is still 1, it implies a negative result from magnitude subtraction, which shouldn't happen if minuend > subtrahend.
      // This path assumes minuend's magnitude is >= subtrahend's magnitude.
  ...
  subtract(otherBigInt) {
    ...
    if (!this.forceCPU && !otherBigInt.forceCPU && this.canvas && typeof webglUtilsModule !== 'undefined' && (gl = webglUtilsModule.initWebGL(this.canvas))) {
      ...
      let resultSign; // Not pre-assigned based on this.sign initially for WebGL path

      if (comparison === 0) {
        return new BigIntPrimitive("0", this.canvas, { forceCPU: true }); // Result is zero
      } else if (comparison > 0) { // |this| > |otherBigInt|
        minuend = this.abs();
        subtrahend = otherBigInt.abs();
        resultSign = this.sign;
      } else { // comparison < 0, i.e. |this| < |otherBigInt|
        minuend = otherBigInt.abs();
        subtrahend = this.abs();
        // If this.subtract(other) and |this| < |other|, result is -(other - this)
        // The sign of the result should be opposite to the sign of `this` if `this` and `other` had same sign.
        // Or, more directly, if subtracting B from A (A-B):
        // If A=5, B=10 (5-10 = -5), resultSign = -A.sign (if A is positive) or -this.sign
        // If A=-5, B=-2 (-5 - (-2) = -3), resultSign = A.sign (if A is negative) or this.sign.
        // This path (A-B) is taken if A and B have the same sign.
        // The magnitude is |B|-|A|. The sign is -(sign of A) or -(sign of B).
        resultSign = -this.sign;
      }
      // The problematic block below is removed:
      ...
      if (!program) {
        ...
        // Fallback to the main CPU path logic for subtraction
        // This ensures consistent calculations when WebGL shaders can't be compiled.
        // Note: 'this' and 'otherBigInt' refer to the original inputs to subtract().
        const originalThis = this;
        const originalOther = otherBigInt;

        // --- Start of duplicated CPU path logic ---
        if (originalThis.sign !== originalOther.sign) {
            const termToAdd = new BigIntPrimitive(originalOther, originalThis.canvas, { forceCPU: true }); // Ensure CPU for this internal step
            termToAdd.sign = -termToAdd.sign;
            return originalThis.add(termToAdd); // This add will use its own logic (CPU or WebGL attempt)
        }

        const fallbackCommonExponent = Math.min(originalThis.exponent, originalOther.exponent);
        let fallbackThisCoeffStr = originalThis._getCoefficientString();
        const fallbackNumZerosThis = originalThis.exponent - fallbackCommonExponent;
        if (fallbackNumZerosThis > 0) fallbackThisCoeffStr += '0'.repeat(fallbackNumZerosThis);
        const fallbackAlignedThis = BigIntPrimitive.fromCoefficientString(fallbackThisCoeffStr, originalThis.canvas, { forceCPU: true });

        let fallbackOtherCoeffStr = originalOther._getCoefficientString();
        const fallbackNumZerosOther = originalOther.exponent - fallbackCommonExponent;
        if (fallbackNumZerosOther > 0) fallbackOtherCoeffStr += '0'.repeat(fallbackNumZerosOther);
        const fallbackAlignedOther = BigIntPrimitive.fromCoefficientString(fallbackOtherCoeffStr, originalThis.canvas, { forceCPU: true });

        const fallbackMagnitudeComparison = fallbackAlignedThis.compareMagnitude(fallbackAlignedOther);
        let fallbackResultSign;
        let fallbackTempMinuend, fallbackTempSubtrahend;

        if (fallbackMagnitudeComparison === 0) {
            return new BigIntPrimitive("0", originalThis.canvas, { forceCPU: true });
        } else if (fallbackMagnitudeComparison > 0) {
            fallbackTempMinuend = fallbackAlignedThis;
            fallbackTempSubtrahend = fallbackAlignedOther;
            fallbackResultSign = originalThis.sign;
        } else {
            fallbackTempMinuend = fallbackAlignedOther;
            fallbackTempSubtrahend = fallbackAlignedThis;
            fallbackResultSign = -originalThis.sign;
        }

        if (!fallbackTempMinuend.isZero()) fallbackTempMinuend.sign = 1;
        if (!fallbackTempSubtrahend.isZero()) fallbackTempSubtrahend.sign = 1;

        const fallbackCoreResult = fallbackTempMinuend._core_subtract(fallbackTempSubtrahend);
        const fallbackFinalResult = new BigIntPrimitive(fallbackCoreResult, originalThis.canvas, { forceCPU: true });
        fallbackFinalResult.sign = fallbackResultSign;
        fallbackFinalResult.exponent = fallbackCommonExponent;

        if (fallbackFinalResult.isZero()) {
            fallbackFinalResult.sign = 1;
            fallbackFinalResult.exponent = 0;
        }
        return fallbackFinalResult;
        // --- End of duplicated CPU path logic ---
      }
      ...
      if (gl.checkFramebufferStatus(gl.FRAMEBUFFER) !== gl.FRAMEBUFFER_COMPLETE) {
        ...
        // Fallback to the main CPU path logic for subtraction (same as above)
        // Note: 'this' and 'otherBigInt' refer to the original inputs to subtract().
        const originalThisFb = this;
        const originalOtherFb = otherBigInt;

        // --- Start of duplicated CPU path logic ---
        if (originalThisFb.sign !== originalOtherFb.sign) {
            const termToAddFb = new BigIntPrimitive(originalOtherFb, originalThisFb.canvas, { forceCPU: true });
            termToAddFb.sign = -termToAddFb.sign;
            return originalThisFb.add(termToAddFb);
        }

        const fallbackCommonExponentFb = Math.min(originalThisFb.exponent, originalOtherFb.exponent);
        let fallbackThisCoeffStrFb = originalThisFb._getCoefficientString();
        const fallbackNumZerosThisFb = originalThisFb.exponent - fallbackCommonExponentFb;
        if (fallbackNumZerosThisFb > 0) fallbackThisCoeffStrFb += '0'.repeat(fallbackNumZerosThisFb);
        const fallbackAlignedThisFb = BigIntPrimitive.fromCoefficientString(fallbackThisCoeffStrFb, originalThisFb.canvas, { forceCPU: true });

        let fallbackOtherCoeffStrFb = originalOtherFb._getCoefficientString();
        const fallbackNumZerosOtherFb = originalOtherFb.exponent - fallbackCommonExponentFb;
        if (fallbackNumZerosOtherFb > 0) fallbackOtherCoeffStrFb += '0'.repeat(fallbackNumZerosOtherFb);
        const fallbackAlignedOtherFb = BigIntPrimitive.fromCoefficientString(fallbackOtherCoeffStrFb, originalThisFb.canvas, { forceCPU: true });

        const fallbackMagnitudeComparisonFb = fallbackAlignedThisFb.compareMagnitude(fallbackAlignedOtherFb);
        let fallbackResultSignFb;
        let fallbackTempMinuendFb, fallbackTempSubtrahendFb;

        if (fallbackMagnitudeComparisonFb === 0) {
            return new BigIntPrimitive("0", originalThisFb.canvas, { forceCPU: true });
        } else if (fallbackMagnitudeComparisonFb > 0) {
            fallbackTempMinuendFb = fallbackAlignedThisFb;
            fallbackTempSubtrahendFb = fallbackAlignedOtherFb;
            fallbackResultSignFb = originalThisFb.sign;
        } else {
            fallbackTempMinuendFb = fallbackAlignedOtherFb;
            fallbackTempSubtrahendFb = fallbackAlignedThisFb;
            fallbackResultSignFb = -originalThisFb.sign;
        }

        if (!fallbackTempMinuendFb.isZero()) fallbackTempMinuendFb.sign = 1;
        if (!fallbackTempSubtrahendFb.isZero()) fallbackTempSubtrahendFb.sign = 1;

        const fallbackCoreResultFb = fallbackTempMinuendFb._core_subtract(fallbackTempSubtrahendFb);
        const fallbackFinalResultFb = new BigIntPrimitive(fallbackCoreResultFb, originalThisFb.canvas, { forceCPU: true });
        fallbackFinalResultFb.sign = fallbackResultSignFb;
        fallbackFinalResultFb.exponent = fallbackCommonExponentFb;

        if (fallbackFinalResultFb.isZero()) {
            fallbackFinalResultFb.sign = 1;
            fallbackFinalResultFb.exponent = 0;
        }
        return fallbackFinalResultFb;
        // --- End of duplicated CPU path logic ---
      }
      ...
      if (rawOutputDataRGBA) {
        // Assuming readDataFromTexture (with isOutput=false) returns a flat array of float values (one per component)
        for (let i = 0; i < texWidth; i++) {
          const limbVal = rawOutputDataRGBA[i];
          console.log(`[WebGL Subtract] Shader Output Texel ${i}: limbVal=${limbVal}`);
          resultLimbsReversed.push(limbVal === undefined ? 0 : limbVal); // Push 0 if undefined to prevent TypeErrors
        }
      } else {
        console.error("rawOutputDataRGBA from WebGL read was null. Subtraction may be incorrect.");
        // Ensure resultLimbsReversed is at least [0] if WebGL fails to prevent errors downsteam
        resultLimbsReversed = [0];
    ...
    if (this.sign !== otherBigInt.sign) {
      // e.g. 5 - (-3)  -> 5 + 3
      // e.g. -5 - (3) -> -5 + (-3)
      // This case correctly delegates to add, which has its own alignment logic.
      const termToAdd = new BigIntPrimitive(otherBigInt, this.canvas, { forceCPU: otherBigInt.forceCPU });
      termToAdd.sign = -termToAdd.sign;
      return this.add(termToAdd);
    }

    // Signs are the same. Determine minuend and subtrahend based on magnitude.
    // The final result's sign will depend on this comparison and the original sign.
    const commonExponent = Math.min(this.exponent, otherBigInt.exponent);

    let thisCoeffStr = this._getCoefficientString();
    const numZerosThis = this.exponent - commonExponent;
    if (numZerosThis > 0) thisCoeffStr += '0'.repeat(numZerosThis);
    // Create with forceCPU: true as these are for internal CPU calculation steps
    const alignedThis = BigIntPrimitive.fromCoefficientString(thisCoeffStr, this.canvas, { forceCPU: true });
    // alignedThis now has limbs representing the scaled coefficient, and exponent 0.

    let otherCoeffStr = otherBigInt._getCoefficientString();
    const numZerosOther = otherBigInt.exponent - commonExponent;
    ...
    // alignedOther now has limbs representing the scaled coefficient, and exponent 0.

    // Compare magnitudes of aligned coefficients (which are now true integers with exp 0)
    const magnitudeComparison = alignedThis.compareMagnitude(alignedOther);
    let resultSign;
    let tempMinuend, tempSubtrahend;

    if (magnitudeComparison === 0) {
      return new BigIntPrimitive("0", this.canvas, { forceCPU: true }); // Result is zero
    } else if (magnitudeComparison > 0) { // |this| > |otherBigInt| (magnitudes of aligned numbers)
      tempMinuend = alignedThis;
      tempSubtrahend = alignedOther;
      resultSign = this.sign; // Sign of result is sign of original this
    } else { // |this| < |otherBigInt| (magnitudes of aligned numbers)
      tempMinuend = alignedOther;
      tempSubtrahend = alignedThis;
      resultSign = -this.sign; // Sign of result is opposite of original this
    }

    // _core_subtract expects positive numbers (magnitudes for its internal logic)
    // alignedThis and alignedOther are already positive from their construction using coefficient strings.
    // tempMinuend and tempSubtrahend will also be positive.
    // Ensure their sign property is explicitly 1 if they are not zero.
    if (!tempMinuend.isZero()) tempMinuend.sign = 1;
    if (!tempSubtrahend.isZero()) tempSubtrahend.sign = 1;

    const coreResult = tempMinuend._core_subtract(tempSubtrahend); // coreResult has sign=1

    // Create the final result object directly using copy constructor for limbs, then set properties
    const finalResult = new BigIntPrimitive(coreResult, this.canvas, { forceCPU: this.forceCPU || otherBigInt.forceCPU });
    finalResult.sign = resultSign;
    finalResult.exponent = commonExponent;

    // Final normalization for zero result
  ...
  static fromCoefficientString(coeffStr, canvas, options = {}) {
    const instance = new BigIntPrimitive("0", canvas, options); // Basic setup, forceCPU handled by options
    instance.sign = 1;
    instance.exponent = 0;

    if (coeffStr === "0" || coeffStr === "") { // Handle empty string as zero too
        instance.limbs = [0];
    } else {
        let tempLimbs = [];
        ...
        while (currentPos > 0) {
            const start = Math.max(0, currentPos - BASE_LOG10);
            const limbStr = coeffStr.substring(start, currentPos);
            tempLimbs.unshift(parseInt(limbStr, 10));
            currentPos = start;
        }
        instance.limbs = tempLimbs.length > 0 ? tempLimbs : [0];

        // Remove leading zero limbs from the coefficient string if it was like "00123"
        // This ensures that a coefficient string like "0000" becomes [0].
        while (instance.limbs.length > 1 && instance.limbs[0] === 0) {
            instance.limbs.shift();
        }
        // If, after all, it's zero (e.g. from input "0" or "000"), ensure canonical zero.
        // This check is also important if the input coeffStr was, for example, "0000"
        // and BASE_LOG10 is 2, leading to limbs [0,0] which then becomes [0].
        if (instance.limbs.length === 1 && instance.limbs[0] === 0) {
            // instance.exponent is already 0
            // instance.sign is already 1
        }
    }
    return instance;
  }
  ...
  multiply(otherBigInt) {
    ...
    let absResult;
    let glInstance = null; // Renamed to avoid conflict

    const useWebGL = !self.forceCPU && !otherBigInt.forceCPU && self.canvas &&
                     typeof webglUtilsModule !== 'undefined' &&
                     (glInstance = webglUtilsModule.initWebGL(self.canvas));

    let webGLFailed = false; // Flag to track if WebGL path encounters issues

    if (useWebGL) {
      const num1Abs = self.abs();
      const num2Abs = otherBigInt.abs();
      let accumulatedResult = new BigIntPrimitive("0", self.canvas, { forceCPU: true }); // Accumulator on CPU

      const num1LimbsReversed = [...num1Abs.limbs].reverse(); // LSB first

      for (let i = 0; i < num1LimbsReversed.length; i++) {
        const currentNum1Limb = num1LimbsReversed[i];

        // Optimization: if a limb of num1 is 0, its product with num2 is 0.
        // Only skip if it's not the only limb (0 * X = 0 handled by initial check).
        if (currentNum1Limb === 0 && num1LimbsReversed.length > 1) {
            continue;
        }

        const partialProduct = self._webgl_multiply_one_limb_by_bigint(currentNum1Limb, num2Abs);

        if (!partialProduct) {
          console.warn("WebGL multiplication for one limb failed. Falling back to CPU for entire multiplication.");
          webGLFailed = true;
          break; // Exit loop and proceed to full CPU fallback
        }

        // Exponent adjustment for this partial product:
        // partialProduct from _webgl_... has exponent num2Abs.exponent (relative to num2Abs's coefficient)
        // It needs to be shifted by currentNum1Limb's positional value and num1Abs's own exponent.
        // Effective exponent for this partial product = (exponent of currentNum1Limb) + (exponent of num2Abs)
        // Exponent of currentNum1Limb = num1Abs.exponent (for LSL of num1) + i * BASE_LOG10 (for this specific limb's position)
        partialProduct.exponent += (num1Abs.exponent + i * BASE_LOG10);

        accumulatedResult = accumulatedResult.add(partialProduct); // CPU add for accumulation
      }

      if (!webGLFailed) {
        absResult = accumulatedResult;
        // The exponent of absResult should now be correctly calculated by the series of additions.
        // Each partialProduct had its absolute exponent set before addition.
        // The .add() method aligns exponents. So, absResult.exponent should be the correct final absolute exponent.
      }
    }

    // If WebGL wasn't used or failed, fall into CPU path
    if (!useWebGL || webGLFailed) {
       ...
       // CPU path uses _core_multiply or Karatsuba.
       // These methods expect their inputs (num1, num2 for _core_multiply; a,b,c,d for Karatsuba)
       // to be effectively integer coefficients for the multiplication part.
       // The final exponent (sum of original exponents) is applied after the magnitude is computed.

       // Create integer coefficient representations for CPU math
       const cpuNum1Coeff = new BigIntPrimitive(tempNum1Abs.limbs.join(''), self.canvas, {forceCPU: true});
       cpuNum1Coeff.exponent = tempNum1Abs.exponent; // Keep original exponent for now if _core_multiply handles it.
                                                  // Or, more simply, ensure _core_multiply gets numbers whose .toString() is the pure coefficient.
                                                  // The existing CPU path for Karatsuba does:
                                                  // new BigIntPrimitive(self.limbs.join('') + '0'.repeat(self.exponent), ...)
                                                  // This implies baking the exponent into the coefficient string for CPU ops.

       // Let's use the approach of baking exponent into coefficient for CPU path, ensuring consistency.
       const cpuTempNum1 = new BigIntPrimitive(tempNum1Abs._getCoefficientString() + '0'.repeat(tempNum1Abs.exponent < 0 ? 0 : tempNum1Abs.exponent), self.canvas, {forceCPU: true});
       if (tempNum1Abs.exponent < 0) cpuTempNum1.exponent = tempNum1Abs.exponent; // If it was fractional, restore that aspect for a moment.
                                                                               // This is getting complicated. _core_multiply expects positive BigInts.
                                                                               // And their exponents are used in _multiply_limb_by_bigint & shifts.

       // Simplification: _core_multiply and Karatsuba path should operate on magnitudes (positive, integer coefficients)
       // and the final combined exponent is applied once.

       // tempNum1Abs and tempNum2Abs are already magnitudes.
       // _core_multiply(num1, num2) expects num1, num2 to be positive.
       // It uses num1.limbs and num2.limbs, and num2.exponent.
       // The shifts inside _core_multiply are by `i` (limb index), not BASE_LOG10 * i.

       if (tempNum1Abs.limbs.length < KARATSUBA_THRESHOLD || tempNum2Abs.limbs.length < KARATSUBA_THRESHOLD) {
           absResult = self._core_multiply(tempNum1Abs, tempNum2Abs);
           // _core_multiply's result exponent is based on its inputs' exponents and internal logic.
       } else {
           // Karatsuba path
           // It needs to operate on integer coefficients.
           // Original big.js converts to string, then effectively to integer coefficient by handling exponent.
           // Here, tempNum1Abs and tempNum2Abs are already magnitudes.
           // _splitAt needs to work on these. Let's assume _splitAt works correctly on these BigIntPrimitive magnitudes.
           // The recursive calls to .multiply inside Karatsuba must be CPU-forced if we are in this CPU path.

           const n_cpu = Math.max(tempNum1Abs.limbs.length, tempNum2Abs.limbs.length);
           const m_cpu = Math.floor(n_cpu / 2);

           if (m_cpu === 0) {
               absResult = self._core_multiply(tempNum1Abs, tempNum2Abs);
           } else {
               const { low: b, high: a } = tempNum1Abs._splitAt(m_cpu);
               const { low: d, high: c } = tempNum2Abs._splitAt(m_cpu);

               // Ensure recursive calls use CPU path by creating new options with forceCPU:true
               const cpuOpts = { forceCPU: true };
               const p0 = a.multiply(c, cpuOpts);
               const p1 = b.multiply(d, cpuOpts);
               const sum_ab = a.add(b); // add doesn't have forceCPU in this context, assumes CPU if inputs are CPU
               const sum_cd = c.add(d);
               const p2_temp = sum_ab.multiply(sum_cd, cpuOpts);
               const p0_plus_p1 = p0.add(p1);
               const p2 = p2_temp.subtract(p0_plus_p1); // subtract also assumes CPU
               const p0_shifted = p0._multiplyByPowerOfBase(2 * m_cpu);
               const p2_shifted = p2._multiplyByPowerOfBase(m_cpu);
               let tempSum = p0_shifted.add(p2_shifted);
               absResult = tempSum.add(p1);
           }
       }
       // After CPU path (either _core_multiply or Karatsuba on magnitudes)
       // The result `absResult` has a magnitude. Its exponent needs to be sum of original input exponents.
       absResult.exponent = self.exponent + otherBigInt.exponent;
    }

    // Common post-processing
    // Normalization of limbs and exponent
    if (absResult && !absResult.isZero()) { // Check if absResult is defined
        // Remove trailing zero limbs from the coefficient and adjust exponent accordingly.
        // A limb is a group of digits (BASE_LOG10).
        // If LSB limb is 0, it's like the coefficient was implicitly larger,
        // so removing it means the exponent (power of 10) needs to increase.
        while (absResult.limbs.length > 1 && absResult.limbs[absResult.limbs.length - 1] === 0) {
            absResult.limbs.pop();
            absResult.exponent += BASE_LOG10;
        }
        // Remove leading zero limbs from coefficient (should not typically occur if numbers are normalized)
        while (absResult.limbs.length > 1 && absResult.limbs[0] === 0) {
            absResult.limbs.shift();
            // Exponent doesn't change here, as it's tied to the LSB's position.
        }
    } else if (!absResult) {
      // This case should ideally not be reached if logic is correct (e.g. WebGL failure leading to CPU path)
      // However, if absResult somehow remains undefined, create a zero BigInt.
      absResult = new BigIntPrimitive("0", self.canvas, {forceCPU: true});
    }
  ...
  _webgl_multiply_one_limb_by_bigint(singleHostLimb, otherNumberBigInt_abs) {
    // `singleHostLimb` is a JS number (one limb of self)
    // `otherNumberBigInt_abs` is a BigIntPrimitive (abs value of otherBigInt)
    // This function will use WebGL with multiply_limb.frag shader
    // to compute singleHostLimb * otherNumberBigInt_abs.

    const gl = webglUtilsModule.initWebGL(this.canvas);
    // Guard against cases where gl might not be initializable, though multiply() should check first.
    if (!gl) {
      console.error("WebGL context not available in _webgl_multiply_one_limb_by_bigint.");
      return null; // Indicate failure
    }

    const otherLimbsLSB = [...otherNumberBigInt_abs.limbs].reverse();
    // texWidth needs to accommodate all limbs of otherNumberBigInt_abs + potential carry out of its MSB
    const texWidth = otherLimbsLSB.length + 1;

    const vertShader = webglUtilsModule.createShader(gl, gl.VERTEX_SHADER, multiplyLimbVertexShaderSrc);
    const fragShader = webglUtilsModule.createShader(gl, gl.FRAGMENT_SHADER, multiplyLimbFragmentShaderSrc);
    const program = webglUtilsModule.createProgram(gl, vertShader, fragShader);

    if (!program) {
      console.error("Failed to create shader program for limb multiplication. Fallback needed.");
    ...
    const paddedOtherNumData = new Float32Array(texWidth); // Operates on B_j limbs
    otherLimbsLSB.forEach((val, idx) => paddedOtherNumData[idx] = val);

    // u_carryTexture in multiply_limb.frag is for carry between B_j and B_{j+1} when multiplying by A_i.
    // For the first B_0, this carryIn is 0. The shader itself propagates this.
    // So, the u_carryTexture should be all zeros initially.
    const initialCarryData = new Float32Array(texWidth).fill(0.0);

    const textureOtherNum = webglUtilsModule.createDataTexture(gl, paddedOtherNumData, texWidth, 1, false);
    const textureCarryIn = webglUtilsModule.createDataTexture(gl, initialCarryData, texWidth, 1, false); // Represents carry *within* the singleHostLimb * otherNumber operation
    const outputTexture = webglUtilsModule.createDataTexture(gl, null, texWidth, 1, false); // For shader to write into
    ...
    if (gl.checkFramebufferStatus(gl.FRAMEBUFFER) !== gl.FRAMEBUFFER_COMPLETE) {
      console.error("Framebuffer incomplete for limb multiplication.");
      gl.deleteProgram(program); gl.deleteShader(vertShader); gl.deleteShader(fragShader);
      gl.deleteTexture(textureOtherNum); gl.deleteTexture(textureCarryIn); gl.deleteTexture(outputTexture);
      gl.deleteFramebuffer(fbo);
      return null; // Indicate failure
    }
    ...
    // Set uniforms
    gl.uniform1f(gl.getUniformLocation(program, "u_limbVal"), singleHostLimb);
    gl.uniform1f(gl.getUniformLocation(program, "BASE"), BASE);
    // TEX_WIDTH uniform might be used by shader if it needs to know its own width, e.g. for complex indexing.
    // For multiply_limb.frag as written, v_texCoord is sufficient for 1D texture access.
    // gl.uniform1f(gl.getUniformLocation(program, "TEX_WIDTH"), parseFloat(texWidth));


    gl.activeTexture(gl.TEXTURE0);
    gl.bindTexture(gl.TEXTURE_2D, textureOtherNum);
    gl.uniform1i(gl.getUniformLocation(program, "u_otherNumTexture"), 0);

    gl.activeTexture(gl.TEXTURE1);
    gl.bindTexture(gl.TEXTURE_2D, textureCarryIn);
    ...
    const rawOutputDataRGBA = webglUtilsModule.readDataFromTexture(gl, fbo, texWidth, 1, false); // Get RGBA

    // JS-side carry propagation for the result of (singleHostLimb * otherNumberBigInt_abs)
    // The shader computes (A_i * B_j) + C_in_shader_at_B_j -> R_j, G_j (carry for A_i * B_{j+1})
    // C_in_shader_at_B_j is from u_carryTexture. If u_carryTexture is all zeros, then
    // R_j = (A_i * B_j) % BASE and G_j = floor((A_i * B_j) / BASE).
    // The JS must then correctly sum these: true_limb_j = R_j + G_{j-1} (from previous B limb).
    let resultLimbsLSB = [];
    let currentLimbCarry = 0; // This is the G_{j-1} effectively
    if (rawOutputDataRGBA) {
      for (let j = 0; j < texWidth; j++) { // Iterate over limbs of B processed by shader
        const shaderResultLimb_R = rawOutputDataRGBA[j * 4 + 0]; // (A_i * B_j + C_internal_from_shader_carry_texture_at_j) % BASE
        const shaderCarryOut_G = rawOutputDataRGBA[j * 4 + 1];   // floor((A_i * B_j + C_internal_from_shader_carry_texture_at_j) / BASE)
                                                               // Since C_internal_from_shader_carry_texture_at_j is always 0 for this simplified pass:
                                                               // shaderResultLimb_R = (A_i * B_j) % BASE
                                                               // shaderCarryOut_G = floor((A_i * B_j) / BASE)

        const sumForCurrentLimb = shaderResultLimb_R + currentLimbCarry;
        resultLimbsLSB.push(sumForCurrentLimb % BASE);
        currentLimbCarry = Math.floor(sumForCurrentLimb / BASE) + shaderCarryOut_G;
      }
    }

    // After processing all B limbs, if there's a remaining carry, push it.
    while (currentLimbCarry > 0) {
        resultLimbsLSB.push(currentLimbCarry % BASE);
        currentLimbCarry = Math.floor(currentLimbCarry / BASE);
    }

    // Cleanup
    gl.deleteProgram(program); gl.deleteShader(vertShader); gl.deleteShader(fragShader);
    gl.deleteTexture(textureOtherNum); gl.deleteTexture(textureCarryIn); gl.deleteTexture(outputTexture);
    gl.deleteFramebuffer(fbo); gl.deleteBuffer(positionBuffer);

    // Normalize: remove trailing zeros (which are leading zeros as it's LSB first)
    while (resultLimbsLSB.length > 1 && resultLimbsLSB[resultLimbsLSB.length - 1] === 0) {
      resultLimbsLSB.pop();
    }
    if (resultLimbsLSB.length === 0) resultLimbsLSB = [0];


    const resultBigInt = new BigIntPrimitive("0", this.canvas, { forceCPU: true });
    resultBigInt.limbs = resultLimbsLSB.reverse(); // Convert LSB first to MSB first for BigIntPrimitive

    if (resultBigInt.isZero()) {
        resultBigInt.exponent = 0;
    } else {
        // The exponent of (A_i * B) is B.exponent.
        // The shift for A_i's position (effectively A_i * BASE^k) is handled in the main multiply() method by adjusting the exponent there.
        resultBigInt.exponent = otherNumberBigInt_abs.exponent;
    }
    return resultBigInt;
  }
