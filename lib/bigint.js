...
class BigIntPrimitive {
  ...
  _core_add(positiveOtherBigInt) {
    ...
    try {
      ...
      let vertexShader, fragmentShader; // Keep references for potential deletion if new program fails
      ...
      if (!program) {
        ...
        if (program) {
          ...
          // Shaders are linked to program, can be deleted after successful linking
          if (vertexShader) gl.deleteShader(vertexShader);
          if (fragmentShader) gl.deleteShader(fragmentShader);
        } else {
          // Program creation failed, clean up shaders if they were created
          if (vertexShader) gl.deleteShader(vertexShader);
          if (fragmentShader) gl.deleteShader(fragmentShader);
      ...
      if (!texNum1 || !texNum2 || !texCarryIn || !texOutput) {
          // No need to delete program here as it's cached or failed creation handled above
      ...
      if (gl.checkFramebufferStatus(gl.FRAMEBUFFER) !== gl.FRAMEBUFFER_COMPLETE) {
        ...
        // No need to delete program here
      ...
      // Program and shaders are not deleted here due to caching
  ...
  _core_subtract(positiveOtherBigInt) {
    ...
    try {
      ...
      let program = gl._programCache[opKey];
      let vertexShader, fragmentShader; // Keep references for potential deletion
      ...
      if (!texNum1 || !texNum2 || !texBorrowIn || !texOutput) {
          // Program not deleted due to caching
          if (texNum1) gl.deleteTexture(texNum1);
          if (texNum2) gl.deleteTexture(texNum2);
      ...
      gl.deleteFramebuffer(fbo);
      gl.deleteBuffer(vertexBuffer);
      // Program and shaders not deleted due to caching
