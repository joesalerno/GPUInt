...
class BigIntPrimitive {
  ...
  _webgl_multiply_full(gl, num1, num2) { // Added gl as first parameter
    ...
    try {
        ...
        // Read partial products for debugging
        const partialProductsRawData = webglUtilsModule.readDataFromTexture(gl, fbPartialProducts, partialProductTexDim, partialProductTexDim, false);
        console.log(`[Multiply Debug 7*8] Pass 1 (PartialProducts) output (RGBA, ${partialProductTexDim}x${partialProductTexDim}): [${partialProductsRawData.slice(0,4)}]`);
        ...
        console.log(`[Multiply Debug 7*8] Pass 2 (SumAntiDiagonals) output (RGBA, ${outputTexWidthCoeffSums}x1): [${coeffSumsRawData.slice(0,4)}]`);
        ...
        // Debug log for initial propagation data
        const initialRGPairs = [];
        for (let k = 0; k < Math.min(4, propagateTexWidthForTexture); k++) {
            initialRGPairs.push(`${initialPropagationDataRGBA[k*4+0]},${initialPropagationDataRGBA[k*4+1]}`);
        }
        console.log(`[Multiply Debug 7*8] Pass 3 Initial Data for Propagation (R,G pairs from texCoeffSums, texture_width ${propagateTexWidthForTexture}): [${initialRGPairs.join(',')}]`);
        ...
        while(carriesRemaining && iterationCount < MAX_ITERATIONS) {
            ...
            // Debug log for iteration output
            const iterRGPairs = [];
            for (let k = 0; k < Math.min(4, propagateTexWidthForTexture); k++) {
                const rVal = outputData[k*4+0];
                const gVal = outputData[k*4+1];
                const rFormatted = rVal === 0 ? rVal.toFixed(9) : rVal.toFixed(8);
                const gFormatted = gVal.toFixed(9);
                iterRGPairs.push(`R${k}:${rFormatted} G${k}:${gFormatted}`);
            }
            console.log(`[Multiply Debug 7*8] Pass 3 (Carry Propagation) iter ${iterationCount} output (Detailed R,G pairs): [${iterRGPairs.join('; ')}]`);
        ...
        const finalRGPairs = [];
        for (let k = 0; k < Math.min(propagateTexWidthForTexture, finalLimbsLSL.length + 1); k++) {
            const r = k < finalLimbsLSL.length ? finalLimbsLSL[k] : 0;
            finalRGPairs.push(`${r},0`);
        }
        console.log(`[Multiply Debug 7*8] Pass 3 Final Output (R,G pairs from texture_width ${propagateTexWidthForTexture}): [${finalRGPairs.join(',')}]`);
        console.log(`[Multiply Debug 7*8] Final Limbs (LSL, sanitized, width ${finalLimbsLSL.length}): [${finalLimbsLSL.join(',')}]`);
  ...
  _webgl_divide(gl, dividend, divisor) {
    ...
    try {
        ...
        let X0;
        const divisorValue = parseFloat(B_prime.toString());
        
        if (divisorValue <= 0) {
            throw new Error("Division by zero or negative number");
        }
        
        // Use a simple approximation: 1/divisor
        const startingApprox = 1.0 / divisorValue;
        X0 = new BigIntPrimitive(startingApprox.toString(), this.canvas, { forceCPU: true });
        
        // Calculate number of iterations needed (fewer iterations for better performance)
        const targetDP = BigIntPrimitive.DP + 5; // Extra precision for intermediate calculations
        const numIterations = Math.min(8, Math.max(3, targetDP / 5)); // Reasonable iteration count
        ...
        const result = product.round(BigIntPrimitive.DP, BigIntPrimitive.RM);
