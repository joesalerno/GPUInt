...
class BigIntPrimitive {
  constructor(value, canvas, options = {}) {
    ...
    console.log('[BigIntPrimitive Constructor] forceCPU initialized to:', this.forceCPU);
  }
  ...
  _core_add(positiveOtherBigInt) {
    ...
    try {
      ...
      // --- START OF NEW FBO PARAMETER LOGGING BLOCK ---
      console.log('[FBO PARAMS _core_add] Framebuffer is complete. Querying attachment parameters...');
      const debugAttachment = gl.COLOR_ATTACHMENT0;
      const debugObjType = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, debugAttachment, gl.FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE);
      let debugObjTypeStr = 'Unknown';
      if (debugObjType === gl.TEXTURE) debugObjTypeStr = 'TEXTURE';
      else if (debugObjType === gl.RENDERBUFFER) debugObjTypeStr = 'RENDERBUFFER';
      else if (debugObjType === gl.NONE) debugObjTypeStr = 'NONE';
      else debugObjTypeStr = `Other (${debugObjType})`;
      console.log(`[FBO PARAMS _core_add] COLOR_ATTACHMENT0 Object Type: ${debugObjTypeStr}`);

      const debugRedSize = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, debugAttachment, gl.FRAMEBUFFER_ATTACHMENT_RED_SIZE);
      const debugGreenSize = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, debugAttachment, gl.FRAMEBUFFER_ATTACHMENT_GREEN_SIZE);
      const debugBlueSize = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, debugAttachment, gl.FRAMEBUFFER_ATTACHMENT_BLUE_SIZE);
      const debugAlphaSize = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, debugAttachment, gl.FRAMEBUFFER_ATTACHMENT_ALPHA_SIZE);
      console.log(`[FBO PARAMS _core_add] Attachment RGBA Bit Depths: R=${debugRedSize}, G=${debugGreenSize}, B=${debugBlueSize}, A=${debugAlphaSize}`);

      const GL_FLOAT_ENUM_VAL = 0x1406;
      const GL_UNSIGNED_NORMALIZED_ENUM_VAL = 0x8C17;
      const debugComponentTypeEnumVal = 0x8211;
      let debugComponentTypeStrVal = 'N/A or Error querying COMPONENT_TYPE';
      try {
          const debugCompType = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, debugAttachment, debugComponentTypeEnumVal);
          if (debugCompType === GL_FLOAT_ENUM_VAL) debugComponentTypeStrVal = `FLOAT (${GL_FLOAT_ENUM_VAL})`;
          else if (debugCompType === GL_UNSIGNED_NORMALIZED_ENUM_VAL) debugComponentTypeStrVal = `UNSIGNED_NORMALIZED (${GL_UNSIGNED_NORMALIZED_ENUM_VAL})`;
          else if (debugCompType === 0) debugComponentTypeStrVal = '0 (Query may have failed or not applicable)';
          else debugComponentTypeStrVal = `Other (${debugCompType})`;
      } catch (e_fbo_param_query) {
          console.warn('[FBO PARAMS _core_add] Error querying COMPONENT_TYPE (0x8211):', e_fbo_param_query.message);
      }
      console.log(`[FBO PARAMS _core_add] Attachment Component Type (0x8211): ${debugComponentTypeStrVal}`);

      const glErrorAfterQuery = gl.getError();
      if (glErrorAfterQuery !== gl.NO_ERROR) {
          console.warn('[FBO PARAMS _core_add] GL error immediately after querying FBO params:', glErrorAfterQuery);
      }
      // --- END OF NEW FBO PARAMETER LOGGING BLOCK ---
      ...
      for (let i = 0; i < maxLength; i++) {
        ...
        if (isDebugCase) console.log(`[DEBUG _core_add CPU] i=${i}, initial limb1=${num1Limbs[i]}, initial limb2=${num2Limbs[i]}`);
      ...
      while (carry > 0) {
        const currentPushVal = carry % BASE;
        const nextCarry = Math.floor(carry / BASE);
  ...
  _core_subtract(positiveOtherBigInt) {
    ...
    try {
      ...
      // --- START OF NEW FBO PARAMETER LOGGING BLOCK ---
      console.log('[FBO PARAMS _core_subtract] Framebuffer is complete. Querying attachment parameters...');
      const debugAttachment = gl.COLOR_ATTACHMENT0;
      const debugObjType = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, debugAttachment, gl.FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE);
      let debugObjTypeStr = 'Unknown';
      if (debugObjType === gl.TEXTURE) debugObjTypeStr = 'TEXTURE';
      ...
      console.log(`[FBO PARAMS _core_subtract] COLOR_ATTACHMENT0 Object Type: ${debugObjTypeStr}`);

      const debugRedSize = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, debugAttachment, gl.FRAMEBUFFER_ATTACHMENT_RED_SIZE);
      const debugGreenSize = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, debugAttachment, gl.FRAMEBUFFER_ATTACHMENT_GREEN_SIZE);
      const debugBlueSize = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, debugAttachment, gl.FRAMEBUFFER_ATTACHMENT_BLUE_SIZE);
      const debugAlphaSize = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, debugAttachment, gl.FRAMEBUFFER_ATTACHMENT_ALPHA_SIZE);
      console.log(`[FBO PARAMS _core_subtract] Attachment RGBA Bit Depths: R=${debugRedSize}, G=${debugGreenSize}, B=${debugBlueSize}, A=${debugAlphaSize}`);
      ...
      try {
          ...
          console.warn('[FBO PARAMS _core_subtract] Error querying COMPONENT_TYPE (0x8211):', e_fbo_param_query.message);
      }
      console.log(`[FBO PARAMS _core_subtract] Attachment Component Type (0x8211): ${debugComponentTypeStrVal}`);

      const glErrorAfterQuery = gl.getError();
      if (glErrorAfterQuery !== gl.NO_ERROR) {
          console.warn('[FBO PARAMS _core_subtract] GL error immediately after querying FBO params:', glErrorAfterQuery);
      }
      // --- END OF NEW FBO PARAMETER LOGGING BLOCK ---
  ...
  _multiply_limb_by_bigint(limbValue, otherNumber) {
       ...
       if (gl.checkFramebufferStatus(gl.FRAMEBUFFER) !== gl.FRAMEBUFFER_COMPLETE) {
        ...
        // --- START OF NEW FBO PARAMETER LOGGING BLOCK ---
        console.log('[FBO PARAMS _multiply_limb_by_bigint] Framebuffer is complete. Querying attachment parameters...');
        const debugAttachment = gl.COLOR_ATTACHMENT0;
        const debugObjType = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, debugAttachment, gl.FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE);
        let debugObjTypeStr = 'Unknown';
        if (debugObjType === gl.TEXTURE) debugObjTypeStr = 'TEXTURE';
        ...
        console.log(`[FBO PARAMS _multiply_limb_by_bigint] COLOR_ATTACHMENT0 Object Type: ${debugObjTypeStr}`);

        const debugRedSize = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, debugAttachment, gl.FRAMEBUFFER_ATTACHMENT_RED_SIZE);
        const debugGreenSize = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, debugAttachment, gl.FRAMEBUFFER_ATTACHMENT_GREEN_SIZE);
        const debugBlueSize = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, debugAttachment, gl.FRAMEBUFFER_ATTACHMENT_BLUE_SIZE);
        const debugAlphaSize = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, debugAttachment, gl.FRAMEBUFFER_ATTACHMENT_ALPHA_SIZE);
        console.log(`[FBO PARAMS _multiply_limb_by_bigint] Attachment RGBA Bit Depths: R=${debugRedSize}, G=${debugGreenSize}, B=${debugBlueSize}, A=${debugAlphaSize}`);
        ...
        try {
            ...
            console.warn('[FBO PARAMS _multiply_limb_by_bigint] Error querying COMPONENT_TYPE (0x8211):', e_fbo_param_query.message);
        }
        console.log(`[FBO PARAMS _multiply_limb_by_bigint] Attachment Component Type (0x8211): ${debugComponentTypeStrVal}`);

        const glErrorAfterQuery = gl.getError();
        if (glErrorAfterQuery !== gl.NO_ERROR) {
            console.warn('[FBO PARAMS _multiply_limb_by_bigint] GL error immediately after querying FBO params:', glErrorAfterQuery);
        }
        // --- END OF NEW FBO PARAMETER LOGGING BLOCK ---
