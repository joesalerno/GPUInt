...
class BigIntPrimitive {
    ...
    if (!this.forceCPU && !otherBigInt.forceCPU && this.canvas && typeof webglUtilsModule !== 'undefined') {
      try {
        ...
        if (this.sign !== otherBigInt.sign) {
          ...
          // Persistent program and buffers
          ...
          const texCarryIn = BigIntPrimitive._getOrCreateTexture(cache, 'add_carryIn', texWidth, 1, false);
          const texOutput = BigIntPrimitive._getOrCreateTexture(cache, 'add_output', texWidth, 1, true);
          // Upload data
          gl.bindTexture(gl.TEXTURE_2D, texNum1); gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, texWidth, 1, gl.RGBA, gl.FLOAT, (() => { let arr = new Float32Array(texWidth * 4); for (let i = 0; i < texWidth; i++) arr[i * 4] = limbsA_scaled_f32[i]; return arr; })());
          gl.bindTexture(gl.TEXTURE_2D, texNum2); gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, texWidth, 1, gl.RGBA, gl.FLOAT, (() => { let arr = new Float32Array(texWidth * 4); for (let i = 0; i < texWidth; i++) arr[i * 4] = limbsB_scaled_f32[i]; return arr; })());
          gl.bindTexture(gl.TEXTURE_2D, texCarryIn); gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, texWidth, 1, gl.RGBA, gl.FLOAT, new Float32Array(texWidth * 4));
          // Framebuffer
          ...
          gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer); const positionAttributeLocation = gl.getAttribLocation(addProgram, "a_position"); gl.enableVertexAttribArray(positionAttributeLocation); gl.vertexAttribPointer(positionAttributeLocation, 2, gl.FLOAT, false, 0, 0);
          gl.bindBuffer(gl.ARRAY_BUFFER, texCoordBuffer); const texCoordAttributeLocation = gl.getAttribLocation(addProgram, "a_texCoord"); gl.enableVertexAttribArray(texCoordAttributeLocation); gl.vertexAttribPointer(texCoordAttributeLocation, 2, gl.FLOAT, false, 0, 0);
          gl.drawArrays(gl.TRIANGLES, 0, 6);
          // Multi-pass GPU carry propagation (reuse persistent resources)
          let carryPasses = 0;
          let carryTexInput = texOutput;
          let carryDone = false;
          const MAX_CARRY_PASSES = Math.ceil(Math.log2(texWidth)) + 2;
          const carryProgram = BigIntPrimitive._getOrCreateProgram(cache, 'carry', vertexShaderSrc, propagateCarriesFragmentShaderSrc);
          const carryTexOutput = BigIntPrimitive._getOrCreateTexture(cache, 'carry_output', texWidth, 1, true);
          const carryFbOutput = BigIntPrimitive._getOrCreateFramebuffer(cache, 'carry_fb', carryTexOutput);
          while (!carryDone && carryPasses < MAX_CARRY_PASSES) {
            ...
            if (!carryDone) {
              // Swap input/output for next pass (reuse persistent textures)
              // No need to delete, just swap
              let tmp = carryTexInput;
              carryTexInput = carryTexOutput;
              // carryTexOutput = tmp; // Not needed, always write to carryTexOutput
            }
            carryPasses++;
          }
  ...
  divide(divisorBigInt) {
    ...
    if (!this.forceCPU && !absDivisor.forceCPU && this.canvas && typeof webglUtilsModule !== 'undefined') {
      try {
        const cache = BigIntPrimitive._getWebGLResources(this.canvas);
        if (!cache) throw new Error("WEBGL_CAPABILITY_ERROR: WebGL context not obtainable for divide.");
        const gl = cache.gl;
        // Log for test expectations
        console.log("[GPU Divide Path START]");
        // Try GPU division first
  const quotientResult = this._webgl_divide(cache, absDividend, absDivisor);
  ...
  _webgl_divide(cache, dividend, divisor) {
    // Use persistent WebGL context and resources for division
    const gl = cache.gl;
    try {
      ...
      // --- GPU Newton-Raphson Reciprocal Iterations ---
      // Setup textures for dividend, divisor, and reciprocal estimate
      const texWidth = Math.max(dividend.limbs.length, divisor.limbs.length);
      const dividendTex = BigIntPrimitive._getOrCreateTexture(cache, 'div_dividend', texWidth, 1, false);
      const divisorTex = BigIntPrimitive._getOrCreateTexture(cache, 'div_divisor', texWidth, 1, false);
      const reciprocalTexA = BigIntPrimitive._getOrCreateTexture(cache, 'div_recipA', texWidth, 1, false);
      const reciprocalTexB = BigIntPrimitive._getOrCreateTexture(cache, 'div_recipB', texWidth, 1, false);
      // Upload dividend and divisor
      gl.bindTexture(gl.TEXTURE_2D, dividendTex);
      gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, texWidth, 1, gl.RGBA, gl.FLOAT, (() => { let arr = new Float32Array(texWidth * 4); for (let i = 0; i < texWidth; i++) arr[i * 4] = dividend.limbs[i] || 0; return arr; })());
      gl.bindTexture(gl.TEXTURE_2D, divisorTex);
      gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, texWidth, 1, gl.RGBA, gl.FLOAT, (() => { let arr = new Float32Array(texWidth * 4); for (let i = 0; i < texWidth; i++) arr[i * 4] = divisor.limbs[i] || 0; return arr; })());

      // Initial reciprocal estimate (1/divisor[0])
      let initialRecip = 1.0 / (divisor.limbs[0] || 1);
      gl.bindTexture(gl.TEXTURE_2D, reciprocalTexA);
      gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, texWidth, 1, gl.RGBA, gl.FLOAT, (() => { let arr = new Float32Array(texWidth * 4); for (let i = 0; i < texWidth; i++) arr[i * 4] = initialRecip; return arr; })());

      // Prepare framebuffer for output
      const fbRecip = BigIntPrimitive._getOrCreateFramebuffer(cache, 'div_fb', reciprocalTexB);

      // Use the division shader for Newton-Raphson iterations
      const program = cache.divisionProgram;
      gl.useProgram(program);
      gl.viewport(0, 0, texWidth, 1);
      // Set up attribute pointers (assume already done in persistent setup)

      // Uniform locations
      const u_dividendTexture = gl.getUniformLocation(program, 'u_dividendTexture');
      const u_divisorTexture = gl.getUniformLocation(program, 'u_divisorTexture');
      const u_quotientTexture = gl.getUniformLocation(program, 'u_quotientTexture');
      const u_base = gl.getUniformLocation(program, 'u_base');
      const u_texWidth = gl.getUniformLocation(program, 'u_texWidth');
      const u_iteration = gl.getUniformLocation(program, 'u_iteration');

      // Set static uniforms
      gl.uniform1f(u_base, BASE);
      gl.uniform1f(u_texWidth, texWidth);

      // Newton-Raphson iterations on GPU
      let ping = reciprocalTexA, pong = reciprocalTexB;
      const numIterations = 6; // Can be tuned for accuracy
      for (let iter = 0; iter < numIterations; iter++) {
        gl.activeTexture(gl.TEXTURE0); gl.bindTexture(gl.TEXTURE_2D, divisorTex); gl.uniform1i(u_divisorTexture, 0);
        gl.activeTexture(gl.TEXTURE1); gl.bindTexture(gl.TEXTURE_2D, ping); gl.uniform1i(u_quotientTexture, 1);
        // Not using dividend in reciprocal iteration, but set anyway
        gl.activeTexture(gl.TEXTURE2); gl.bindTexture(gl.TEXTURE_2D, dividendTex); gl.uniform1i(u_dividendTexture, 2);
        gl.uniform1i(u_iteration, iter);
        gl.bindFramebuffer(gl.FRAMEBUFFER, fbRecip);
        gl.drawArrays(gl.TRIANGLES, 0, 6);
        // Swap ping/pong
        let tmp = ping; ping = pong; pong = tmp;
      }

      // Now ping contains the refined reciprocal
      // Final multiply: dividend * reciprocal (on GPU)
      // (Reuse division shader or use multiply shader as appropriate)
      // For now, use division shader for multiply step
      gl.activeTexture(gl.TEXTURE0); gl.bindTexture(gl.TEXTURE_2D, dividendTex); gl.uniform1i(u_dividendTexture, 0);
      gl.activeTexture(gl.TEXTURE1); gl.bindTexture(gl.TEXTURE_2D, divisorTex); gl.uniform1i(u_divisorTexture, 1);
      gl.activeTexture(gl.TEXTURE2); gl.bindTexture(gl.TEXTURE_2D, ping); gl.uniform1i(u_quotientTexture, 2);
      gl.uniform1i(u_iteration, numIterations);
      gl.bindFramebuffer(gl.FRAMEBUFFER, fbRecip);
      gl.drawArrays(gl.TRIANGLES, 0, 6);

      // Read back result
      gl.bindFramebuffer(gl.FRAMEBUFFER, fbRecip);
      const resultData = webglUtilsModule.readDataFromTexture(gl, fbRecip, texWidth, 1, false);
      // Convert resultData to BigIntPrimitive
      const limbs = [];
      for (let i = 0; i < texWidth; i++) limbs.push(resultData[i * 4]);
      let coeffStr = limbs.length > 0 ? String(limbs[limbs.length - 1]) : "0";
      for (let i = limbs.length - 2; i >= 0; i--) coeffStr += String(limbs[i]).padStart(BASE_LOG10, '0');
      const result = BigIntPrimitive.fromCoefficientString(coeffStr, this.canvas, { forceCPU: true });
