...
class BigIntPrimitive {
  ...
  _multiply_limb_by_bigint(limbValue, otherNumber) {
   ...
   _core_multiply(num1, num2) {
    ...
    // The calling public multiply() method should handle signs and forceCPU on these inputs.
    // We use this.forceCPU for new instances created within this core method.
    if (num1.isZero() || num2.isZero()) {
        return new BigIntPrimitive("0", this.canvas, { forceCPU: this.forceCPU });
    }
    // totalResult will inherit forceCPU from `this` (the instance _core_multiply is called on)
