                ...
                if (quadBuffer) {
                  ...
                  if (gpuResultRGBA) {
                    ...
                    for (let i = 0; i < texWidth; i++) {
                      // Shader output: sum_limb in R, carry_out in G
                      const sumLimbFromShader = gpuResultRGBA[i * 4 + 0];
                      const carryFromShader = gpuResultRGBA[i * 4 + 1];

                      const currentSum = sumLimbFromShader + jsCarry;
                      resultLimbsReversed.push(currentSum % BASE);
                      jsCarry = Math.floor(currentSum / BASE) + carryFromShader;
                    }
                    while (jsCarry > 0) {
                      resultLimbsReversed.push(jsCarry % BASE);
                      jsCarry = Math.floor(jsCarry / BASE);
                    }
                    sumMagnitudeResultLimbs = resultLimbsReversed.reverse();
                    gpuPathSuccess = true;
                  }
