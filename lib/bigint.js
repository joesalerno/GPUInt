...
class BigIntPrimitive {
      ...
      try {
        ...
        if (this.sign !== otherBigInt.sign) {
          ...
          try {
            ...
            if (!vs || !fs) {
              throw new Error("WEBGL_FRAMEBUFFER_ERROR: Failed to create shaders for addition.")
            }
            ...
            if (!texNum1 || !texNum2 || !texCarryIn || !texOutput) {
              throw new Error("WEBGL_FRAMEBUFFER_ERROR: Failed to create required textures for addition.")
            }
            ...
            // Multi-pass GPU carry propagation
            let carryPasses = 0;
            let carryTexInput = texOutput;
            let carryTexOutput = null;
            let carryFbOutput = null;
            let carryProgram = null;
            let carryDone = false;
            const MAX_CARRY_PASSES = Math.ceil(Math.log2(texWidth)) + 2; // Safety margin
            while (!carryDone && carryPasses < MAX_CARRY_PASSES) {
              // Setup propagate_carries shader
              const carryVs = webglUtilsModule.createShader(gl, gl.VERTEX_SHADER, vertexShaderSrc);
              const carryFs = webglUtilsModule.createShader(gl, gl.FRAGMENT_SHADER, propagateCarriesFragmentShaderSrc);
              carryProgram = webglUtilsModule.createProgram(gl, carryVs, carryFs);
              if (!carryProgram) throw new Error('WebGL program creation failed for carry propagation.');

              carryTexOutput = webglUtilsModule.createDataTexture(gl, null, texWidth, 1, true);
              carryFbOutput = gl.createFramebuffer();
              gl.bindFramebuffer(gl.FRAMEBUFFER, carryFbOutput);
              gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, carryTexOutput, 0);
              gl.useProgram(carryProgram);
              gl.viewport(0, 0, texWidth, 1);

              gl.activeTexture(gl.TEXTURE0);
              gl.bindTexture(gl.TEXTURE_2D, carryTexInput);
              gl.uniform1i(gl.getUniformLocation(carryProgram, "u_inputTexture"), 0);
              gl.uniform1f(gl.getUniformLocation(carryProgram, "u_base"), BASE);
              gl.uniform1f(gl.getUniformLocation(carryProgram, "u_textureWidth"), texWidth);

              // Setup attributes (reuse positionBuffer, texCoordBuffer)
              gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
              const posLoc = gl.getAttribLocation(carryProgram, "a_position");
              gl.enableVertexAttribArray(posLoc);
              gl.vertexAttribPointer(posLoc, 2, gl.FLOAT, false, 0, 0);
              gl.bindBuffer(gl.ARRAY_BUFFER, texCoordBuffer);
              const texLoc = gl.getAttribLocation(carryProgram, "a_texCoord");
              gl.enableVertexAttribArray(texLoc);
              gl.vertexAttribPointer(texLoc, 2, gl.FLOAT, false, 0, 0);

              gl.drawArrays(gl.TRIANGLES, 0, 6);

              // Read back the carry channel (G) to check if any carry remains
              const carryData = webglUtilsModule.readDataFromTexture(gl, carryFbOutput, texWidth, 1, false);
              let maxCarry = 0;
              for (let i = 0; i < texWidth; i++) {
                if (carryData[i * 4 + 1] > 0.0) { maxCarry = 1; break; }
              }
              carryDone = maxCarry === 0;

              // Prepare for next pass if needed
              if (!carryDone) {
                // Swap input/output for next pass
                gl.deleteTexture(carryTexInput);
                gl.deleteFramebuffer(carryFbOutput);
                gl.deleteProgram(carryProgram);
                carryTexInput = carryTexOutput;
              }
              carryPasses++;
            }

            // Read final result from GPU (R channel)
            const finalData = webglUtilsModule.readDataFromTexture(gl, carryFbOutput, texWidth, 1, false);
            const actualLimbsArr = new Array(texWidth);
            for (let i = 0; i < texWidth; i++) actualLimbsArr[i] = finalData[i * 4 + 0];
            ...
            let finalCoeffStr = String(actualLimbsArr[actualLimbsArr.length - 1]);
            for (let i = actualLimbsArr.length - 2; i >= 0; i--) finalCoeffStr += String(actualLimbsArr[i]).padStart(BASE_LOG10, '0');
            ...
            webglSuccess = true;
            // Cleanup
            if (carryTexInput) gl.deleteTexture(carryTexInput);
            if (carryTexOutput) gl.deleteTexture(carryTexOutput);
            if (carryFbOutput) gl.deleteFramebuffer(carryFbOutput);
            if (carryProgram) gl.deleteProgram(carryProgram);
            return webGLResult;
  ...
  _webgl_subtract(gl, minuendLimbsLSLPadded, subtrahendLimbsLSLPadded, texWidth) {
    ...
    let program, texMinuend, texSubtrahend, texBorrowIn, texOutput, fbOutput, positionBuffer;
    try {
      ...
      // Multi-pass GPU borrow propagation (reuse propagate_carries shader for borrows)
      let borrowPasses = 0;
      let borrowTexInput = texOutput;
      let borrowTexOutput = null;
      let borrowFbOutput = null;
      let borrowProgram = null;
      let borrowDone = false;
      const MAX_BORROW_PASSES = Math.ceil(Math.log2(texWidth)) + 2;
      while (!borrowDone && borrowPasses < MAX_BORROW_PASSES) {
        // Setup propagate_carries shader (used for borrows)
        const borrowVs = webglUtilsModule.createShader(gl, gl.VERTEX_SHADER, vertexShaderSrc);
        const borrowFs = webglUtilsModule.createShader(gl, gl.FRAGMENT_SHADER, propagateCarriesFragmentShaderSrc);
        borrowProgram = webglUtilsModule.createProgram(gl, borrowVs, borrowFs);
        if (!borrowProgram) throw new Error('WebGL program creation failed for borrow propagation.');

        borrowTexOutput = webglUtilsModule.createDataTexture(gl, null, texWidth, 1, true);
        borrowFbOutput = gl.createFramebuffer();
        gl.bindFramebuffer(gl.FRAMEBUFFER, borrowFbOutput);
        gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, borrowTexOutput, 0);
        gl.useProgram(borrowProgram);
        gl.viewport(0, 0, texWidth, 1);

        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, borrowTexInput);
        gl.uniform1i(gl.getUniformLocation(borrowProgram, "u_inputTexture"), 0);
        gl.uniform1f(gl.getUniformLocation(borrowProgram, "u_base"), BASE);
        gl.uniform1f(gl.getUniformLocation(borrowProgram, "u_textureWidth"), texWidth);

        gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
        const posLoc = gl.getAttribLocation(borrowProgram, "a_position");
        gl.enableVertexAttribArray(posLoc);
        gl.vertexAttribPointer(posLoc, 2, gl.FLOAT, false, 0, 0);
        gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
        const texLoc = gl.getAttribLocation(borrowProgram, "a_texCoord");
        gl.enableVertexAttribArray(texLoc);
        gl.vertexAttribPointer(texLoc, 2, gl.FLOAT, false, 0, 0);

        gl.drawArrays(gl.TRIANGLES, 0, 6);

        // Read back the borrow channel (G) to check if any borrow remains
        const borrowData = webglUtilsModule.readDataFromTexture(gl, borrowFbOutput, texWidth, 1, false);
        let maxBorrow = 0;
        for (let i = 0; i < texWidth; i++) {
          if (borrowData[i * 4 + 1] > 0.0) { maxBorrow = 1; break; }
        }
        borrowDone = maxBorrow === 0;

        // Prepare for next pass if needed
        if (!borrowDone) {
          gl.deleteTexture(borrowTexInput);
          gl.deleteFramebuffer(borrowFbOutput);
          gl.deleteProgram(borrowProgram);
          borrowTexInput = borrowTexOutput;
        }
        borrowPasses++;
      }

      // Read final result from GPU (R channel)
      const finalData = webglUtilsModule.readDataFromTexture(gl, borrowFbOutput, texWidth, 1, false);
      const actualLimbsArrLSL = new Array(texWidth);
      for (let i = 0; i < texWidth; i++) actualLimbsArrLSL[i] = finalData[i * 4 + 0];
      ...
      // Cleanup
      if (borrowTexInput) gl.deleteTexture(borrowTexInput);
      if (borrowTexOutput) gl.deleteTexture(borrowTexOutput);
      if (borrowFbOutput) gl.deleteFramebuffer(borrowFbOutput);
      if (borrowProgram) gl.deleteProgram(borrowProgram);
  ...
  _webgl_multiply_full(gl, num1, num2) { // Added gl as first parameter
    ...
    try {
      ...
      if (posAttrLocMultiply === -1) { throw new Error("WEBGL_SHADER_ERROR: Could not find attribute 'a_position' in multiply shader") }
      gl.enableVertexAttribArray(posAttrLocMultiply); gl.vertexAttribPointer(posAttrLocMultiply, 2, gl.FLOAT, false, 0, 0);
      ...
      if (posAttrLocSum === -1) { throw new Error("WEBGL_SHADER_ERROR: Could not find attribute 'a_position' in sum antiDiagonals shader") }
      ...
      let propagateTexWidthActual = outputTexWidthCoeffSums + 1;
      ...
      texPropagatePing = webglUtilsModule.createDataTexture(gl, initialPropagationDataRGBA, propagateTexWidthForTexture, 1, true);
      ...
      if (posAttrLocProp === -1) { throw new Error("WEBGL_SHADER_ERROR: Could not find attribute 'a_position' in propagate carries shader") }
      gl.enableVertexAttribArray(posAttrLocProp);
      gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
      gl.vertexAttribPointer(posAttrLocProp, 2, gl.FLOAT, false, 0, 0);

      let currentPropagateInputTexture = texPropagatePing;
      let currentPropagateOutputFramebuffer = fbPropagatePong;
      
      // A carry can propagate at most one limb per step. In the worst case, a carry
      // needs to travel across all limbs. We'll perform a fixed number of iterations
      // sufficient for all carries to settle, avoiding costly GPU-CPU syncs in a loop.
      const NUM_ITERATIONS = propagateTexWidthForTexture;

      for (let i = 0; i < NUM_ITERATIONS; i++) {
        gl.bindFramebuffer(gl.FRAMEBUFFER, currentPropagateOutputFramebuffer);
        gl.viewport(0, 0, propagateTexWidthForTexture, 1);
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, currentPropagateInputTexture);
        gl.uniform1i(uInputTexLocProp, 0);
        gl.uniform1f(uBaseLocProp, BASE);
        gl.uniform1f(uTexWidthLocProp, propagateTexWidthForTexture);

        gl.drawArrays(gl.TRIANGLES, 0, 6);

        // Swap ping-pong buffers for the next iteration
        const tempTexture = currentPropagateInputTexture;
        currentPropagateInputTexture = (currentPropagateOutputFramebuffer === fbPropagatePing) ? texPropagatePing : texPropagatePong;
        currentPropagateOutputFramebuffer = (currentPropagateOutputFramebuffer === fbPropagatePing) ? fbPropagatePong : fbPropagatePing;
      }

      // After the loop, the final result was written to currentPropagateOutputFramebuffer.
      gl.finish(); // Ensure all operations are done before reading
      const finalResultRGBA = webglUtilsModule.readDataFromTexture(gl, currentPropagateOutputFramebuffer, propagateTexWidthForTexture, 1, false);
      ...
      // Fallback to CPU on any internal error
  ...
  _webgl_reciprocal_using_bigint_ops(B_prime_BigInt, X0_BigInt, numIterations, targetOutputDP) {
    ...
    // Use much higher working precision for exact results - scale with input size
    const inputSizeForWork = Math.max(this.limbs.length, B_prime_BigInt.limbs.length)
    const workingDP = Math.max(50, targetOutputDP + 50) + Math.min(inputSizeForWork * 10, 50) // Much higher precision
    ...
    // Use CPU operations for all Newton-Raphson arithmetic to avoid precision issues
    const B_prime_CPU = new BigIntPrimitive(B_prime_BigInt.toString(), this.canvas, { forceCPU: true })
    const X0_CPU = new BigIntPrimitive(X0_BigInt.toString(), this.canvas, { forceCPU: true })
    const two = new BigIntPrimitive("2", this.canvas, { forceCPU: true })

    let X_current = X0_CPU

    console.log(`[_webgl_reciprocal_using_bigint_ops] B_prime_CPU: ${B_prime_CPU.toString()}`)
    console.log(`[_webgl_reciprocal_using_bigint_ops] X0_CPU: ${X0_CPU.toString()}`)
    ...
    for (let i = 0; i < numIterations; i++) {
      ...
      // Step 3: X_current * (2 - B * X_current)
      ...
      // Check for convergence - if X hasn't changed much, we're done
      if (i > 0) {
        const diff = next_X.subtract(X_current).abs()
        const threshold = new BigIntPrimitive("1e-" + (workingDP - 2), this.canvas, { forceCPU: true })
    ...
    // Verify the result: X * B should be close to 1
    const verification = X_current.multiply(B_prime_CPU)
  ...
  _webgl_divide(gl, dividend, divisor) {
    // Use BigInt-based Newton-Raphson reciprocal calculation for GPU division
    // This is more accurate than pure shader-based division

    try {
      // For simple cases where divisor is 1, just return dividend
      if (divisor.limbs.length === 1 && divisor.limbs[0] === 1 && divisor.exponent === 0) {
        const result = new BigIntPrimitive(dividend, this.canvas, { forceCPU: true })
        return result
      }

      // Generate starting approximation for reciprocal
      const B_prime = new BigIntPrimitive(divisor, this.canvas, { forceCPU: true })

      // Better starting approximation based on divisor value
      let X0

      // Calculate target precision with much more buffer for exact results
      // For exact division, we need significantly more precision than the target
      const inputSizeForTarget = Math.max(dividend.limbs.length, divisor.limbs.length)
      const inputDigits = Math.max(dividend.toString().length, divisor.toString().length)
      const targetDP = Math.max(BigIntPrimitive.DP + 50, inputDigits + 30, 50) // Much higher precision for exact results

      // For very large numbers, use a more sophisticated starting approximation
      const divisorStr = B_prime.toString()
      let divisorValue

      if (divisorStr.length > 15) {
        // For very large numbers, use scientific notation to avoid precision loss
        const scientificStr = B_prime.toExponential(15)
        divisorValue = parseFloat(scientificStr)
      } else {
        divisorValue = parseFloat(divisorStr)
      }
      ...
      // Use more precise starting approximation
      let startingApprox = 1.0 / divisorValue

      // If the initial approximation is too small or large, adjust it
      if (startingApprox < 1e-50 || startingApprox > 1e50) {
        // For very large or very small numbers, use a different approach
        const B_str = B_prime.toString()
        const B_magnitude = B_str.length
        startingApprox = parseFloat("1e-" + (B_magnitude - 1))
      }

      // Ensure starting approximation has enough precision
      const startingPrecision = Math.max(25, targetDP)
      X0 = new BigIntPrimitive(startingApprox.toPrecision(startingPrecision), this.canvas, { forceCPU: true })

      // Calculate number of iterations needed - more iterations for higher precision
      const numIterations = Math.floor(Math.min(15, Math.max(8, Math.log10(targetDP) * 3)))

      // Calculate reciprocal using WebGL-accelerated operations
      const reciprocal = this._webgl_reciprocal_using_bigint_ops(B_prime, X0, numIterations, targetDP)

      // Special case: if dividend is 1, return the reciprocal directly to avoid precision loss
      if (dividend.limbs.length === 1 && dividend.limbs[0] === 1 && dividend.exponent === 0) {
        // Apply the same rounding as we would for the product
        const result = reciprocal.round(BigIntPrimitive.DP, BigIntPrimitive.RM)
        console.log(`[GPU Divide Path END]`)
        return result
      }

      // For large numbers, use higher precision for the multiplication
      const originalDP = BigIntPrimitive.DP
      const originalRM = BigIntPrimitive.RM

      // Use higher precision for large number multiplication to avoid precision loss
      const inputSize = Math.max(dividend.limbs.length, divisor.limbs.length)
      const extraPrecision = Math.max(10, inputSize * 5) // Scale precision with input size
      BigIntPrimitive.DP = originalDP + extraPrecision
      BigIntPrimitive.RM = BigIntPrimitive.roundHalfEven // Use banker's rounding for intermediate calculations

      // Multiply dividend by reciprocal
      const product = dividend.multiply(reciprocal)

      // Restore original precision settings
      BigIntPrimitive.DP = originalDP
      BigIntPrimitive.RM = originalRM

      // Apply final rounding with original precision
      const result = product.round(originalDP, originalRM)

      console.log(`[GPU Divide Path END]`)
      return result

    } catch (e) {
      // Fall back to CPU if anything fails
      throw new Error(`GPU division failed: ${e.message}`)
    }
  }
