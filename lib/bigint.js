...
class BigIntPrimitive {
  ...
  _webgl_multiply_full(num1, num2) {
    ...
    const is7x8 = num1.limbs.length === 1 && num1.limbs[0] === 7 && num2.limbs.length === 1 && num2.limbs[0] === 8;
    // Debug for "largeNum * 1" scenario. num1 is selfCoeff, num2 is otherCoeff.
    // selfCoeff for "123456789012345678901234567890" (original)
    // otherCoeff for "1" (original)
    // num1.limbs will be LSL of selfCoeff, num2.limbs will be LSL of otherCoeff ([1])
    const isLargeNumTimesOne = num2.limbs.length === 1 && num2.limbs[0] === 1 && num1.limbs.length > 1;
    const is9999x9999 = num1.limbs.length === 1 && num1.limbs[0] === 9999 && num2.limbs.length === 1 && num2.limbs[0] === 9999;
    // Check for LSL: [9999, 9999] for num1 and num2
    const is99999999x99999999 =
        num1.limbs.length === 2 && num1LimbsLSL[0] === 9999 && num1LimbsLSL[1] === 9999 &&
        num2.limbs.length === 2 && num2LimbsLSL[0] === 9999 && num2LimbsLSL[1] === 9999;

    const isDebugCaseMultiply = is9999x9999; // Focus on 9999*9999 for readback issue
    let debugCaseName = "N/A";
    if (is7x8) debugCaseName = "7*8";
    else if (isLargeNumTimesOne) debugCaseName = "largeN*1";
    else if (is9999x9999) debugCaseName = "9999*9999";
    else if (is99999999x99999999) debugCaseName = "99999999*99999999";
    ...
    try {
        ...
        gl.uniform1f(gl.getUniformLocation(sumAntidiagonalsProgram, "u_lenA"), lenA);
        gl.uniform1f(gl.getUniformLocation(sumAntidiagonalsProgram, "u_lenB"), lenB);
        gl.uniform1f(gl.getUniformLocation(sumAntidiagonalsProgram, "u_productTexWidth"), partialProductTexDim);
        gl.uniform1f(gl.getUniformLocation(sumAntidiagonalsProgram, "u_productTexHeight"), partialProductTexDim);
        gl.uniform1f(gl.getUniformLocation(sumAntidiagonalsProgram, "u_base"), BASE);
        gl.uniform1f(gl.getUniformLocation(sumAntidiagonalsProgram, "u_outputTexWidth"), outputTexWidthCoeffSums);
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer); // Already has data
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, texCoordBuffer); // Already has data
        gl.vertexAttribPointer(texCoordAttrLocSum, 2, gl.FLOAT, false, 0, 0);

        gl.viewport(0, 0, outputTexWidthCoeffSums, 1);
        gl.clearColor(0,0,0,0); gl.clear(gl.COLOR_BUFFER_BIT);
        gl.drawArrays(gl.TRIANGLES, 0, 6);
        ...
        resultBigInt.exponent = num1.exponent + num2.exponent; // Exponent from original numbers
        // Sign is handled by the caller `multiply` method. This function returns magnitude.
  ...
  _webgl_reciprocal(gl, texB, texBWidth, texX0, texX0Width, numIterations, targetPrecisionLimbs) {
    ...
    // let texCurrentX = texX0; // Duplicate declaration
    // let currentXWidth = texX0Width; // Duplicate declaration
    let fbCurrentX = null;
