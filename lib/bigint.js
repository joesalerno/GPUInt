...
class BigIntPrimitive {
  ...
  _multiply_limb_by_bigint(limbValue, otherNumber) {
    ...
    // This method computes limbValue * otherNumber.
    // It assumes otherNumber is positive. The sign is handled by the caller.

    // Zero checks are done first as they are cheap and apply to both paths.
    if (limbValue === 0 || otherNumber.isZero()) {
        return new this.constructor("0", this.canvas);
    }

    try {
      // --- START OF WEBGL ATTEMPT ---
      if (!this.canvas) {
          throw new Error("Canvas not available for WebGL operation in _multiply_limb_by_bigint.");
      }
      ...
      const carryInLimbsData = new Float32Array(maxLength); // Should be all zeros
      ...
      const texCarryIn = webglUtilsModule.createDataTexture(gl, carryInLimbsData, texWidth, texHeight, false); // Pass all-zero carry texture
      const texOutput = webglUtilsModule.createDataTexture(gl, new Float32Array(texWidth * texHeight * 4), texWidth, texHeight, true);
      ...
      gl.uniform1i(gl.getUniformLocation(program, "u_carryTexture"), 1); // Corrected from u_carryTexture to u_carryTexture (was already correct)
      ...
      const finalResultLimbsWebGL = [];
      let propagatedCarryWebGL = 0;
      for (let i = 0; i < maxLength; i++) {
          let currentProductSum = resultLimbsFromGPU[i] + propagatedCarryWebGL;
          finalResultLimbsWebGL.push(currentProductSum % BASE);
          propagatedCarryWebGL = carryOutFromGPU[i] + Math.floor(currentProductSum / BASE);
      }

      while (propagatedCarryWebGL > 0) {
          finalResultLimbsWebGL.push(propagatedCarryWebGL % BASE);
          propagatedCarryWebGL = Math.floor(propagatedCarryWebGL / BASE);
      }

      // Normalize
      while (finalResultLimbsWebGL.length > 1 && finalResultLimbsWebGL[finalResultLimbsWebGL.length - 1] === 0) {
        finalResultLimbsWebGL.pop();
      }
      if (finalResultLimbsWebGL.length === 0) { finalResultLimbsWebGL.push(0); }
      ...
      const resultNumWebGL = new this.constructor("0", this.canvas);
      resultNumWebGL.limbs = finalResultLimbsWebGL;
      resultNumWebGL.sign = 1;
      if (resultNumWebGL.isZero()) resultNumWebGL.sign = 1;
      return resultNumWebGL;
      // --- END OF WEBGL ATTEMPT ---

    } catch (error) {
      // console.warn(`WebGL _multiply_limb_by_bigint failed: ${error.message}. Falling back to CPU.`); // Optional

      // CPU Fallback Logic
      let resultLimbsCPU = [];
      let carryCPU = 0;
      for (let i = 0; i < otherNumber.limbs.length; i++) {
        const product = limbValue * otherNumber.limbs[i] + carryCPU;
      ...
      // Normalize
      while (resultLimbsCPU.length > 1 && resultLimbsCPU[resultLimbsCPU.length - 1] === 0) {
        resultLimbsCPU.pop();
      }
      if (resultLimbsCPU.length === 0) { // Should only occur if limbValue was 0 or otherNumber was 0 (already handled)
        resultLimbsCPU.push(0);
      }

      const resultNumCPU = new this.constructor("0", this.canvas);
      resultNumCPU.limbs = resultLimbsCPU;
      resultNumCPU.sign = 1; // Result is positive as inputs are effectively positive here
  ...
  _longDivide(positiveDividend, positiveDivisor) {
    ...
    // const Ctor = this.constructor; // REMOVED REDUNDANT DECLARATION - Ctor is already defined above

    const P_DIVIDEND_STR = "12345678901234567890";
    const P_DIVISOR_STR = "987654321";
    const isDebuggingThisCase = (positiveDividend.toString() === P_DIVIDEND_STR && positiveDivisor.toString() === P_DIVISOR_STR);

    if (isDebuggingThisCase) {
      console.log(`--- _longDivide DEBUG START ---`);
      console.log(`Initial Dividend: ${positiveDividend.toString()} (limbs: ${positiveDividend.limbs.join(',')})`);
      console.log(`Initial Divisor: ${positiveDivisor.toString()} (limbs: ${positiveDivisor.limbs.join(',')})`);
    }
    ...
    for (let i = positiveDividend.limbs.length - 1; i >= 0; i--) {
      ...
      if (isDebuggingThisCase) {
        console.log(`\n[Outer Loop i=${i}] Bringing down limb: ${dividendLimb}`);
        console.log(`  CWD before shift: ${currentWorkingDividend.toString()} (limbs: ${currentWorkingDividend.limbs.join(',')})`);
      }

      // 1. Multiply currentWorkingDividend by BASE
      currentWorkingDividend = currentWorkingDividend._multiplyByPowerOfBase(1);
      if (isDebuggingThisCase) {
        console.log(`  CWD after shift: ${currentWorkingDividend.toString()} (limbs: ${currentWorkingDividend.limbs.join(',')})`);
      }

      // 2. Add the current dividendLimb
      // Create a BigIntPrimitive for the single limb to use the 'add' method.
      const limbAsBigInt = new Ctor(String(dividendLimb), this.canvas); // this.canvas will be null if called from test
      currentWorkingDividend = currentWorkingDividend.add(limbAsBigInt);

      if (isDebuggingThisCase) {
        console.log(`  CWD after adding limb ${dividendLimb}: ${currentWorkingDividend.toString()} (limbs: ${currentWorkingDividend.limbs.join(',')})`);
      }
      ...
      if (isDebuggingThisCase) {
        comparisonResult = currentWorkingDividend.compareMagnitude(positiveDivisor);
        console.log(`  Starting inner subtraction loop. CWD: ${currentWorkingDividend.toString()}, Divisor: ${positiveDivisor.toString()}`);
        console.log(`  Initial CWD.compareMagnitude(Divisor): ${comparisonResult}`);
      }

      while (!(comparisonResult = currentWorkingDividend.compareMagnitude(positiveDivisor), comparisonResult < 0) && !currentWorkingDividend.isZero()) {
        // Loop while CWD >= PD and CWD is not zero.
        // The check `!currentWorkingDividend.isZero()` is mostly for safety if PD is zero, but that's an error case handled earlier.
        // If CWD becomes zero, `compareMagnitude(PD)` would be -1 (if PD non-zero), terminating.
        // If CWD is zero and PD is zero, `compareMagnitude` is 0, but PD zero is error.
        if (isDebuggingThisCase) {
             console.log(`    Inner loop: CWD: ${currentWorkingDividend.toString()} (limbs: ${currentWorkingDividend.limbs.join(',')}), Q_limb_val: ${quotientLimbValue}, Comparison: ${comparisonResult}`);
        }
      ...
      if (isDebuggingThisCase) {
        console.log(`  Inner loop finished. Final Q_limb_val: ${quotientLimbValue}`);
        console.log(`  CWD (step remainder): ${currentWorkingDividend.toString()} (limbs: ${currentWorkingDividend.limbs.join(',')})`);
      }
    ...
    if (isDebuggingThisCase) {
      ...
      console.log(`Final Quotient Limbs (MSB first): [${quotientLimbs.join(',')}]`);
    }
    ...
    if (quotientLimbs.length === 0 || (quotientLimbs.length === 1 && quotientLimbs[0] === 0)) {
    ...
    if (isDebuggingThisCase) {
      console.log(`Final assembled Quotient: ${finalQuotient.toString()}`);
      console.log(`Final Remainder: ${currentWorkingDividend.toString()}`);
      console.log(`--- _longDivide DEBUG END ---`);
    }
