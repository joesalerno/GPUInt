...
describe('BigIntPrimitive', () => {
  ...
  describe('multiply() - public method with Karatsuba and sign logic', () => {
    ...
    it('should use Karatsuba for large numbers (calls _splitAt and _core_multiply at base)', () => {
        ...
        // For 80-digit numbers with threshold 20:
        // L1: n=80,m=40. (sub-problems > 20, so Karatsuba)
        // L2: n=40,m=20. (sub-problems == 20, so Karatsuba by current rule < not >=)
        // L3: n=20,m=10. (sub-problems < 20, so _core_multiply)
        // Expected _core_multiply calls = 3*3*3 = 27.
        expect(coreMultiplySpy.mock.calls.length).toBe(27);
        // Public multiply calls: 1 (initial) + 3 (L1) + 9 (L2) = 13
        expect(publicMultiplySpy.mock.calls.length).toBe(13);
