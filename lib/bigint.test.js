...
describe('BigIntPrimitive', () => {
  ...
  describe('add() with WebGL mock', () => {
    ...
    it('should add two multi-limb numbers with carries', () => {
      ...
      readDataMock.mockImplementation((gl, fbo, width, height, isOutput) => {
        const outputPixelDataRGBA = new Float32Array(width*4);
        // Inputs: N1 (123456789) -> limbs [6789, 2345, 1]
        //         N2 (987654321) -> limbs [4321, 8765, 9]
        // Shader output for (L1[i]+L2[i]):
        // L0 (6789+4321=11110): resultLimb=1110, carryOut=1
        // L1 (2345+8765=11110): resultLimb=1110, carryOut=1
        // L2 (1+9=10):         resultLimb=10,  carryOut=0
      ...
      const result = num1.add(num2);
      // Expect the true arithmetic sum.
  ...
  describe('multiply() - public method with Karatsuba and sign logic', () => {
    ...
    it('should use Karatsuba for large numbers (calls _splitAt and _core_multiply at base)', () => {
        ...
        // So, a.multiply(c) etc. use _core_multiply.
        // The initial call + 3 recursive calls to public multiply.
        expect(publicMultiplySpy.mock.calls.length).toBe(4); // Corrected from 1 to 4
  ...
  describe('_estimate_quotient_limb()', () => {
    const instance = new BigIntPrimitive("0", mockCanvas); // Dummy instance to call _estimate_quotient_limb
    const divisor = new BigIntPrimitive("20", mockCanvas);
    let mlbbSpy;

    beforeEach(() => {
      // Mock _multiply_limb_by_bigint for calls made by _estimate_quotient_limb
      // This mock uses native BigInt for robustness in calculating product.
      mlbbSpy = vi.spyOn(instance, '_multiply_limb_by_bigint').mockImplementation((limb, dBigInt) => {
        const jsProd = BigInt(limb) * BigInt(dBigInt.toString());
        return new BigIntPrimitive(jsProd.toString(), mockCanvas);
      });
    });
    ...
    it('estimates quotient limb correctly when segment < divisor', () => {
      const segment = new BigIntPrimitive("10", mockCanvas);
      expect(instance._estimate_quotient_limb(segment, divisor)).toBe(0);
    });

    it('estimates quotient limb correctly when segment === divisor', () => {
      const segment = new BigIntPrimitive("20", mockCanvas);
      expect(instance._estimate_quotient_limb(segment, divisor)).toBe(1);
    });

    it('estimates quotient limb for segment > divisor (e.g., 45 / 20 -> q_limb=2)', () => {
      const segment = new BigIntPrimitive("45", mockCanvas);
      expect(instance._estimate_quotient_limb(segment, divisor)).toBe(2);
    });

    it('estimates quotient limb for segment slightly less than 3*divisor (e.g., 59 / 20 -> q_limb=2)', () => {
      const segment = new BigIntPrimitive("59", mockCanvas);
      expect(instance._estimate_quotient_limb(segment, divisor)).toBe(2);
    });

    it('estimates quotient limb at BASE-1 correctly', () => {
      // segment = (BASE-1) * 20. Example: BASE=10000, (9999)*20 = 199980
      const segmentValue = BigInt(BigIntPrimitive.BASE - 1) * BigInt(20);
      const segment = new BigIntPrimitive(segmentValue.toString(), mockCanvas);
      expect(instance._estimate_quotient_limb(segment, divisor)).toBe(BigIntPrimitive.BASE - 1);
    });

    it('estimates quotient limb when product is very large', () => {
      const largeDivisor = new BigIntPrimitive("1".repeat(20), mockCanvas); // A large divisor
      const segmentVal = BigInt(5) * BigInt(largeDivisor.toString());
      const segment = new BigIntPrimitive(segmentVal.toString(), mockCanvas);
      // Redefine spy for this specific instance as 'instance' is shared.
      // This is a bit tricky as the spy is on instance.prototype, not specific instance.
      // The beforeEach spy should be fine.
      expect(instance._estimate_quotient_limb(segment, largeDivisor)).toBe(5);
    });
     it('estimates quotient limb as 0 if segment is 0', () => {
      const segment = new BigIntPrimitive("0", mockCanvas);
      expect(instance._estimate_quotient_limb(segment, divisor)).toBe(0);
    });
  });

  describe('divide() - public method', () => {
    // Helper function for testing division results
    function expectDivision(dividendStr, divisorStr, expectedQStr, expectedRStr) {
      const dividend = new BigIntPrimitive(dividendStr, mockCanvas);
      const divisor = new BigIntPrimitive(divisorStr, mockCanvas);

      // Robustly mock core arithmetic operations for the duration of this divide test
      // to ensure the division logic itself is tested without interference from
      // potentially unspecific WebGL mocks for underlying operations.
      const mlbbSpy = vi.spyOn(BigIntPrimitive.prototype, '_multiply_limb_by_bigint').mockImplementation(function(limb, otherNum) {
        const resultBigInt = BigInt(limb) * BigInt(otherNum.toString());
        return new BigIntPrimitive(resultBigInt.toString(), this.canvas);
      });

      const coreAddSpy = vi.spyOn(BigIntPrimitive.prototype, '_core_add').mockImplementation(function(otherNum) {
        const resultBigInt = BigInt(this.abs().toString()) + BigInt(otherNum.abs().toString()); // _core_add expects positive
        return new BigIntPrimitive(resultBigInt.toString(), this.canvas);
      });

      const coreSubtractSpy = vi.spyOn(BigIntPrimitive.prototype, '_core_subtract').mockImplementation(function(otherNum) {
        // _core_subtract expects this.abs() >= otherNum.abs()
        const thisVal = BigInt(this.abs().toString());
        const otherVal = BigInt(otherNum.abs().toString());
        const resultBigInt = thisVal >= otherVal ? thisVal - otherVal : otherVal - thisVal; // Simplified: assumes correct calling context
        return new BigIntPrimitive(resultBigInt.toString(), this.canvas);
      });

      // Spy on _core_divide itself to check if it's called, but don't mock its implementation here
      // as we are testing the logic that _calls_ _core_divide.
      // The actual _core_divide will run using the above mocks for its helper arithmetic.
      const actualCoreDivideSpy = vi.spyOn(dividend, '_core_divide');
      ...
      const expectedQuotientSign = (expectedQStr === "0") ? 1 : ((dividend.sign === divisor.sign) ? 1 : -1);
      const expectedRemainderSign = (expectedRStr === "0") ? 1 : dividend.sign;

      expect(quotient.sign).toBe(expectedQuotientSign);
      expect(remainder.sign).toBe(expectedRemainderSign);

      mlbbSpy.mockRestore();
      coreAddSpy.mockRestore();
      coreSubtractSpy.mockRestore();
      actualCoreDivideSpy.mockRestore(); // Restore the spy on _core_divide
    }

    it('throws error for division by zero', () => {
      const n1 = new BigIntPrimitive("10", mockCanvas);
      const n0 = new BigIntPrimitive("0", mockCanvas);
      expect(() => n1.divide(n0)).toThrow("Division by zero");
    });

    it('dividend zero results in Q=0, R=0', () => {
      expectDivision("0", "10", "0", "0");
    });

    it('abs(dividend) < abs(divisor) results in Q=0, R=dividend', () => {
      expectDivision("3", "10", "0", "3");
    });

    it('abs(dividend) < abs(divisor) with negative dividend', () => {
      expectDivision("-3", "10", "0", "-3");
    });

    it('abs(dividend) < abs(divisor) with negative divisor', () => {
      expectDivision("3", "-10", "0", "3");
    });

    it('abs(dividend) < abs(divisor) with both negative', () => {
      expectDivision("-3", "-10", "0", "-3");
    });

    describe('Standard division cases', () => {
      it('10 / 3 = Q:3, R:1',         () => expectDivision("10", "3", "3", "1"));
      it('10 / 5 = Q:2, R:0',         () => expectDivision("10", "5", "2", "0"));
      it('7 / 1 = Q:7, R:0',           () => expectDivision("7", "1", "7", "0"));
      it('7 / 7 = Q:1, R:0',           () => expectDivision("7", "7", "1", "0"));
      it('12345 / 67 = Q:184, R:17',  () => expectDivision("12345", "67", "184", "17"));
      it('876 / 23 = Q:38, R:2', () => expectDivision("876", "23", "38", "2"));


      // Cases involving BASE
      const baseStr = String(BigIntPrimitive.BASE);
      const basePlusOne = String(BigIntPrimitive.BASE + 1);
      const baseTimesTwo = String(BigIntPrimitive.BASE * 2);
      const baseMinusOne = String(BigIntPrimitive.BASE -1);

      it(`${baseTimesTwo} / ${baseStr} = Q:2, R:0`, () => expectDivision(baseTimesTwo, baseStr, "2", "0"));
      it(`(${baseTimesTwo}+5) / ${baseStr} = Q:2, R:5`, () => expectDivision(String(BigIntPrimitive.BASE * 2 + 5), baseStr, "2", "5"));
      it(`(${baseStr}*20+5) / ${baseStr} = Q:20, R:5`, () => expectDivision(String(BigIntPrimitive.BASE * 20 + 5), baseStr, "20", "5"));
      it(`${baseMinusOne} / ${baseStr} = Q:0, R:${baseMinusOne}`, () => expectDivision(baseMinusOne, baseStr, "0", baseMinusOne));
      it(`${baseStr} / ${basePlusOne} = Q:0, R:${baseStr}`, () => expectDivision(baseStr, basePlusOne, "0", baseStr));
       it(`10000 / 10000 = Q:1, R:0 (BASE specific)`, () => expectDivision("10000", "10000", "1", "0"));
       it(`9999 / 10000 = Q:0, R:9999 (BASE specific)`, () => expectDivision("9999", "10000", "0", "9999"));
       it(`10001 / 10000 = Q:1, R:1 (BASE specific)`, () => expectDivision("10001", "10000", "1", "1"));
       it(`19999 / 10000 = Q:1, R:9999 (BASE specific)`, () => expectDivision("19999", "10000", "1", "9999"));
       it(`20000 / 10000 = Q:2, R:0 (BASE specific)`, () => expectDivision("20000", "10000", "2", "0"));
    });

    describe('Sign combinations for division', () => {
      it('(+Dve / -Dvr) -> -Q, +R (10 / -3 = Q:-3, R:1)', () => expectDivision("10", "-3", "-3", "1"));
      it('(-Dve / +Dvr) -> -Q, -R (-10 / 3 = Q:-3, R:-1)', () => expectDivision("-10", "3", "-3", "-1"));
      it('(-Dve / -Dvr) -> +Q, -R (-10 / -3 = Q:3, R:-1)', () => expectDivision("-10", "-3", "3", "-1"));

      it('(+Dve / -Dvr) -> -Q, +R (12345 / -67 = Q:-184, R:17)', () => expectDivision("12345", "-67", "-184", "17"));
      it('(-Dve / +Dvr) -> -Q, -R (-12345 / 67 = Q:-184, R:-17)', () => expectDivision("-12345", "67", "-184", "-17"));
      it('(-Dve / -Dvr) -> +Q, -R (-12345 / -67 = Q:184, R:-17)', () => expectDivision("-12345", "-67", "184", "-17"));

      it('Division resulting in zero quotient with signs: -3 / 10 = Q:0, R:-3', () => expectDivision("-3", "10", "0", "-3"));
      it('Division resulting in zero quotient with signs: 3 / -10 = Q:0, R:3', () => expectDivision("3", "-10", "0", "3"));
    });

    // Test that _core_divide is indeed called for non-trivial cases
    it('_core_divide is called for actual division scenarios', () => {
        const dividend = new BigIntPrimitive("100", mockCanvas);
        const divisor = new BigIntPrimitive("10", mockCanvas);
        const coreDivideSpy = vi.spyOn(dividend, '_core_divide').mockImplementation(() => {
            return {
                quotient: new BigIntPrimitive("10", mockCanvas),
                remainder: new BigIntPrimitive("0", mockCanvas)
            };
        });
        dividend.divide(divisor);
        expect(coreDivideSpy).toHaveBeenCalledTimes(1);
        // Check that it's called with abs values
        expect(coreDivideSpy.mock.calls[0][0].toString()).toBe("100");
        expect(coreDivideSpy.mock.calls[0][0].sign).toBe(1);
        expect(coreDivideSpy.mock.calls[0][1].toString()).toBe("10");
        expect(coreDivideSpy.mock.calls[0][1].sign).toBe(1);
        coreDivideSpy.mockRestore();
    });
  });
});
