...
describe('BigIntPrimitive', () => {
  ...
  describe('_core_multiply() - internal multiplication logic', () => {
    ...
    it('single-limb * single-limb', () => {
        ...
        // No mock for _core_add, use actual implementation
    ...
    it('multi-limb * single-limb (e.g., 10001 * 5 = 50005, BASE 10k)', () => {
      // setupMockWebGLForCoreMultiply(); // Handled by beforeEach
        const instanceForCPU = new BigIntPrimitive("0", mockCanvas, {forceCPU: true});
        const num1 = new BigIntPrimitive('10001', mockCanvas); // limbs [1, 1] (BASE 10k)
        const num2 = new BigIntPrimitive('5', mockCanvas);     // limbs [5] (BASE 10k)
        const localPrototypeMlbbSpy = vi.spyOn(BigIntPrimitive.prototype, '_multiply_limb_by_bigint');

        // No mock for _core_add, use actual implementation
        const result = instanceForCPU._core_multiply(num1, num2);
        expect(localPrototypeMlbbSpy).toHaveBeenCalledTimes(2);
        expect(result.toString()).toBe('50005');
    });
    it('123 * 45 = 5535', () => {
      const instanceForCPU = new BigIntPrimitive("0", mockCanvas, {forceCPU: true});
      const num1 = new BigIntPrimitive('123', mockCanvas); // limbs [123] (BASE 10k)
      const num2 = new BigIntPrimitive('45', mockCanvas);  // limbs [45] (BASE 10k)
      const localPrototypeMlbbSpy = vi.spyOn(BigIntPrimitive.prototype, '_multiply_limb_by_bigint');

      // Using actual _multiply_limb_by_bigint and _core_add implementations
      const result = instanceForCPU._core_multiply(num1, num2);

      // num1.limbs (MSB if BASE=10k) = [123]. Reversed LSB-first for processing in _core_multiply = [123]. Loop i=0.
      // So, _multiply_limb_by_bigint is called once with 123.
      expect(localPrototypeMlbbSpy).toHaveBeenCalledTimes(1);
      expect(localPrototypeMlbbSpy).toHaveBeenNthCalledWith(1, 123, num2); // num2 is BigIntPrimitive('45')
      expect(result.toString()).toBe('5535');
      // Removed incorrect mock implementations and duplicate assertions that were here.
    });
  });
...
describe('divide()', () => {
  ...
  test('DEBUG TEST: 10 / 4 (DP=1) _decimalDivide raw output', () => {
    ...
    console.log(`DEBUG TEST 10/4: Calling _decimalDivide with:`);
    console.log(`DEBUG TEST 10/4:   dividend (num1.abs().toString()): ${num1.abs().toString()}`);
    console.log(`DEBUG TEST 10/4:   divisor (num2.abs().toString()): ${num2.abs().toString()}`);
    console.log(`DEBUG TEST 10/4:   internalPrecision: ${internalPrecision}`);
