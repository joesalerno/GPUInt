...
describe('BigIntPrimitive', () => {
  ...
  describe('_core_subtract() with WebGL mock', () => {
    ...
    it('should subtract with borrow handled by shader and JS propagation', () => {
      ...
      // For _core_subtract, inputs should be aligned. "12345" (limbs [1,2,3,4,5], exp 0) and "2346" (limbs [2,3,4,6], exp 0)
      const num1 = new BigIntPrimitive('0', mockCanvas); num1.limbs = [1,2,3,4,5]; num1.exponent=0; num1.sign=1;
      const num2 = new BigIntPrimitive('0', mockCanvas); num2.limbs = [2,3,4,6]; num2.exponent=0; num2.sign=1;
    ...
    it('should correctly subtract "10000" - "1" (with aligned inputs)', () => {
      setupMockWebGLForSubtract();

      const num1 = new BigIntPrimitive('0', mockCanvas);
      num1.limbs = [1,0,0,0,0]; // Representing 10000 with exponent 0
      num1.exponent = 0;
      num1.sign = 1;

      const num2 = new BigIntPrimitive('0', mockCanvas);
      num2.limbs = [1];         // Representing 1 with exponent 0
      num2.exponent = 0;
      num2.sign = 1;

      // WebGL mock for readDataFromTexture is not relevant for _core_subtract as it's CPU only.
      // The 'with WebGL mock' in describe seems misleading for this CPU method.
      // vi.spyOn(webglUtils,'readDataFromTexture').mockImplementation(/* ... */); // Removed
    ...
    it('should handle multi-limb subtraction with borrows', () => {
      setupMockWebGLForSubtract();
      // num1='12345678' -> limbs [1,2,3,4,5,6,7,8], exp 0 (after constructor normalization)
      // num2='3456789'  -> limbs [3,4,5,6,7,8,9], exp 0
      const num1 = new BigIntPrimitive('0', mockCanvas); num1.limbs = [1,2,3,4,5,6,7,8]; num1.exponent=0; num1.sign=1;
      const num2 = new BigIntPrimitive('0', mockCanvas); num2.limbs = [3,4,5,6,7,8,9]; num2.exponent=0; num2.sign=1;

      // vi.spyOn(webglUtils,'readDataFromTexture').mockImplementation((gl,fbo,width,height,isOutput) => {
      //   const o=new Float32Array(width*4);
      //   // This mock needs to be correct for the actual limb-wise subtraction of 12345678 - 3456789
      //   // 12345678
      //   // -3456789
      //   // ---------
      //   //  8888889
      //   // limbs for 8888889 are [8,8,8,8,8,8,9]
      //   // The mock here was for a BASE=10000 system, which is not current.
      //   // For single digit limbs, the result [8,8,8,8,8,8,9] would be directly computed.
      //   // The _core_subtract is CPU only, so no WebGL mock needed here.
      //   return o;
      // });
  ...
  describe('_multiply_limb_by_bigint() with WebGL mock', () => {
    ...
    it('limbValue * single-limb otherNumber, no final carry', () => {
      setupMockWebGLForMulLimb();
      const otherNumber = new BigIntPrimitive('1000', mockCanvas); // limbs [1,0,0,0], exp 0
    ...
    it('limbValue * single-limb otherNumber, with final carry', () => {
      setupMockWebGLForMulLimb();
      const otherNumber = new BigIntPrimitive('3000', mockCanvas); // limbs [3,0,0,0], exp 0
    ...
    it('limbValue * multi-limb otherNumber, no JS propagated carry', () => {
      setupMockWebGLForMulLimb();
      const otherNumber = new BigIntPrimitive('32001', mockCanvas); // limbs [3,2,0,0,1] exp 0
    ...
    it('limbValue * multi-limb otherNumber, with JS propagated carry', () => {
      setupMockWebGLForMulLimb();
      const otherNumber = new BigIntPrimitive('10001', mockCanvas); // limbs [1,0,0,0,1] exp 0
    ...
    it('limbValue * otherNumber, where final propagatedCarry requires splitting', () => {
        setupMockWebGLForMulLimb();
        const otherNumber = new BigIntPrimitive('9999', mockCanvas); // limbs [9,9,9,9] exp 0
  ...
  describe('_core_multiply() - internal multiplication logic', () => {
    ...
    it('multi-limb * single-limb (e.g., 10001 * 5 = 50005, BASE 10k)', () => {
      setupMockWebGLForCoreMultiply();
        ...
        mlbbSpy.mockImplementationOnce((l,o) => (l===1&&o.toString()==='5')?new BigIntPrimitive('5',mockCanvas):new BigIntPrimitive('0',mockCanvas)); // For limb 10001 -> 1
        mlbbSpy.mockImplementationOnce((l,o) => (l===0&&o.toString()==='5')?new BigIntPrimitive('0',mockCanvas):new BigIntPrimitive('0',mockCanvas)); // For limb 10001 -> 0
        mlbbSpy.mockImplementationOnce((l,o) => (l===0&&o.toString()==='5')?new BigIntPrimitive('0',mockCanvas):new BigIntPrimitive('0',mockCanvas)); // For limb 10001 -> 0
        mlbbSpy.mockImplementationOnce((l,o) => (l===0&&o.toString()==='5')?new BigIntPrimitive('0',mockCanvas):new BigIntPrimitive('0',mockCanvas)); // For limb 10001 -> 0
        mlbbSpy.mockImplementationOnce((l,o) => (l===1&&o.toString()==='5')?new BigIntPrimitive('5',mockCanvas):new BigIntPrimitive('0',mockCanvas)); // For limb 10001 -> 1
        ...
        const result = instanceForCanvas._core_multiply(num1, num2);
        expect(mlbbSpy).toHaveBeenCalledTimes(num1.limbs.length); expect(result.toString()).toBe('50005');
    });
    it('123 * 45 = 5535', () => {
      setupMockWebGLForCoreMultiply();
        ...
        const result = instanceForCanvas._core_multiply(num1, num2);
        expect(mlbbSpy).toHaveBeenCalledWith(123, num2); // This will fail as _core_multiply calls it limb by limb
