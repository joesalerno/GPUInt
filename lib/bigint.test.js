...
/*
  describe('sqrt()', () => {
    ...
    test('Error Handling: negative inputs', () => {
      const neg = new BigIntPrimitive('-2');
      expect(() => neg.sqrt()).toThrowError('[big.js] No square root of negative number');
    });
    ...
    describe('Perfect Squares', () => {
      test('integer perfect squares', () => {
        expect(new BigIntPrimitive('4').sqrt().toString()).toBe('2');
        expect(new BigIntPrimitive('9').sqrt().toString()).toBe('3');
        expect(new BigIntPrimitive('16').sqrt().toString()).toBe('4');
        expect(new BigIntPrimitive('100').sqrt().toString()).toBe('10');
        expect(new BigIntPrimitive('144').sqrt().toString()).toBe('12');
        expect(new BigIntPrimitive('625').sqrt().toString()).toBe('25');
      });

      test('decimal perfect squares', () => {
        BigIntPrimitive.DP = 5;
        expect(new BigIntPrimitive('0.25').sqrt().toString()).toBe('0.5');
        expect(new BigIntPrimitive('0.0001').sqrt().toString()).toBe('0.01');
        expect(new BigIntPrimitive('6.25').sqrt().toString()).toBe('2.5');
        expect(new BigIntPrimitive('0.00000001').sqrt().toString()).toBe('0.0001'); // 1e-8
      });

      test('large number perfect squares', () => {
        BigIntPrimitive.DP = 0;
        expect(new BigIntPrimitive('1000000').sqrt().toString()).toBe('1000');
        expect(new BigIntPrimitive('1234567890123456790123456789').sqrt().toString()).toBe('11111111111111111'); // approx, depends on internal precision. Let's verify with more DP
        BigIntPrimitive.DP = 2; // Need enough DP for the result
         // 111111111.11 * 111111111.11 = 12345679012.3456790121 - this will test large number handling
        expect(new BigIntPrimitive('12345679012.3456790121').sqrt().toString()).toBe('111111.11');


        const veryLargeSquare = '1' + '0'.repeat(30); // 10^30
        const veryLargeRoot = '1' + '0'.repeat(15); // 10^15
        expect(new BigIntPrimitive(veryLargeSquare).sqrt().toString()).toBe(veryLargeRoot);
      });
    });

    describe('Precision Control (BigIntPrimitive.DP)', () => {
      test('sqrt(2) with varying DP', () => {
        BigIntPrimitive.DP = 0;
        expect(new BigIntPrimitive('2').sqrt().toString()).toBe('1'); // Rounded to 0 DP

        BigIntPrimitive.DP = 5;
        expect(new BigIntPrimitive('2').sqrt().toString()).toBe('1.41421');

        BigIntPrimitive.DP = 10;
        expect(new BigIntPrimitive('2').sqrt().toString()).toBe('1.4142135624');

        BigIntPrimitive.DP = 20; // Default for many tests
        expect(new BigIntPrimitive('2').sqrt().toString()).toBe('1.4142135623730950488');
      });

      test('sqrt(3) with varying DP', () => {
        BigIntPrimitive.DP = 0;
        expect(new BigIntPrimitive('3').sqrt().toString()).toBe('2'); // Rounded to 0 DP

        BigIntPrimitive.DP = 5;
        expect(new BigIntPrimitive('3').sqrt().toString()).toBe('1.73205');

        BigIntPrimitive.DP = 10;
        expect(new BigIntPrimitive('3').sqrt().toString()).toBe('1.7320508102');
      });
    });

    describe('Rounding Mode Control (BigIntPrimitive.RM)', () => {
      // sqrt(2.1025) = 1.45
      // sqrt(2.4025) = 1.55
      // sqrt(8) approx 2.8284271247...

      test('sqrt(2.1025) with RM_HALF_UP and RM_DOWN (DP=1)', () => {
        BigIntPrimitive.DP = 1;
        const n = new BigIntPrimitive('2.1025');

        BigIntPrimitive.RM = BigIntPrimitive.roundHalfUp; // Default
        expect(n.sqrt().toString()).toBe('1.5'); // 1.45 rounds to 1.5

        BigIntPrimitive.RM = BigIntPrimitive.roundDown;
        expect(n.sqrt().toString()).toBe('1.4');
      });

      test('sqrt(2.4025) with RM_HALF_UP and RM_DOWN (DP=1)', () => {
        BigIntPrimitive.DP = 1;
        const n = new BigIntPrimitive('2.4025'); // sqrt is 1.55

        BigIntPrimitive.RM = BigIntPrimitive.roundHalfUp;
        expect(n.sqrt().toString()).toBe('1.6'); // 1.55 rounds to 1.6

        BigIntPrimitive.RM = BigIntPrimitive.roundDown;
        expect(n.sqrt().toString()).toBe('1.5');
      });

      test('sqrt(8) with various rounding modes (DP=4)', () => {
        BigIntPrimitive.DP = 4; // sqrt(8) ~ 2.828427...
        const eight = new BigIntPrimitive('8');

        BigIntPrimitive.RM = BigIntPrimitive.roundDown; // 0
        expect(eight.sqrt().toString()).toBe('2.8284');

        BigIntPrimitive.RM = BigIntPrimitive.roundHalfUp; // 1
        expect(eight.sqrt().toString()).toBe('2.8284'); // next digit is 2

        BigIntPrimitive.RM = BigIntPrimitive.roundHalfEven; // 2
        // 2.82842... last digit is 4 (even), next is 2, so round down
        expect(eight.sqrt().toString()).toBe('2.8284');

        // Test with a value that would make roundHalfEven differ
        // sqrt(x) = Y.abcde5... where e is even, then round down
        // sqrt(x) = Y.abcdf5... where f is odd, then round up
        // Example: sqrt(7.999684) approx 2.8283679... (DP=5)
        // RM_HALF_EVEN should be 2.82837
        // Let's use a number that results in xxxxx5 exact for RM_HALF_EVEN test
        // (2.82845)^2 = 8.0000060025
        BigIntPrimitive.DP = 4; // back to 4 for this
        const numForHalfEven = new BigIntPrimitive('8.0000060025'); // sqrt = 2.82845
        BigIntPrimitive.RM = BigIntPrimitive.roundHalfEven; // 2
        expect(numForHalfEven.sqrt().toString()).toBe('2.8284'); // 4 is even, so round down

        const numForHalfEvenOdd = new BigIntPrimitive('7.9994492025'); // sqrt = 2.828315
         BigIntPrimitive.RM = BigIntPrimitive.roundHalfEven; // 2
        expect(numForHalfEvenOdd.sqrt().toString()).toBe('2.8283'); // 1 is odd, 5 rounds up

        BigIntPrimitive.RM = BigIntPrimitive.roundUp; // 3
        expect(eight.sqrt().toString()).toBe('2.8285'); // always rounds up if any fractional part beyond DP
      });
    });

    describe('Larger numbers and complex scenarios', () => {
      test('sqrt(12345.6789)', () => {
        BigIntPrimitive.DP = 8; // sqrt is approx 111.1111107...
        expect(new BigIntPrimitive('12345.6789').sqrt().toString()).toBe('111.11111071');
      });

      test('sqrt(2e-20)', () => {
        BigIntPrimitive.DP = 15; // sqrt is 1.414213562373095e-10
        // Note: toString() for very small numbers might use scientific notation by default
        // or might produce '0.000...'. We need to ensure the comparison is robust.
        // The internal representation is what matters for sqrt.
        // The test here will depend on the default PE/NE values for toString.
        // Let's set PE high enough to avoid sci notation if possible for direct comparison.
        const originalPE = BigIntPrimitive.PE;
        BigIntPrimitive.PE = 100; // Allow more digits before sci notation
        const expected = '0.000000000141421'; // 1.41421e-10 with 15 DP on the value part
        expect(new BigIntPrimitive('2e-20').sqrt().toString()).toBe(expected);
        BigIntPrimitive.PE = originalPE;
      });

      test('sqrt(1e-40)', () => {
        BigIntPrimitive.DP = 25; // sqrt is 1e-20
        const originalPE = BigIntPrimitive.PE;
        BigIntPrimitive.PE = 100;
        const expected = '0.00000000000000000001'; // 1e-20
        expect(new BigIntPrimitive('1e-40').sqrt().toString()).toBe(expected);
        BigIntPrimitive.PE = originalPE;
      });

      test('sqrt of a number that results in many zeros after decimal before significant digits', () => {
        BigIntPrimitive.DP = 25;
        // (0.0000000003)^2 = 9e-20
        const n = new BigIntPrimitive('9e-20'); // sqrt is 3e-10
        const originalPE = BigIntPrimitive.PE;
        BigIntPrimitive.PE = 100;
        expect(n.sqrt().toString()).toBe('0.00000000030000000000'); // String form with DP
        BigIntPrimitive.PE = originalPE;
      });
    });
});
...
describe('BigIntPrimitive.prototype.toString() scenarios', () => {
  ...
  describe('prec()', () => {
    ...
    describe('Argument Validation', () => {
      ...
      it('should throw for invalid significant digits (sd)', () => {
        expect(() => num.prec(0)).toThrow('[big.js] Invalid precision');
        expect(() => num.prec(1e6 + 1)).toThrow('[big.js] Invalid precision');
        expect(() => num.prec(1.5)).toThrow('[big.js] Invalid precision');
        expect(() => num.prec(NaN)).toThrow('[big.js] Invalid precision');
        expect(() => num.prec('abc')).toThrow('[big.js] Invalid precision');
      });
      it('should throw for invalid rounding mode (rm)', () => {
        expect(() => num.prec(5, -1)).toThrow('[big.js] Invalid rounding mode');
        expect(() => num.prec(5, 4)).toThrow('[big.js] Invalid rounding mode');
        expect(() => num.prec(5, 1.5)).toThrow('[big.js] Invalid rounding mode');
      });
    });

    describe('Basic Behavior', () => {
      it('x.prec() or x.prec(null/undefined) should return a new instance equal to x', () => {
        const x = new BigIntPrimitive('123.4567');
        const x_prec_undef = x.prec(undefined);
        expect(x_prec_undef.toString()).toBe('123.4567');
        expect(x_prec_undef).not.toBe(x);
        const x_prec_null = x.prec(null);
        expect(x_prec_null.toString()).toBe('123.4567');
        expect(x_prec_null).not.toBe(x);
      });
      it('new BigIntPrimitive("0").prec(N) should return "0"', () => {
        expect(new BigIntPrimitive('0').prec(5).toString()).toBe('0');
        expect(new BigIntPrimitive('0.000').prec(2).toString()).toBe('0');
      });
      it('should return a copy if current significant digits <= sd', () => {
        const n1 = new BigIntPrimitive('1.23'); // 3 SDs
        expect(n1.prec(3).toString()).toBe('1.23');
        expect(n1.prec(3)).not.toBe(n1);
        expect(n1.prec(5).toString()).toBe('1.23'); // 3 <= 5
        expect(n1.prec(5)).not.toBe(n1);

        const n2 = new BigIntPrimitive('12300'); // 3 SDs (123 * 10^2)
        expect(n2.prec(3).toString()).toBe('12300');
        expect(n2.prec(5).toString()).toBe('12300');
      });
    });

    describe('Rounding Examples (big.js compatibility)', () => {
      const x = new BigIntPrimitive('9876.54321');
      it('x.prec(2) -> "9900"', () => {
        expect(x.prec(2).toString()).toBe('9900');
      });
      it('x.prec(7) -> "9876.543"', () => {
        expect(x.prec(7).toString()).toBe('9876.543');
      });
      it('x.prec(20) -> "9876.54321"', () => {
        expect(x.prec(20).toString()).toBe('9876.54321');
      });
      it('x.prec(1, BigIntPrimitive.roundDown) -> "9000"', () => {
        expect(x.prec(1, BigIntPrimitive.roundDown).toString()).toBe('9000');
      });
      it('x.prec(1, BigIntPrimitive.roundHalfUp) -> "10000"', () => {
        expect(x.prec(1, BigIntPrimitive.roundHalfUp).toString()).toBe('10000');
      });

      it('new BigIntPrimitive("12345").prec(3) -> "12300"', () => {
        expect(new BigIntPrimitive('12345').prec(3).toString()).toBe('12300');
      });
      it('new BigIntPrimitive("12345").prec(3, BigIntPrimitive.roundDown) -> "12300"', () => {
        expect(new BigIntPrimitive('12345').prec(3, BigIntPrimitive.roundDown).toString()).toBe('12300');
      });
       it('new BigIntPrimitive("12399").prec(3, BigIntPrimitive.roundDown) -> "12300"', () => {
        expect(new BigIntPrimitive('12399').prec(3, BigIntPrimitive.roundDown).toString()).toBe('12300');
      });
       it('new BigIntPrimitive("12399").prec(3, BigIntPrimitive.roundHalfUp) -> "12400"', () => {
        expect(new BigIntPrimitive('12399').prec(3, BigIntPrimitive.roundHalfUp).toString()).toBe('12400');
      });

      it('new BigIntPrimitive("999").prec(1, BigIntPrimitive.roundHalfUp) -> "1000"', () => {
        expect(new BigIntPrimitive('999').prec(1, BigIntPrimitive.roundHalfUp).toString()).toBe('1000');
      });
      it('new BigIntPrimitive("999").prec(1, BigIntPrimitive.roundDown) -> "900"', () => {
        expect(new BigIntPrimitive('999').prec(1, BigIntPrimitive.roundDown).toString()).toBe('900');
      });
      it('new BigIntPrimitive("999").prec(2, BigIntPrimitive.roundHalfUp) -> "1000"', () => {
          expect(new BigIntPrimitive('999').prec(2, BigIntPrimitive.roundHalfUp).toString()).toBe('1000');
      });
      it('new BigIntPrimitive("999").prec(3, BigIntPrimitive.roundHalfUp) -> "999"', () => {
          expect(new BigIntPrimitive('999').prec(3, BigIntPrimitive.roundHalfUp).toString()).toBe('999');
      });

      it('new BigIntPrimitive("0.0012345").prec(3) -> "0.00123"', () => {
        expect(new BigIntPrimitive('0.0012345').prec(3).toString()).toBe('0.00123');
      });
      it('new BigIntPrimitive("0.0000099").prec(1, BigIntPrimitive.roundHalfUp) -> "0.00001"', () => {
        expect(new BigIntPrimitive('0.0000099').prec(1, BigIntPrimitive.roundHalfUp).toString()).toBe('0.00001');
      });

      it('new BigIntPrimitive("1.000").prec(1) -> "1"', () => {
          const n = new BigIntPrimitive("1.000");
          expect(n.prec(1).toString()).toBe('1');
      });
      it('new BigIntPrimitive("1.000").prec(4) -> "1"', () => {
          const n = new BigIntPrimitive("1.000");
          expect(n.prec(4).toString()).toBe('1');
      });
      it('new BigIntPrimitive("123.45000").round(5).prec(4) -> "123.5"', () => {
          const n = new BigIntPrimitive('123.45000').round(5);
          expect(n.toString()).toBe('123.45000');
          expect(n.prec(4).toString()).toBe('123.5');
      });
    });
  });
  ...
  describe('toPrecision()', () => {
    ...
    beforeEach(() => {
      ...
      originalPE = BigIntPrimitive.PE;
      originalNE = BigIntPrimitive.NE;
      // Default for tests, mirroring big.js
      BigIntPrimitive.RM = BigIntPrimitive.roundHalfUp;
    });
    ...
    describe('Argument Validation', () => {
      const num = new BigIntPrimitive('123.45');
      it('should throw for invalid significant digits (sd)', () => {
        expect(() => num.toPrecision(0)).toThrow('[big.js] Invalid precision');
        expect(() => num.toPrecision(1e6 + 1)).toThrow('[big.js] Invalid precision');
        expect(() => num.toPrecision(1.5)).toThrow('[big.js] Invalid precision');
        expect(() => num.toPrecision(NaN)).toThrow('[big.js] Invalid precision');
        expect(() => num.toPrecision(null)).toThrow('[big.js] Invalid precision');
        expect(() => num.toPrecision(undefined)).toThrow('[big.js] Invalid precision');
        expect(() => num.toPrecision('abc')).toThrow('[big.js] Invalid precision');
      });
      it('should throw for invalid rounding mode (rm)', () => {
        expect(() => num.toPrecision(5, -1)).toThrow('[big.js] Invalid rounding mode');
        expect(() => num.toPrecision(5, 4)).toThrow('[big.js] Invalid rounding mode');
        expect(() => num.toPrecision(5, 1.5)).toThrow('[big.js] Invalid rounding mode');
      });
    });

    describe('Zero Handling', () => {
      it('new BigIntPrimitive("0").toPrecision(N)', () => {
        expect(new BigIntPrimitive('0').toPrecision(1)).toBe('0');
        expect(new BigIntPrimitive('0').toPrecision(3)).toBe('0.00');
        expect(new BigIntPrimitive('0.000').toPrecision(4)).toBe('0.000');
      });
    });

    describe('Formatting Examples (MDN & big.js inspired)', () => {
      it('Fixed notation examples', () => {
        const n1 = new BigIntPrimitive('5.123456');
        expect(n1.toPrecision(5)).toBe('5.1235'); // Rounded
        expect(n1.toPrecision(2)).toBe('5.1');   // Rounded
        expect(n1.toPrecision(1)).toBe('5');     // Rounded

        const n2 = new BigIntPrimitive('0.000123');
        expect(n2.toPrecision(2)).toBe('0.00012'); // Does not go to sci notation yet
        expect(n2.toPrecision(1)).toBe('0.0001');  // Rounded

        const n3 = new BigIntPrimitive('123.45');
        expect(n3.toPrecision(3)).toBe('123'); // Rounded
        expect(n3.toPrecision(4)).toBe('123.5'); // Rounded
        expect(n3.toPrecision(5)).toBe('123.45');
        expect(n3.toPrecision(7)).toBe('123.4500'); // Padded
      });

      it('Scientific notation examples', () => {
        BigIntPrimitive.PE = 5; // To trigger sci notation earlier for some cases
        BigIntPrimitive.NE = -3;

        const n1 = new BigIntPrimitive('12345');
        expect(n1.toPrecision(2)).toBe('1.2e+4'); // 12345 rounded to 2 SD is 12000. 12000 orderMagExp = 4. sd=2. 4>=2.
        expect(n1.toPrecision(4)).toBe('1.235e+4');

        const n2 = new BigIntPrimitive('0.00012345'); // orderMagExp = -4
        expect(n2.toPrecision(2)).toBe('1.2e-4'); // 0.00012345 rounded to 2SD is 0.00012. orderMagExp=-4. sd=2. -4 < -6 is false. -4 >= 2 is false. Fixed?
                                                  // MDN: 0.00012345.toPrecision(2) is "0.00012"
                                                  // Big.js: new Big(0.00012345).toPrecision(2) is "1.2e-4"
                                                  // Our logic: roundedNumOrderMagExp for 0.00012 is -4. sd=2. -4 < -6 (false). -4 >= 2 (false). So fixed.
                                                  // dpForFixed = max(0, 2 - (-4+1)) = max(0, 2 - (-3)) = 5. toFixed(5) -> "0.00012"
                                                  // Ah, the condition `roundedNumOrderMagExp >= sd` forces sci for big.js
                                                  // Let's adjust the condition in the method to match big.js more closely for these cases.
                                                  // For now, this test will reflect current impl.

        const n3 = new BigIntPrimitive('1.23e5'); // Already in sci
        expect(n3.toPrecision(2)).toBe('1.2e+5');

        const n4 = new BigIntPrimitive('999'); // orderMagExp = 2
        expect(n4.toPrecision(1, BigIntPrimitive.roundHalfUp)).toBe('1e+3'); // 999 rounds to 1000. orderMagExp of 1000 is 3. sd=1. 3>=1.

        const n5 = new BigIntPrimitive('0.0000099'); // orderMagExp = -6
        expect(n5.toPrecision(1, BigIntPrimitive.roundHalfUp)).toBe('1.0e-5'); // rounds to 0.00001. orderMagExp = -5. sd=1. -5 < -6 false. -5 >= 1 false.
                                                                          // toFixed(dp = max(0, 1 - (-5+1)) = max(0, 1 - (-4)) = 5) -> "0.00001"
                                                                          // This one seems to fail the typical browser toPrecision logic for sci notation trigger.
                                                                          // big.js: new Big('0.0000099').toPrecision(1) is "1e-5"
      });
    });
  });
});
