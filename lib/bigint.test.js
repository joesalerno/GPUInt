...
describe('Strict Mode', () => {
  ...
  afterEach(() => {
    BigIntPrimitive.strict = originalStrict; // Reset to original value, not necessarily false
  });
  ...
  describe('Constructor', () => {
    ...
    it('should NOT throw when constructing with a BigIntPrimitive instance in strict mode', () => {
      const existingBigInt = new BigIntPrimitive('10'); // Create this non-strictly first
      BigIntPrimitive.strict = true; // Then enable strict mode for the test
    ...
    it('should allow number construction if strict mode is explicitly set to false locally', () => {
      ...
      expect(() => new BigIntPrimitive(123)).not.toThrow();
      BigIntPrimitive.strict = true; // Re-enable for subsequent tests in this block
    });
  });
  ...
  describe('toNumber()', () => {
    ...
    it('should convert "1.0000000000000001" (17 decimal places, safe) without error', () => {
      const val = '1.0000000000000001'; // Number(val) is 1.0000000000000001
      const n = new BigIntPrimitive(val);
      expect(n.toNumber()).toBe(Number(val));
    });

    it('should throw Error for "1.0000000000000000001" (20 decimal places, loses precision) in strict mode', () => {
      const val = '1.0000000000000000001'; // Number(val) is 1
      const n = new BigIntPrimitive(val);
    ...
    it('should throw Error for numbers that lose precision with Number() conversion', () => {
      const largePreciseVal = "9007199254740991.1"; // Number() is 9007199254740991
      ...
      const valTooLargeForExactNumber = "12345678901234567890123"; // Number(this) is 1.2345678901234568e+22
    ...
    it('should not throw for (Number.MAX_SAFE_INTEGER + 1).toString() if Number() is exact', () => {
      const val = (BigInt(Number.MAX_SAFE_INTEGER) + BigInt(1)).toString(); // "9007199254740992"
      const n = new BigIntPrimitive(val);
      // Number(val) is 9007199254740992, which is exact for JS Number
    ...
    it('should throw for "NaN" string if intermediate BigInt creation fails (strict mode)', () => {
        const n = new BigIntPrimitive("0"); // Valid BigInt, but we'll mock its toString
        n.toString = () => "NaN"; // Mock toString to simulate this scenario for toNumber
        // Number("NaN") is NaN. new BigIntPrimitive(NaN, ...) will throw during its own parsing.
        // This is caught by toNumber. Number.isNaN(primitiveNumber) is true, so it throws the specific NaN error.
    ...
    it('should NOT throw for toNumber() if strict mode is explicitly false, even if precision is lost', () => {
      BigIntPrimitive.strict = false;
      const val = '1.0000000000000000001'; // Loses precision
      const n = new BigIntPrimitive(val);
  ...
  describe('General Strict Mode Behavior', () => {
    it('BigIntPrimitive.strict = false; new BigIntPrimitive(1).toNumber() should work', () => {
      BigIntPrimitive.strict = false;
      const n = new BigIntPrimitive(1); // Allowed
      expect(n.toNumber()).toBe(1); // Allowed
    });
