...
describe('prec()', () => {
  ...
  const testCases = [
    ...
    // Zero handling
    // For new Big(0).prec(3).toString() === "0.00"
    // For new Big(0).prec(2).toString() === "0.0"
    // For new Big(0).prec(4).toString() === "0.000"
    // The `exp` here is the internal representation after toExponential, but the final toString is different.
    // The tests below will be adjusted to check the final string directly.
    { val: "0", sd: 3, exp: "0.00" }, // Adjusted expected string for direct comparison
    { val: "0.000", sd: 2, exp: "0.0" }, // Adjusted expected string
    { val: "-0", sd: 4, exp: "0.000" }, // Adjusted expected string
  ...
  testCases.forEach(tc => {
    it(`should correctly process prec(${tc.sd}, ${tc.rm !== undefined ? tc.rm : 'defaultRM'}) for "${tc.val}" to be like "${tc.exp}" (big.js string)`, () => {
      ...
      // The result of prec is a BigIntPrimitive.
      // For zero cases, tc.exp is the final expected string.
      // For non-zero, tc.exp is the exponential form, and we check its toString vs result.toString.
      if (tc.val === "0" || tc.val === "0.000" || tc.val === "-0") {
        expect(result.toString()).toBe(tc.exp);
      } else {
        const expectedNum = new BigIntPrimitive(tc.exp); // tc.exp is an exponential string for these
