...
describe('BigIntPrimitive', () => {
  ...
  describe('add() with WebGL mock', () => {
    ...
    it('should add two multi-limb numbers with carries', () => {
      ...
      // Inserted logging
      if (result) {
        const actualResultString = result.toString();
        const expectedResultString = '11111111110';
        console.log("Failing Test Debug: Actual String Length:", actualResultString.length);
        console.log("Failing Test Debug: Expected String Length:", expectedResultString.length);

        let actualHex = '';
        for (let i = 0; i < actualResultString.length; i++) actualHex += actualResultString.charCodeAt(i).toString(16).padStart(2, '0');
        console.log("Failing Test Debug: Actual String Hex-like:", actualHex);

        let expectedHex = '';
        for (let i = 0; i < expectedResultString.length; i++) expectedHex += expectedResultString.charCodeAt(i).toString(16).padStart(2, '0');
        console.log("Failing Test Debug: Expected String Hex-like:", expectedHex);

        console.log("Failing Test Debug: Actual Result Limbs:", JSON.stringify(result.limbs));
      } else {
        console.log("Failing Test Debug: Result is null or undefined.");
      }
    ...
    it('BUGFIX BROWSER ADD: 20000 + 5333 => 25333', () => {
      ...
      // Temporarily spy on readDataFromTexture to see what it would return if not mocked by a more specific test.
      const readDataSpy = vi.spyOn(webglUtils, 'readDataFromTexture');

      const num1 = new BigIntPrimitive(num1Str, mockCanvas);
      const num2 = new BigIntPrimitive(num2Str, mockCanvas);
      const result = num1.add(num2);

      // Log actual limbs for debugging if it fails
      if (result.toString() !== expectedSumStr) {
        console.log(`DEBUG ADD: Input ${num1Str} (${JSON.stringify(num1.limbs)}) + ${num2Str} (${JSON.stringify(num2.limbs)})`);
        console.log(`DEBUG ADD: Expected ${expectedSumStr}, Got ${result.toString()} (${JSON.stringify(result.limbs)})`);
      }

      expect(result.toString()).toBe(expectedSumStr);
      readDataSpy.mockRestore(); // Clean up spy
    });
  });
