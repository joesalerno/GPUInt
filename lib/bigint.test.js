...
vi.mock('./webgl-utils', () => {
  // Helper to create a more complete mock GL context
  const createMockGLContext = () => ({
    createShader: vi.fn().mockImplementation(type => ({ type, id: 'mockGLShaderObject-' + Math.random() })),
    shaderSource: vi.fn(), // *** ADDED ***
    compileShader: vi.fn(),
    getShaderParameter: vi.fn((shader, pname) => {
      if (pname === 0x8B81 /* COMPILE_STATUS */) return true; // Assume compile success
      return undefined;
    }),
    getShaderInfoLog: vi.fn().mockReturnValue('mock shader info log'),
    deleteShader: vi.fn(),
    createProgram: vi.fn().mockImplementation(() => ({ id: 'mockGLProgramObject-' + Math.random() })),
    attachShader: vi.fn(),
    linkProgram: vi.fn(),
    getProgramParameter: vi.fn((program, pname) => {
      if (pname === 0x8B82 /* LINK_STATUS */) return true; // Assume link success
      return undefined;
    }),
    getProgramInfoLog: vi.fn().mockReturnValue('mock program info log'),
    deleteProgram: vi.fn(),
    getAttribLocation: vi.fn().mockReturnValue(0), // Default valid location
    getUniformLocation: vi.fn().mockReturnValue(0), // Default valid location
    ...
    uniform1i: vi.fn(),
    uniform1f: vi.fn(),
    createFramebuffer: vi.fn().mockReturnValue({ id: 'mockGLFramebufferObject' }),
    bindFramebuffer: vi.fn(),
    framebufferTexture2D: vi.fn(),
    checkFramebufferStatus: vi.fn().mockReturnValue(36053), // FRAMEBUFFER_COMPLETE
    createBuffer: vi.fn().mockReturnValue({ id: 'mockGLBufferObject' }),
    bindBuffer: vi.fn(),
    bufferData: vi.fn(),
    viewport: vi.fn(),
    useProgram: vi.fn(),
    ...
    texParameteri: vi.fn(),
    texImage2D: vi.fn(),
    readPixels: vi.fn(), // Important for readDataFromTexture
    getExtension: vi.fn(extensionName => { // Handle OES_texture_float and OES_vertex_array_object
      if (extensionName === 'OES_texture_float' || extensionName === 'OES_vertex_array_object') return {}; // Return a truthy object
      return null;
    }),
    createVertexArray: vi.fn().mockReturnValue({ id: 'mockVAO' }), // For OES_vertex_array_object
    bindVertexArray: vi.fn(),
    deleteVertexArray: vi.fn(),
    // GL Constants (ensure these are available if webgl-utils or bigint.js uses them on gl directly)
    ...
    TRIANGLES: 0x0004,
    STATIC_DRAW: 0x88E4,
    // Add TEXTURE0, TEXTURE1 etc. if needed by the actual code for u_sampler uniforms
    TEXTURE0: 0x84C0,
    TEXTURE1: 0x84C1,
    TEXTURE2: 0x84C2,
  });
  ...
  return {
    initWebGL: vi.fn(canvas => canvas ? createMockGLContext() : null),
    createShader: vi.fn((gl, type, source) => {
      const shader = gl.createShader(type);
      gl.shaderSource(shader, source);
      gl.compileShader(shader);
      if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
        console.error("Mocked webglUtils.createShader: Compile failed - Info: " + gl.getShaderInfoLog(shader));
        gl.deleteShader(shader);
        return null;
      }
      return shader;
    }),
    createProgram: vi.fn((gl, vertexShader, fragmentShader) => {
      const program = gl.createProgram();
      gl.attachShader(program, vertexShader);
      gl.attachShader(program, fragmentShader);
      gl.linkProgram(program);
      if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
        console.error("Mocked webglUtils.createProgram: Link failed - Info: " + gl.getProgramInfoLog(program));
        gl.deleteProgram(program);
        return null;
      }
      return program;
    }),
    createDataTexture: vi.fn().mockImplementation((gl, dataArray, width, height, useRGBA = false) => {
      if (!gl.getExtension('OES_texture_float')) return null; // Simulate check
      return { id: 'mockTextureData', width, height, useRGBA };
    }),
    readDataFromTexture: vi.fn().mockImplementation((gl, framebuffer, width, height, extractSingleComponent = true) => {
      // Default mock for readDataFromTexture. Tests for specific operations like 'add'
      // should mock this further to return calculated results.
      const numComponents = extractSingleComponent ? 1 : 4;
      const dataSize = width * height * numComponents;
      // For add tests to pass, this needs to return sum & carry in .r & .g
      // If extractSingleComponent is false, it means RGBA is expected.
      // The 'add' operation in bigint.js reads RGBA and then extracts R and G.
      // So this mock should return an RGBA array.
      const returnData = new Float32Array(width * height * 4);
      // Example: fill with some patterned data or zeros
      // This default might not be what specific tests need but prevents errors.
      return returnData;
    }),
  };
});
