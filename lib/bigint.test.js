...
vi.mock('./webgl-utils', () => ({
  initWebGL: vi.fn((canvas) => {
    ...
    // Return null to simulate WebGL failure for most tests
    return null;
  }),
  ...
  getFramebufferStatusString: vi.fn((gl, status) => `Mock framebuffer status: ${status}`),
}));

describe('BigIntPrimitive', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    // Default: initWebGL returns null to simulate WebGL failure
  ...
  describe('pow()', () => {
    it('should calculate powers correctly for positive integer exponents', () => {
      expect(new BigIntPrimitive('2').pow(3).toString()).toBe('8');
      expect(new BigIntPrimitive('3').pow(4).toString()).toBe('81');
      expect(new BigIntPrimitive('10').pow(2).toString()).toBe('100');
      expect(new BigIntPrimitive('1').pow(100).toString()).toBe('1');
      const largeBase = new BigIntPrimitive('10000'); // Exactly BASE
      const largePower = largeBase.pow(2); // 10000^2 = 100,000,000
      expect(largePower.toString()).toBe('100000000');
    });

    it('should handle exponent of 0 and 1 correctly', () => {
      expect(new BigIntPrimitive('123').pow(0).toString()).toBe('1');
      expect(new BigIntPrimitive('123').pow(1).toString()).toBe('123');
      expect(new BigIntPrimitive('-5').pow(0).toString()).toBe('1');
      expect(new BigIntPrimitive('-5').pow(1).toString()).toBe('-5');
    });

    it('should handle base of 0 correctly', () => {
      expect(new BigIntPrimitive('0').pow(0).toString()).toBe('1'); // 0^0 is 1 by convention in many libraries
      expect(new BigIntPrimitive('0').pow(100).toString()).toBe('0');
    });

    it('should handle negative exponents correctly', () => {
      // Set a specific DP for predictable division results in these tests
      const originalDP = BigIntPrimitive.DP;
      BigIntPrimitive.DP = 4; // e.g. for 1/4 = 0.2500, 1/100 = 0.0100

      expect(new BigIntPrimitive('2').pow(-2).toString()).toBe('0.25');
      expect(new BigIntPrimitive('4').pow(-1).toString()).toBe('0.25');
      expect(new BigIntPrimitive('10').pow(-1).toString()).toBe('0.1');
      expect(new BigIntPrimitive('10').pow(-2).toString()).toBe('0.01');

      // Test division by zero for 0.pow(-x)
      // 0.pow(-2) -> 1 / (0.pow(2)) -> 1 / 0 -> Division by zero
      expect(() => new BigIntPrimitive('0').pow(-2)).toThrow("Division by zero");

      BigIntPrimitive.DP = originalDP; // Restore original DP
    });

    it('should handle base of 1 and -1 correctly', () => {
      expect(new BigIntPrimitive('1').pow(0).toString()).toBe('1');
      expect(new BigIntPrimitive('1').pow(5).toString()).toBe('1');
      expect(new BigIntPrimitive('-1').pow(0).toString()).toBe('1');
      expect(new BigIntPrimitive('-1').pow(2).toString()).toBe('1');  // (-1)^2 = 1
      expect(new BigIntPrimitive('-1').pow(3).toString()).toBe('-1'); // (-1)^3 = -1
    });

    it('should throw error for non-integer exponent', () => {
      expect(() => new BigIntPrimitive('2').pow(1.5)).toThrow('Exponent must be an integer.');
    });

    it('should throw error for very large positive exponent', () => {
      expect(() => new BigIntPrimitive('2').pow(1000001)).toThrow('Exponent too large.');
    });

    it('should return a new BigIntPrimitive instance', () => {
      const num = new BigIntPrimitive('3');
      const result = num.pow(2);
      expect(result).toBeInstanceOf(BigIntPrimitive);
      expect(result).not.toBe(num);
    });
  });
  ...
  describe('_core_multiply() - internal multiplication logic', () => {
    ...
    testCases.forEach(tc => {
      it(`should correctly multiply ${tc.aStr} and ${tc.bStr} to get ${tc.expectedStr} (${tc.note})`, () => {
        ...
        const result = instance._core_multiply_cpu(numA, numB);
...
describe('BigIntPrimitive.DP', () => {
  ...
  it('div() method should respect BigIntPrimitive.DP', () => {
    BigIntPrimitive.DP = 5;
    expect(new BigIntPrimitive('1').divide(new BigIntPrimitive('3')).toString()).toBe('0.33333');

    BigIntPrimitive.DP = 2;
    expect(new BigIntPrimitive('1').divide(new BigIntPrimitive('3')).toString()).toBe('0.33');
  });

  it('sqrt() method should respect BigIntPrimitive.DP', () => {
    BigIntPrimitive.DP = 5;
    expect(new BigIntPrimitive('2').sqrt().toString()).toBe('1.41421'); // sqrt(2) to 5 DP

    BigIntPrimitive.DP = 2;
    expect(new BigIntPrimitive('2').sqrt().toString()).toBe('1.41');    // sqrt(2) to 2 DP
  });

  it('pow() method with negative exponent should respect BigIntPrimitive.DP', () => {
    BigIntPrimitive.DP = 5;
    expect(new BigIntPrimitive('3').pow(-2).toString()).toBe('0.11111'); // 1/9 to 5 DP

    BigIntPrimitive.DP = 2;
    expect(new BigIntPrimitive('3').pow(-2).toString()).toBe('0.11');    // 1/9 to 2 DP
  });
});

describe('BigIntPrimitive.RM', () => {
  let originalRM;
  ...
  it('round() method should respect BigIntPrimitive.RM when no rm argument is passed', () => {
    BigIntPrimitive.RM = BigIntPrimitive.roundHalfUp;
    expect(new BigIntPrimitive('1.25').round(1).toString()).toBe('1.3');
    expect(new BigIntPrimitive('1.24').round(1).toString()).toBe('1.2');

    BigIntPrimitive.RM = BigIntPrimitive.roundHalfEven;
    expect(new BigIntPrimitive('1.25').round(1).toString()).toBe('1.2');
    expect(new BigIntPrimitive('1.35').round(1).toString()).toBe('1.4');

    BigIntPrimitive.RM = BigIntPrimitive.roundDown;
    expect(new BigIntPrimitive('1.29').round(1).toString()).toBe('1.2');

    BigIntPrimitive.RM = BigIntPrimitive.roundUp;
    expect(new BigIntPrimitive('1.21').round(1).toString()).toBe('1.3');
    expect(new BigIntPrimitive('-1.21').round(1).toString()).toBe('-1.2'); // roundUp towards zero for negative
  });

  it('div() method should use BigIntPrimitive.RM by default', () => {
    BigIntPrimitive.DP = 2;
    // 2/3 = 0.666...
    BigIntPrimitive.RM = BigIntPrimitive.roundHalfUp;
    expect(new BigIntPrimitive('2').divide(new BigIntPrimitive('3')).toString()).toBe('0.67');

    BigIntPrimitive.RM = BigIntPrimitive.roundDown;
    expect(new BigIntPrimitive('2').divide(new BigIntPrimitive('3')).toString()).toBe('0.66');
  });

  it('toExponential() method should use BigIntPrimitive.RM by default', () => {
    // 45.6, dp=0. RM_HALF_UP -> 5e+1. RM_DOWN -> 4e+1
    BigIntPrimitive.RM = BigIntPrimitive.roundHalfUp;
    expect(new BigIntPrimitive('45.6').toExponential(0)).toBe('5e+1');

    BigIntPrimitive.RM = BigIntPrimitive.roundDown;
    expect(new BigIntPrimitive('45.6').toExponential(0)).toBe('4e+1');
  });

  it('toFixed() method should use BigIntPrimitive.RM by default', () => {
    // 1.25, dp=1. RM_HALF_UP -> 1.3. RM_DOWN -> 1.2
    BigIntPrimitive.RM = BigIntPrimitive.roundHalfUp;
    expect(new BigIntPrimitive('1.25').toFixed(1)).toBe('1.3');

    BigIntPrimitive.RM = BigIntPrimitive.roundDown;
    expect(new BigIntPrimitive('1.25').toFixed(1)).toBe('1.2');
  });

  it('toPrecision() method should use BigIntPrimitive.RM by default', () => {
    // 12.345, sd=4. sciExp=1. dpFixed = 4-(1+1)=2
    // prec(4) on 12.345 uses toExponential(3) -> 1.2345e+1 (val 12.345)
    // toFixed(dpFixed=2) is called.
    // RM_HALF_UP for 12.345 to 2dp -> 12.35
    // RM_DOWN for 12.345 to 2dp -> 12.34
    BigIntPrimitive.RM = BigIntPrimitive.roundHalfUp;
    expect(new BigIntPrimitive('12.345').toPrecision(4)).toBe('12.35');

    BigIntPrimitive.RM = BigIntPrimitive.roundDown;
    expect(new BigIntPrimitive('12.345').toPrecision(4)).toBe('12.34');
  });
});

describe('BigIntPrimitive.NE and BigIntPrimitive.PE', () => {
  let originalNE, originalPE;
  ...
  it('toString() should respect BigIntPrimitive.NE', () => {
    BigIntPrimitive.NE = -3;
    BigIntPrimitive.PE = 21; // Default PE

    expect(new BigIntPrimitive('0.001').toString()).toBe('0.001'); // sciExp -3. -3 < -3 is false. Not exp.
    expect(new BigIntPrimitive('0.0001').toString()).toBe('1e-4');  // sciExp -4. -4 < -3 is true. Exp.

    BigIntPrimitive.NE = -2;
    expect(new BigIntPrimitive('0.001').toString()).toBe('1e-3');   // sciExp -3. -3 < -2 is true. Exp.
    expect(new BigIntPrimitive('0.01').toString()).toBe('0.01');    // sciExp -2. -2 < -2 is false. Not exp.
  });

  it('toString() should respect BigIntPrimitive.PE', () => {
    BigIntPrimitive.NE = -7; // Default NE
    BigIntPrimitive.PE = 3;

    expect(new BigIntPrimitive('1000').toString()).toBe('1e+3');   // sciExp 3. 3 >= 3 is true. Exp.
    expect(new BigIntPrimitive('100').toString()).toBe('100');     // sciExp 2. 2 >= 3 is false. Not exp.

    BigIntPrimitive.PE = 2;
    expect(new BigIntPrimitive('100').toString()).toBe('1e+2');    // sciExp 2. 2 >= 2 is true. Exp.
    expect(new BigIntPrimitive('10').toString()).toBe('10');       // sciExp 1. 1 >= 2 is false. Not exp.
  });
});

describe('BigIntPrimitive.strict', () => {
  let originalStrict;
  ...
  it('constructor should throw if strict is true and input is a number', () => {
    BigIntPrimitive.strict = true;
    expect(() => new BigIntPrimitive(123)).toThrow(TypeError);
    expect(() => new BigIntPrimitive(123)).toThrow('[big.js] String expected');

    BigIntPrimitive.strict = false;
    expect(() => new BigIntPrimitive(123)).not.toThrow();
  });

  it('toNumber() should throw for precision loss if strict is true', () => {
    const numStr = "123456789012345678901"; // Number that loses precision
    const bigNum = new BigIntPrimitive(numStr);

    BigIntPrimitive.strict = true;
    expect(() => bigNum.toNumber()).toThrow(TypeError);
    expect(() => bigNum.toNumber()).toThrow("[big.js] Imprecise conversion: precision loss");

    BigIntPrimitive.strict = false;
    expect(() => bigNum.toNumber()).not.toThrow();
  });

  it('toNumber() should throw for non-finite numbers if strict is true', () => {
    const infStr = "1e500";
    const bigNumInf = new BigIntPrimitive(infStr);

    BigIntPrimitive.strict = true;
    expect(() => bigNumInf.toNumber()).toThrow(TypeError);
    expect(() => bigNumInf.toNumber()).toThrow("[big.js] Imprecise conversion: non-finite number");

    BigIntPrimitive.strict = false;
    expect(bigNumInf.toNumber()).toBe(Infinity);
  });
});
...
describe('prec()', () => {
  ...
  // Test cases: [initialValue, sd, rm (optional), expectedStringAfterPrecThenToString]
    ...
    // With prec not setting _roundedDp, toString will use default NE/PE for formatting.
    // Default NE = -7, PE = 21.
    { val: "123.456", sd: 7, exp: "123.456" },    // Value 123.4560 -> compact "123.456"
    { val: "123.456", sd: 5, exp: "123.46" },    // Value 123.46
    { val: "123.456", sd: 3, exp: "123" },       // Value 123
    { val: "123.456", sd: 2, exp: "120" },       // Value 120
    { val: "123.456", sd: 1, exp: "100" },       // Value 100

    { val: "0.0012345", sd: 4, exp: "0.001235" }, // Value 0.0012345 -> 0.001235
    { val: "0.0012345", sd: 3, exp: "0.00123" },  // Value 0.00123
    { val: "0.0012345", sd: 1, exp: "0.001" },    // Value 0.001

    // Rounding modes
    { val: "999.9", sd: 1, rm: roundHalfUp, exp: "1000" },      // Value 1000
    { val: "999.9", sd: 4, rm: roundHalfUp, exp: "999.9" },   // Value 999.9
    { val: "999.9", sd: 5, rm: roundHalfUp, exp: "999.9" }, // Value 999.90 -> compact "999.9"

    { val: "-123.45", sd: 3, rm: roundHalfUp, exp: "-123" },
    { val: "-123.45", sd: 2, rm: roundHalfUp, exp: "-120" },

    { val: "5.5", sd: 1, rm: roundHalfEven, exp: "6" },
    { val: "6.5", sd: 1, rm: roundHalfEven, exp: "6" },
    { val: "2.5", sd: 1, rm: roundHalfUp, exp: "3" },
    { val: "2.5", sd: 1, rm: roundDown, exp: "2" },

    // Zero handling (these are correct as tc.exp is the final string, and prec handles _roundedDp for zero)
    { val: "0", sd: 3, exp: "0.00" },
    { val: "0.000", sd: 2, exp: "0.0" },
    { val: "-0", sd: 4, exp: "0.000" },

    // More complex cases
    { val: "123456789.123456789", sd: 10, exp: "123456789.1" },
    { val: "123456789.123456789", sd: 5, exp: "123460000" },
    { val: "0.999999", sd: 2, exp: "1" }, // Value 1.0 -> compact "1"
    { val: "0.999999", sd: 1, exp: "1" },
    { val: "9.99e+10", sd: 2, exp: "100000000000" }, // Value 1e11, PE=21, so fixed.
    { val: "1", sd: 3, exp: "1" }, // Value 1.00 -> compact "1"
    { val: "123", sd: 1, exp: "100"},
    { val: "123", sd: 2, exp: "120"},
    { val: "123", sd: 3, exp: "123"},
    { val: "123", sd: 4, exp: "123"}, // Value 123.0 -> compact "123"
  ];
  ...
  testCases.forEach(tc => {
    it(`should correctly process prec(${tc.sd}, ${tc.rm !== undefined ? tc.rm : 'defaultRM'}) for "${tc.val}" to be like "${tc.exp}" (big.js string)`, () => {
      ...
      expect(result.toString()).toBe(tc.exp);
    });
  });
  ...
  const bigJsPrecToStringCases = [
    ...
    { val: "123.456", sd: 7, rm: roundHalfUp, expStr: "123.456" }, // Updated from "123.4560"
    ...
    { val: "999.9", sd: 5, rm: roundHalfUp, expStr: "999.9" },// Updated from "999.90"
    { val: "0", sd: 3, expStr: "0.00" }, // Updated from "0.000"
    { val: "1", sd: 3, expStr: "1" }, // Updated from "1.00"
  ];
  ...
  it('toString of prec result: 123.456, prec(7) -> "123.4560" (big.js like)', () => {
    const fromPrec = new BigIntPrimitive("123.456").prec(7);
    BigIntPrimitive.NE = -10; BigIntPrimitive.PE = 30; // Force fixed notation for this value
    // After prec(7), value is 123.4560. toString() with no _roundedDp (as per current prec) should be "123.456"
    expect(fromPrec.toString()).toBe("123.456");
  });
...
describe('BigIntPrimitive.divide() - big.js conformance', () => {
  let originalDP;
  let originalRM;
  ...
  test('should replicate big.js example: 355 / 113 with default DP (20)', () => {
    BigIntPrimitive.DP = 20; // Default big.js DP
    BigIntPrimitive.RM = 1; // Default big.js RM (roundHalfUp)
    const x = new BigIntPrimitive('355');
    const y = new BigIntPrimitive('113');
    // Expected value from big.js: '3.14159292035398230088'
    // Our library might produce more digits due to internalPrecision, then round.
    // The key is that after rounding to DP=20, it should match.
    // Since toString() now doesn't pad unnecessarily, we expect the natural string form.
    // Let's check the string representation after explicit rounding for comparison if direct toString differs.
    const result = x.divide(y);
    // If BigIntPrimitive.DP is 20, result.round(20, BigIntPrimitive.RM).toString() should be this.
    // And result.toString() itself should be this if no non-significant zeros are present.
    expect(result.toString()).toBe('3.14159292035398230088');
  });

  test('should replicate big.js example: 355 / 113 with DP = 2', () => {
    BigIntPrimitive.DP = 2;
    BigIntPrimitive.RM = 1; // roundHalfUp
    const x = new BigIntPrimitive('355');
    const y = new BigIntPrimitive('113');
    // 355/113 = 3.14159...
    // Rounded to 2 DP (roundHalfUp): 3.14
    expect(x.divide(y).toString()).toBe('3.14');
  });

  test('should replicate big.js example: 355 / 113 with DP = 2 and RM = roundDown (0)', () => {
    BigIntPrimitive.DP = 2;
    BigIntPrimitive.RM = BigIntPrimitive.roundDown; // 0
    const x = new BigIntPrimitive('355');
    const y = new BigIntPrimitive('113');
    // 355/113 = 3.14159...
    // Rounded to 2 DP (roundDown): 3.14
    expect(x.divide(y).toString()).toBe('3.14');
  });

  test('should replicate big.js example: 1 / 3 with DP = 2 and RM = roundHalfUp (1)', () => {
    BigIntPrimitive.DP = 2;
    BigIntPrimitive.RM = BigIntPrimitive.roundHalfUp; // 1
    const x = new BigIntPrimitive('1');
    const y = new BigIntPrimitive('3');
    // 1/3 = 0.3333...
    // Rounded to 2 DP (roundHalfUp): 0.33
    expect(x.divide(y).toString()).toBe('0.33');
  });

  test('should replicate big.js example: 2 / 3 with DP = 2 and RM = roundHalfUp (1)', () => {
    BigIntPrimitive.DP = 2;
    BigIntPrimitive.RM = BigIntPrimitive.roundHalfUp; // 1
    const x = new BigIntPrimitive('2');
    const y = new BigIntPrimitive('3');
    // 2/3 = 0.6666...
    // Rounded to 2 DP (roundHalfUp): 0.67
    expect(x.divide(y).toString()).toBe('0.67');
  });


  test('should replicate big.js example: x.div(5) where result is integer', () => {
    BigIntPrimitive.DP = 2; // DP setting shouldn't affect integer result string form
    BigIntPrimitive.RM = 1;
    const x = new BigIntPrimitive('355');
    const y = new BigIntPrimitive('5');
    expect(x.divide(y).toString()).toBe('71');
  });

  test('division by integer, non-terminating, DP=5', () => {
    BigIntPrimitive.DP = 5;
    BigIntPrimitive.RM = BigIntPrimitive.roundHalfUp; // roundHalfUp
    const x = new BigIntPrimitive('10');
    const y = new BigIntPrimitive('3');
    // 10/3 = 3.33333...
    expect(x.divide(y).toString()).toBe('3.33333');
  });

  test('division by integer, terminating, less than DP', () => {
    BigIntPrimitive.DP = 5;
    BigIntPrimitive.RM = BigIntPrimitive.roundHalfUp;
    const x = new BigIntPrimitive('1');
    const y = new BigIntPrimitive('8');
    // 1/8 = 0.125
    expect(x.divide(y).toString()).toBe('0.125');
  });

  test('division resulting in many zeros before rounding point, DP=2', () => {
    BigIntPrimitive.DP = 2;
    BigIntPrimitive.RM = BigIntPrimitive.roundHalfUp;
    const x = new BigIntPrimitive('1');
    const y = new BigIntPrimitive('800');
    // 1/800 = 0.00125
    // Rounded to 2 DP (roundHalfUp): 0.00
    expect(x.divide(y).toString()).toBe('0.00');
  });

   test('division of 5 by 2, DP=0', () => {
    BigIntPrimitive.DP = 0;
    BigIntPrimitive.RM = BigIntPrimitive.roundHalfUp; // roundHalfUp
    const x = new BigIntPrimitive('5');
    const y = new BigIntPrimitive('2');
    // 5/2 = 2.5. Rounded to 0 DP (roundHalfUp) is 3
    expect(x.divide(y).toString()).toBe('3');
  });

  test('division of 5 by 2, DP=1', () => {
    BigIntPrimitive.DP = 1;
    BigIntPrimitive.RM = BigIntPrimitive.roundHalfUp; // roundHalfUp
    const x = new BigIntPrimitive('5');
    const y = new BigIntPrimitive('2');
    // 5/2 = 2.5. Rounded to 1 DP is 2.5
    expect(x.divide(y).toString()).toBe('2.5');
  });
});
