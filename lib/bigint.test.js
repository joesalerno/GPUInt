...
describe('BigIntPrimitive', () => {
  ...
  describe('divide() - public method', () => {
    ...
    function expectDivision(dividendStr, divisorStr, expectedQStr, expectedRStr) {
      ...
      // Spy on helpers to ensure they are called as expected for some cases,
      // but mostly treat `divide` as a black box for result correctness.
      const coreDivideSpy = vi.spyOn(dividend, '_core_divide');
      const estimateSpy = vi.spyOn(dividend, '_estimate_quotient_limb');
      const mlbbSpy = vi.spyOn(dividend, '_multiply_limb_by_bigint');
