...
describe('BigIntPrimitive', () => {
  ...
  describe('multiply() - public method with Karatsuba and sign logic', () => {
    ...
    it('Karatsuba integration: large numbers with forceCPU', () => {
      ...
      // if the full large number calculation within the test is too complex to setup right now.
      // Let's use a known small multiplication that would exceed a small KARATSUBA_THRESHOLD if it was set low.
      // For now, this test will focus on ensuring no WebGL calls for a standard multiplication.
      // A more robust Karatsuba CPU test would require a specific threshold setting or very large numbers
      // and a way to confirm the Karatsuba algorithm's recursive CPU calls.

      const num1Str = "1".repeat(80); // 20 limbs
      const num2Str = "2".repeat(80); // 20 limbs
      const expectedResultStr = BigInt(num1Str) * BigInt(num2Str); // Use BigInt for expected

      const n1 = new BigIntPrimitive(num1Str, mockCanvas, { forceCPU: true });
      const n2 = new BigIntPrimitive(num2Str, mockCanvas, { forceCPU: true });

      const initWebGLSpy = vi.spyOn(webglUtils, 'initWebGL');
      // Spy on _multiply_limb_by_bigint to ensure it's being called (and thus its CPU path is tested)
      const multiplyLimbSpy = vi.spyOn(BigIntPrimitive.prototype, '_multiply_limb_by_bigint');

      const result = n1.multiply(n2);

      expect(result.toString()).toBe(String(expectedResultStr));
      expect(initWebGLSpy).not.toHaveBeenCalled();
      expect(multiplyLimbSpy).toHaveBeenCalled(); // Confirms _core_multiply path was taken

      initWebGLSpy.mockRestore();
      multiplyLimbSpy.mockRestore();
    });
