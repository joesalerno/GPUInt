...
describe('divide()', () => {
  ...
  test('DEBUG TEST: 10 / 4 (DP=1) _decimalDivide raw output', () => {
    const num1 = new BigIntPrimitive('10'); // Dividend
    const num2 = new BigIntPrimitive('4');  // Divisor
    const originalDP = BigIntPrimitive.DP;
    BigIntPrimitive.DP = 1; // Context for test name, _decimalDivide uses passed precision directly

    const internalPrecision = 6;

    // Call: instanceNum1._decimalDivide(instanceNum2, precisionForScaling)
    const res_dd = num1.abs()._decimalDivide(num2.abs(), internalPrecision);

    // Expected state for 2.500000 (from 10 * 10^6 / 4 = 2,500,000 * 10^-6)
    // Coefficient "2500000". With BASE_LOG10=4, limbs are [250, 0]
    const expectedLimbArrayString = JSON.stringify([250, 0]);
    const expectedExponentValue = -6;
    // Standard toString() on {limbs:[250,0], exponent:-6} which is 2.500000 will produce "2.5"
    const expectedToStringValue = "2.5";

    const actualLimbArrayString = JSON.stringify(res_dd.limbs);
    const actualExponentValue = res_dd.exponent;

    const oldPE = BigIntPrimitive.PE; BigIntPrimitive.PE = 1e9; // Use large PE/NE for consistent toString
    const oldNE = BigIntPrimitive.NE; BigIntPrimitive.NE = -1e9;
    const actualToStringValue = res_dd.toString();
    BigIntPrimitive.PE = oldPE; BigIntPrimitive.NE = oldNE;

    // Custom throw for more detailed debugging info if needed
    if (actualToStringValue !== expectedToStringValue ||
        actualLimbArrayString !== expectedLimbArrayString ||
        actualExponentValue !== expectedExponentValue) {
      throw new Error(
        `DEBUG TEST FAILED for 10/4 (internalPrecision=${internalPrecision})\n` +
        `  _decimalDivide raw output assertions:\n` +
        `    toString(): "${actualToStringValue}" (expected "${expectedToStringValue}")\n` +
        `    limbs: ${actualLimbArrayString} (expected ${expectedLimbArrayString})\n` +
        `    exponent: ${actualExponentValue} (expected ${expectedExponentValue})\n` +
        `    (Sign was ${res_dd.sign}, isZero=${res_dd.isZero()})`
      );
    }

    expect(actualLimbArrayString).toBe(expectedLimbArrayString);
    expect(actualExponentValue).toBe(expectedExponentValue);
    expect(actualToStringValue).toBe(expectedToStringValue);
