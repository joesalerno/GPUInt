...
describe('BigIntPrimitive', () => {
  ...
  describe('_multiply_limb_by_bigint() with WebGL mock', () => {
    ...
    it('limbValue * single-limb otherNumber, no final carry', () => {
      setupMockWebGLForMulLimb();
      const otherNumber = new BigIntPrimitive('1000', mockCanvas); // limbs: [1,0,0,0], texWidth=4
      vi.spyOn(webglUtils, 'readDataFromTexture').mockImplementation((gl,fbo,w,h,isOutput)=>{
        const o=new Float32Array(w*4); // w = texWidth which is 4 for '1000'
        if (w === 4) { // Check if texWidth is as expected
          // otherNumber.limbs[0]=1 (MSB), limbVal=5. Shader computes: prod=5*1=5. resultLimb=5, carryOut=0.
          o[0*4+0]=5; o[0*4+1]=0; // Mock for otherNumber.limbs[0]
          // otherNumber.limbs[1]=0, limbVal=5. Shader computes: prod=5*0=0. resultLimb=0, carryOut=0.
          o[1*4+0]=0; o[1*4+1]=0; // Mock for otherNumber.limbs[1]
          // otherNumber.limbs[2]=0, limbVal=5. Shader computes: prod=5*0=0. resultLimb=0, carryOut=0.
          o[2*4+0]=0; o[2*4+1]=0; // Mock for otherNumber.limbs[2]
          // otherNumber.limbs[3]=0 (LSB), limbVal=5. Shader computes: prod=5*0=0. resultLimb=0, carryOut=0.
          o[3*4+0]=0; o[3*4+1]=0; // Mock for otherNumber.limbs[3]
        }
        return o;
      });
...
describe('sqrt()', () => {
  ...
  it('sqrt() result should not have _roundedDp property if result is integer and DP was 0 (big.js specific behavior for toString)', () => {
    ...
    expect(res2.toString()).toBe('2.5'); // big.js would show 2.5
