...
describe('BigIntPrimitive', () => {
  ...
  describe('pow()', () => {
    ...
    describe('Input Validation', () => {
      ...
      // Test for positive exponent too large (restored)
      it('should throw Error for too large positive exponent', () => {
      ...
      it('should throw Error for exponent too large', () => {
        expect(() => base.pow(1000001)).toThrow(Error("Exponent magnitude too large."));
      });
    });
  });
...
describe('sqrt()', () => {
  ...
  describe('Precision and Rounding (BigIntPrimitive.DP and BigIntPrimitive.RM)', () => {
    ...
    it('sqrt(very large perfect square)', () => {
      BigIntPrimitive.DP = 0;
      const largeSquare = '15241578750190521';
  ...
  it('sqrt() result formatting adheres to big.js toString() rules', () => {
    BigIntPrimitive.DP = 5;
    const resInt = new BigIntPrimitive('25').sqrt();
    expect(resInt.toString()).toBe('5');
    expect(resInt.hasOwnProperty('_roundedDp')).toBe(false);

    const resExactDec = new BigIntPrimitive('6.25').sqrt();
    expect(resExactDec.toString()).toBe('2.5');
    expect(resExactDec._roundedDp).toBe(1);

    const resInexact = new BigIntPrimitive('2').sqrt();
    expect(resInexact.toString()).toBe('1.41421');
    expect(resInexact._roundedDp).toBe(5);

    BigIntPrimitive.DP = 2;
    const res2 = new BigIntPrimitive('6.25').sqrt();
    expect(res2.toString()).toBe('2.5'); // Changed from '2.50'
    expect(res2._roundedDp).toBe(1);

    BigIntPrimitive.DP = 0;
    const resIntDP0 = new BigIntPrimitive('49').sqrt();
    expect(resIntDP0.toString()).toBe('7');
    expect(resIntDP0.hasOwnProperty('_roundedDp')).toBe(false);
  });
});
...
describe('pow() with negative exponents', () => {
  ...
  it('2.pow(-2) should result in 0.25000 (DP=5)', () => {
    BigIntPrimitive.DP = 5;
    BigIntPrimitive.RM = 1;
    const num = new BigIntPrimitive('2');
  ...
  it('10.pow(-1) should result in 0.1 (DP=1)', () => {
  ...
  it('3.pow(-2) should result in 0.11111 (DP=5, RM=roundHalfUp)', () => {
    BigIntPrimitive.DP = 5;
  ...
  it('3.pow(-2) should result in 0.1111 (DP=4, RM=roundDown)', () => {
  ...
  it('0.pow(-2) should throw an error for division by zero', () => {
